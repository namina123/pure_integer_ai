"""Build a fresh integer-only course for the currently observed graph shapes.

The first three held-out variants are bound to masks and role ordinals observed
in the live three-graph QueryState.  Remaining variants keep the paired
development/held-out contract without introducing source prose or answers.
"""
...
ROOT = Path(r"K:/pure_integer_ai_work/current_runs/broad-structural-course-20260922b")
...
def row(index: int, mask: int, roles: tuple[tuple[int, int], tuple[int, int]]) -> list[object]:
    lead = (0x5200 + index, 0x5220 + index)
    middle = (0x5240 + index, 0x5260 + index, 0x5280 + index)
    tail = (0x52A0 + index, 0x52C0 + index, 0x3002)
    return [[91561, mask, 0, 9400 + index],
            [literal(*lead), graph(*roles[0]), literal(*middle),
             graph(*roles[1]), literal(*tail)]]
...
def manifest(data: bytes, split: str, count: int) -> bytes:
    return json.dumps({
        ...
        "graph_slot_count": 2 * count,
        "license_ids": ["MIT"],
        "minimum_graph_slots_per_variant": 2,
        "official_url": "https://opensource.org/license/mit",
        "relation_qualified_graph_slots": 2 * count,
        "rights_basis": "Project-owned integer graph composition.",
        "schema_version": 2,
        "selection_rule": "Observed three-graph QueryState mask plus two ordered typed graph roles.",
        "source_id": "broad-structural-course-20260922b-" + split,
...
def validate(rows: tuple[list[object], ...]) -> None:
    ...
    for condition, parts in rows:
        ...
        if len(roles) != 2 or len(set(roles)) != 2:
            raise ValueError("each variant needs two distinct graph roles")
...
def main() -> int:
    ...
    states = (HOT | MULTI, COLD | MULTI, HOT | CONFLICT, COLD | CONFLICT,
              ...
              HOT | REF_OPEN, COLD | REF_RESOLVED, HOT | TOPIC,
              COLD | TOPIC)
    ...
    for i, state in enumerate(states):
        for obj in objects:
            ...
            if mask in {item[0] for item in observed}:
                continue
            role = ((1, 1), (1, 2)) if i % 2 else ((4, 1), (4, 2))
            extra.append((mask, role))
    specs = list(observed) + extra
    if len(specs) < 64:
        raise ValueError("course inventory is incomplete")
    heldout_specs = tuple(specs[:20])
    development_specs = tuple(specs[20:64])
