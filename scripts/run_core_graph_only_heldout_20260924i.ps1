...
$runRoot = 'K:\pure_integer_ai_work\current_runs\core-graph-only-heldout-20260924i'; $control = 'K:\pure_integer_ai_work\orchestration\core-graph-only-heldout-20260924i-control'; $state = Join-Path $control 'state.json'; $stdout = Join-Path $control 'stdout.txt'; $stderr = Join-Path $control 'stderr.txt'
...
try {
  ...
  $process = Start-Process -FilePath 'python' -ArgumentList @('-u','scripts\run_core_graph_only_heldout_20260924i.py') -WorkingDirectory $repo -RedirectStandardOutput $stdout -RedirectStandardError $stderr -WindowStyle Hidden -PassThru
