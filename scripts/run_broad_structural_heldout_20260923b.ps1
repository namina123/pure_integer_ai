...
$database = 'K:\pure_integer_ai_work\current_runs\discourse-relation-response-graph-20260917b\training.sqlite3'; $courseRoot = 'K:\pure_integer_ai_work\current_runs\broad-structural-heldout-course-20260923b'; $runRoot = 'K:\pure_integer_ai_work\current_runs\broad-structural-heldout-session-20260923b'; $control = 'K:\pure_integer_ai_work\orchestration\broad-structural-heldout-20260923b-control'; $state = Join-Path $control 'state.json'; $stdout = Join-Path $control 'session.stdout.txt'; $stderr = Join-Path $control 'session.stderr.txt'
...
try {
  ...
  $g1 = '7,0,0,0,1,1,1,1,1,15,1,206,3917972928851022813,0,0,0,0,1,1,1,1,1,2,1,1'; $g2 = '7,0,0,0,1,1,1,1,1,15,1,206,903069331810179240,0,0,0,0,1,1,1,1,1,2,1,1'; $g3 = '7,0,0,0,1,1,1,1,1,15,1,206,585343985805089038,0,0,0,0,1,1,1,1,1,2,1,1'
  ...
  & python -u -m pure_integer_ai.experiments.run_unknown_input_structure_slice --database $database --run-root $runRoot --session-id 92095 --input $in1 --input $in2 --input $in3 --graph-object-key $g1 --graph-object-key $g2 --graph-object-key $g3 --heldout-course $course --heldout-manifest $manifest *> $stdout
