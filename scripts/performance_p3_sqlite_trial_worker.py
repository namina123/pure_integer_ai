"""PERF-P3 benchmark-only SQLite schema/init/read trial.

The worker intentionally stays outside the production backend.  It measures
the existing append-only repository contract and verifies that the measured
read path survives reclaim, restart, and an interrupted write.
"""
...
class _InjectedFailure(RuntimeError):
    """Fault used only to exercise the interrupted-write recovery path."""
...
def _install_backend_timers(
    ...
    """Wrap instance methods so this worker never changes production code."""
...
def run_trial(scale: int, database: Path) -> dict[str, object]:
    """Run one deterministic, bounded trial against a new SQLite file."""
