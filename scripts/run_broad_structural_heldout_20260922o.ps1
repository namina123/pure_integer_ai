...
$courseRoot = 'K:\pure_integer_ai_work\current_runs\broad-structural-course-20260922o'; $runRoot = 'K:\pure_integer_ai_work\current_runs\broad-structural-heldout-session-20260922o'
$control = 'K:\pure_integer_ai_work\orchestration\broad-structural-heldout-20260922o-control'; $state = Join-Path $control 'state.json'; $stdout = Join-Path $control 'session.stdout.txt'; $stderr = Join-Path $control 'session.stderr.txt'
...
try {
  ...
  $course = Join-Path $courseRoot 'heldout_response_course.int.json'; $manifest = Join-Path $courseRoot 'heldout_manifest.json'
  foreach ($path in @($database,$course,$manifest)) { if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "missing input: $path" } }
  $env:PYTHONPATH = $source
  function Integer-Surface([int[]] $values) { return -join ($values | ForEach-Object { [char] $_ }) }
  $in1 = Integer-Surface @(27773,36710); $in2 = Integer-Surface @(23545,35937); $in3 = Integer-Surface @(20027,39064)
  $graph1 = '7,0,0,0,1,1,1,1,1,15,1,206,3917972928851022813,0,0,0,0,1,1,1,1,1,2,1,1'; $graph2 = '7,0,0,0,1,1,1,1,1,15,1,206,903069331810179240,0,0,0,0,1,1,1,1,1,2,1,1'; $graph3 = '7,0,0,0,1,1,1,1,1,15,1,206,585343985805089038,0,0,0,0,1,1,1,1,1,2,1,1'
  ...
  & python -u -m pure_integer_ai.experiments.run_unknown_input_structure_slice --database $database --run-root $runRoot --session-id 92087 --input $in1 --input $in2 --input $in3 --graph-object-key $graph1 --graph-object-key $graph2 --graph-object-key $graph3 --heldout-course $course --heldout-manifest $manifest *> $stdout
  if ($LASTEXITCODE -ne 0) { throw "heldout session exit $LASTEXITCODE" }
  $receipt = Join-Path $runRoot 'receipt.json'; if (-not (Test-Path -LiteralPath $receipt)) { throw 'heldout receipt missing' }
