...
# #729 M5 produced_refs FIFO cap（oracle 标·硬编码 module-level 常量守 bit-identical·
# 禁 set_ 函数）。须 > 单段最大 unit 数（否则本段
# 已 append 的 unit 被截断删破去重语义）。
PRODUCED_REFS_WINDOW = 48
# #729 M5 章边界 carry → prior_topic_refs FIFO cap（oracle 标·硬编码守 bit-identical）。
# prior_topic_refs 是 M7 主题锚 stub 字段·#729 激活
...
class WorkMemory:
    ...
    def add_produced(self, ref: ConceptRef, *, window: int = PRODUCED_REFS_WINDOW) -> None:
        ...
        #729 M5 produced_refs cap：window 满 del [:over] 截断保近期。
    ...
    def add_prior_topic(self, ref: ConceptRef, *, window: int = PRIOR_TOPIC_REFS_WINDOW) -> None:
        ...
        #729 M5 章边界 carry：章末 parts 子集 snapshot 进 prior_topic_refs。
