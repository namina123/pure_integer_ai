...
**bit-identical（gate OFF·CI）**：无 alias_facts.txt（CI/生产 default 无 PURE_INTEGER_AI_LOCAL_DIR）→ resolve_alias_facts 返 []
...
def bootstrap_alias_edges(concept_index, edge_store: EdgeStore, backend,
    ...
    **无文件零副作用硬守（bit-identical·P0）**：pairs 空 → 立即 return 0·**绝不调 concept_index.ensure / set_mark /
      build_refers_stable_edge / query_from**（无 PURE_INTEGER_AI_LOCAL_DIR → resolve 返 [] → 图与不接 P0b bit-identical）。
