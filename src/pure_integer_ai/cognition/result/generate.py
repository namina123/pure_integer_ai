...
def generate_output(dag_path: PathResult, graph: ConceptGraph, workmem: Any,
    ...
    for layer in dag_path.topo_layers:
        ...
        for unit in units:
            ...
            # #729 M5 produced_refs FIFO cap：add_produced 保序去重 + 截断保近期
            # （window=PRODUCED_REFS_WINDOW=48·硬编码守 bit-identical）。cap > 单段 unit 数守去重语义。
...
def carry_to_workmem(workmem: Any, parts: list[OutputPart]) -> None:
    ...
    prior_topic_refs cap=PRIOR_TOPIC_REFS_WINDOW=16 FIFO 截断保近期。
