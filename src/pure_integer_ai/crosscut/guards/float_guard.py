...
双层（§9.2 A7）：
  - 运行时层（本模块 DEBUG）：可在生产热路径 export PURE_INTEGER_AI_AUDIT_FLOAT=0 关闭，
...
# 默认 ON（守测试/dev/CI）。生产热路径 export PURE_INTEGER_AI_AUDIT_FLOAT=0 关运行时守卫。
# 须在 import 本模块前设 env（run 脚本 export 先于 import）。
DEBUG = read_project_env("PURE_INTEGER_AI_AUDIT_FLOAT", "1") != "0"
