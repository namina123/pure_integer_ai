...
from pure_integer_ai.crosscut.environment import read_project_env


def _flag(env_name: str, default: bool) -> bool:
    """env 覆盖（import 时读一次·测试可翻模块属性）。"""
    v = read_project_env(env_name)
    if v is None:
        return default
...
_GATE_OVERRIDES: contextvars.ContextVar[
    tuple[dict[str, bool], ...]
] = contextvars.ContextVar("pure_integer_ai_gate_overrides", default=())
...
# B4 频率观察修正（β_arith 病实测确认·consumer fire 但 w_freq 塌缩）。
