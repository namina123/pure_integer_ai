"""W06-R02 active 集合关系结构的 Generation consumer 与 postcheck。"""
from __future__ import annotations

from pure_integer_ai.cognition.shared.identity import ObjectIdentity
from pure_integer_ai.cognition.shared.scope_identity import document_scope
from pure_integer_ai.cognition.shared.set_relation import SetRelationBudget
...
W06_R02_POSTCHECK_BUDGET = SetRelationBudget(32, 128, 512, 32, 32)
_GENERATION_DIMENSION = LosslessIntegerKey((W06_R02_RUNTIME_NAMESPACE, 803))
_POSTCHECK_VERIFIER = LosslessIntegerKey((W06_R02_RUNTIME_NAMESPACE, 804))
_RESULT_KEYS = {
    W06_R02_OUTCOME_SUPPORT: LosslessIntegerKey((
        W06_R02_RUNTIME_NAMESPACE, 805)),
    W06_R02_OUTCOME_REFUTE: LosslessIntegerKey((
        W06_R02_RUNTIME_NAMESPACE, 806)),
}
...
def query_for_candidate(
        ...
        *, request_key: LosslessIntegerKey,
        ...
        budget: SetRelationBudget = W06_R02_POSTCHECK_BUDGET,
        ) -> W06R02SetQuery:
    """从 typed Role 构造不含表层预期的集合关系查询。"""
    if (not isinstance(candidate, W06RelationCandidate)
            or candidate.substage_key != W06_R02_SUBSTAGE):
        raise W06R02ContractError("set query candidate 不属于 R02")
    if not isinstance(endpoint_resolver, SetRelationEndpointResolver):
        raise TypeError("set query endpoint resolver 类型非法")
    left, right = tuple(
...
class W06R02GenerationRuntime:
    """从 direct active set fact 形成 option、exact Use 和独立 postcheck。"""

    def __init__(
            self,
            view: W06R02View,
            understanding: W06R02QueryRuntime,
            ) -> None:
        if not isinstance(view, W06R02View):
            raise TypeError("R02 Generation view 类型非法")
        if (not isinstance(understanding, W06R02QueryRuntime)
                or understanding.view is not view
                or understanding.consumer != "UNDERSTANDING"):
            raise TypeError("R02 Generation 必须共享 Understanding view")
        self.view = view
        self.understanding = understanding
        self._request_keys: set[LosslessIntegerKey] = set()
        self._choices: list[W06R02GenerationChoice] = []
        self._uses: list[W06R02GenerationUse] = []
        self._outcomes: list[W06R02GenerationOutcome] = []

    @property
    def choices(self) -> tuple[W06R02GenerationChoice, ...]:
        return tuple(self._choices)

    @property
    def uses(self) -> tuple[W06R02GenerationUse, ...]:
        return tuple(self._uses)

    @property
    def outcomes(self) -> tuple[W06R02GenerationOutcome, ...]:
    ...
    def _options(
            self, request: W06R02GenerationRequest,
            ) -> tuple[W06R02GenerationOption, ...]:
    ...
    def choose(
        ...
        if not isinstance(request, W06R02GenerationRequest):
            raise TypeError("R02 generation request 类型非法")
        if request.request_key in self._request_keys:
            raise W06R02ContractError("重复 R02 generation request key")
        options = self._options(request)
        if options:
            status = W06_R02_GENERATION_READY
        elif (not self.view.protocol.set_relation_bridge_connected
              or not self.view.protocol.generation_connected):
            status = W06_R02_GENERATION_UNKNOWN
        elif (not self.view.protocol.direction_connected
              or not self.view.protocol.source_scope_connected):
            status = W06_R02_GENERATION_REJECTED
        else:
            status = W06_R02_GENERATION_UNKNOWN
        choice = W06R02GenerationChoice(
            ...
            LosslessIntegerKey((
                ...
                *self.view.protocol.stable_key(),
                int(status == W06_R02_GENERATION_READY),
            )),
        )
    ...
    def adopt(
        ...
        """采用 option，并经 SetRelationRuntime 提交 exact direct fact Use。"""
        if choice not in self._choices:
            raise W06R02ContractError("generation choice 不属于当前 runtime")
        if choice.status != W06_R02_GENERATION_READY:
            raise W06R02ContractError("非 READY generation choice 不得采用")
        selected = tuple(
            item for item in choice.options if item.stable_key() == option_key)
        if len(selected) != 1:
            raise W06R02ContractError("generation option key 不属于 choice")
        if any(item.choice == choice for item in self._uses):
            raise W06R02ContractError("同一 generation choice 不得重复采用")
        option = selected[0]
        ...
        query = query_for_candidate(
            candidate,
            request_key=LosslessIntegerKey((
                W06_R02_RUNTIME_NAMESPACE, 721, len(self._uses) + 1)),
    ...
    def verify(
            self, use: W06R02GenerationUse,
            ) -> W06R02GenerationOutcome:
        """独立重验 authorization、typed 结构、surface 和集合关系结论。"""
        if use not in self._uses:
            raise W06R02ContractError("generation Use 不属于当前 runtime")
        option = use.option
        ...
        if candidate is not None and self.view.protocol.postcheck_connected:
            ...
            set_status = resolution.status
            recovered = (
                resolution.status == W06_R02_SUPPORTED
        ...
        verdict = (
            ...
            if all((
                ...
                surface_valid,
                set_status == W06_R02_SUPPORTED,
                recovered,
            ))
            else W06_R02_OUTCOME_REFUTE
        )
        ...
        outcome = W06R02GenerationOutcome(
            use,
            verdict,
            GenerationChoiceOutcomeRef(
                outcome_key,
...
def generation_request_for_candidate(
    ...
    if (not isinstance(candidate, W06RelationCandidate)
            or candidate.substage_key != W06_R02_SUBSTAGE):
        raise W06R02ContractError("generation candidate 不属于 R02")
    return W06R02GenerationRequest(
        request_key,
        candidate.proposition.proposition,
        candidate.relation_family,
        candidate.directionality,
