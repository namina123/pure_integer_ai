"""LC-16 TABLE_GRID payload、grid adapter 与冻结 parent pack 目录。"""
...
TABLE_GRID_CARRIER_MANIFEST_PATH = (
    "data/ph2/manifests/lc16_table_grid_carrier_v1.json")
...
_EVIDENCE_PATHS = (
    ("src/pure_integer_ai/experiments/ph2_table_grid_carrier_adapter.py", "ADAPTER"),
    ("src/pure_integer_ai/experiments/ph2_table_grid_carrier_catalog.py", "CATALOG"),
    ("src/pure_integer_ai/experiments/ph2_table_grid_carrier_contract.py", "CONTRACT"),
    ("pyproject.toml", "DEPENDENCY"),
    ("tests/test_lc16_table_grid_carrier_adapter.py", "TEST"),
)


class TableGridCarrierCatalogError(RuntimeError):
...
def _path(root: Path, relative_path: str) -> Path:
    ...
    try:
        ...
        raise TableGridCarrierCatalogError("catalog 路径逃逸") from error
    return target


def _identity(path: Path) -> tuple[int, str]:
    if not path.is_file():
        raise TableGridCarrierCatalogError(f"catalog 文件缺失: {path.name}")
    payload = path.read_bytes()
    return len(payload), hashlib.sha256(payload).hexdigest()


def _evidence_files(root: Path) -> tuple[TableGridCarrierEvidenceFile, ...]:
    result = []
    for relative_path, role in _EVIDENCE_PATHS:
        byte_count, sha256 = _identity(_path(root, relative_path))
        result.append(TableGridCarrierEvidenceFile(
            relative_path, role, byte_count, sha256))
    return tuple(sorted(result, key=lambda item: (item.relative_path, item.role)))


def _parser_version() -> str:
    # parser 身份属于标准库 CSV 与 data-only grid layout 合同，稳定版本
    # 取 schema 版本，而不是宿主 Python 的补丁版本。
...
def _check_budget(
    ...
    if raw_unit_count > budget.max_raw_units:
        raise TableGridCarrierCatalogError("TABLE_GRID raw 超出 parent budget")
    if previous_raw_unit_count > budget.max_raw_units:
        raise TableGridCarrierCatalogError("TABLE_GRID previous raw 超出 parent budget")
    if structure_node_count > budget.max_structure_nodes:
        raise TableGridCarrierCatalogError("TABLE_GRID structure 超出 parent budget")
    if revision_count > budget.max_edges:
        raise TableGridCarrierCatalogError("TABLE_GRID revision 超出 parent budget")
    if envelope_count > budget.max_depth:
        raise TableGridCarrierCatalogError("TABLE_GRID envelope 超出 parent budget")


def build_table_grid_carrier_manifest(
        repository_root: str | Path,
        ) -> TableGridCarrierPayloadManifest:
    """从冻结 parent 与七行 payload 重建 grid/read-order evidence manifest。"""
    ...
    if parent_byte_count <= 0 or parent_sha256 != PARENT_PACK_SHA256:
        raise TableGridCarrierCatalogError("parent pack 文件身份漂移")
    try:
        parent = read_typed_carrier_pack_manifest(parent_path)
        verify_typed_carrier_pack_files(parent, repository_root=root)
    except Exception as error:
        raise TableGridCarrierCatalogError("parent pack 无法严格回验") from error
    if parent.sha256() != parent_sha256:
        raise TableGridCarrierCatalogError("parent pack canonical SHA 漂移")
    ...
    if len(budgets) != 1 or len(cases) != budgets[0].max_cases:
        raise TableGridCarrierCatalogError("parent TABLE_GRID case/budget 漂移")
    budget = budgets[0]
    expected_anchors = (ANCHOR_TEXT_RANGE, ANCHOR_GRID_RECT, ANCHOR_DOCUMENT_REGION)
    if any(item.anchor_kinds != expected_anchors for item in cases):
        raise TableGridCarrierCatalogError("parent TABLE_GRID anchor 漂移")
    sample_path = _path(root, TABLE_GRID_CARRIER_SAMPLE_PATH)
    _, sample_sha256 = _identity(sample_path)
    try:
        records = read_table_grid_carrier_records(sample_path)
    except Exception as error:
        raise TableGridCarrierCatalogError("TABLE_GRID sample 无法严格回读") from error
    if tuple(item.case_key for item in records) != tuple(item.case_key for item in cases):
        raise TableGridCarrierCatalogError("sample case_key 与 parent 不一致")
    if tuple(item.sample_kind for item in records) != tuple(item.sample_kind for item in cases):
        raise TableGridCarrierCatalogError("sample kind 与 parent 不一致")
    if any(item.payload_state != "UNMATERIALIZED" or item.runtime_state != "NOT_RUN"
           for item in cases):
        raise TableGridCarrierCatalogError("parent 已越过 payload/runtime 边界")
    _parser_version()
    materializations = []
    for record in records:
        try:
            adapted = adapt_table_grid_carrier_record(record)
            payload = serialize_table_grid_carrier_materialization(adapted)
            restored = deserialize_table_grid_carrier_materialization(payload, record)
        except Exception as error:
            raise TableGridCarrierCatalogError(
                f"adapter 无法物化 {record.sample_kind}") from error
        if restored != adapted:
            raise TableGridCarrierCatalogError("adapter reload identity 漂移")
        _check_budget(
            budget,
        ...
        materializations.append(TableGridMaterializationIdentity(
            record.case_key,
            len(payload),
    ...
    return TableGridCarrierPayloadManifest(
        FORMAT_VERSION,
        ARTIFACT_VERSION,
        ARTIFACT_STATUS,
        PARENT_PACK_PATH,
