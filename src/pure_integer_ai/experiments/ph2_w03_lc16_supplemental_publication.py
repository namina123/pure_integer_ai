"""W-03 LC-16 supplemental receipt 的公开、不可覆盖发布接口。"""
...
class W03Lc16SupplementalPublicationError(RuntimeError):
    """supplemental receipt 泄漏、损坏或覆盖发布。"""


@dataclass(frozen=True)
class W03Lc16SupplementalPublication:
...
def _reject_private(value: Any) -> None:
    ...
    if isinstance(value, dict):
        if forbidden.intersection(value):
            raise W03Lc16SupplementalPublicationError(
...
def _publication(path: Path, payload: bytes, value: dict[str, Any]) -> W03Lc16SupplementalPublication:
    """由 canonical bytes 构造公开 publication identity。"""
    status = value.get("status")
    if status not in {"PASS", "FAIL", "NE", "BLOCKED"}:
        raise W03Lc16SupplementalPublicationError("supplemental receipt status 非法")
    return W03Lc16SupplementalPublication(
        path,
...
def publish_w03_lc16_supplemental_report(
        path: str | Path,
        report: W03Lc16SupplementalReport,
        ) -> W03Lc16SupplementalPublication:
    """排他创建 supplemental receipt，既存目标一律拒绝覆盖。"""
    if not isinstance(report, W03Lc16SupplementalReport):
        raise TypeError("report 类型非法")
    target = Path(path).resolve()
    if not target.parent.is_dir():
        raise W03Lc16SupplementalPublicationError("receipt 父目录不存在")
    value = report.to_public_dict()
    _reject_private(value)
    if report.status != "PASS":
        raise W03Lc16SupplementalPublicationError(
            "只有 PASS supplemental report 允许公开发布")
    if value.get("artifact_kind") != ARTIFACT_KIND:
        raise W03Lc16SupplementalPublicationError("receipt artifact kind 非法")
...
def read_w03_lc16_supplemental_report(
        path: str | Path,
        ) -> tuple[dict[str, Any], W03Lc16SupplementalPublication]:
    ...
    _reject_private(value)
    if value.get("artifact_kind") != ARTIFACT_KIND:
        raise W03Lc16SupplementalPublicationError("supplemental receipt kind 非法")
    try:
        W03Lc16SupplementalReport.from_public_dict(value)
...
__all__ = [
    "RECEIPT_RELATIVE_PATH", "W03Lc16SupplementalPublication",
