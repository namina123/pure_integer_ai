"""W-04 private evaluator family 的独立生成、冻结与唯一 guard。"""
...
from pure_integer_ai.experiments.ph2_w04_contract import W04_EVALUATION_ORDER
...
def _nonce_bytes(nonce: tuple[int, ...]) -> bytes:
    if (not isinstance(nonce, tuple) or not nonce
        ...
        raise W04PrivateEvaluationError("private nonce 必须是 byte tuple")
    return bytes(nonce)


@dataclass(frozen=True)
class W04PrivateFamilyDocuments:
    ...
    def files(self) -> tuple[tuple[str, bytes], ...]:
        return (
            (W04_PRIVATE_SOURCE_NAME, self.source_bytes),
            (W04_PRIVATE_SCHEMA_NAME, self.schema_bytes),
            (W04_PRIVATE_CASE_NAME, self.case_bytes),
            (W04_PRIVATE_LABEL_NAME, self.label_bytes),
            (W04_PRIVATE_CLUSTER_NAME, self.cluster_bytes),
        )


def build_w04_private_family_documents(
        *,
        candidate_contract_sha256: str,
        candidate_host_freeze_sha256: str,
        nonce: tuple[int, ...] = (7, 11, 19, 23),
        ) -> W04PrivateFamilyDocuments:
    ...
    family_key = _digest({
        ...
        "owner_key": W04_PRIVATE_OWNER_KEY,
    })
    source = {
        "artifact_kind": "PH2_W04_PRIVATE_SOURCE",
        "candidate_contract_sha256": candidate_contract,
        "candidate_host_freeze_sha256": candidate_host,
        ...
        "nonce_commitment": nonce_commitment,
        "owner_key": W04_PRIVATE_OWNER_KEY,
        "source_key": [2026, 804, 1],
    }
    schema_key = _digest({
        "evaluation_order": list(W04_EVALUATION_ORDER),
        "failure_phases": list(W04_EVALUATOR_FAILURE_PHASES),
        "family_key": family_key,
    })
    schema = {
        "artifact_kind": "PH2_W04_PRIVATE_SCHEMA",
        ...
        "evaluation_order": list(W04_EVALUATION_ORDER),
        "failure_phases": list(W04_EVALUATOR_FAILURE_PHASES),
        "fault_registry": list(W04_EVALUATOR_PHASES),
        "format_version": 1,
        "label_fields": [
    ...
    labels = []
    clusters = []
    for ordinal, dimension in enumerate(W04_EVALUATION_ORDER, start=1):
        case_key = _digest({
            "dimension": dimension,
            "family_key": family_key,
            "ordinal": ordinal,
        ...
        challenge = [2026, 804, ordinal, *list(_nonce_bytes(nonce))]
        cases.append({
            "case_key": case_key,
    ...
    case_bytes = canonical_json_bytes({
        "artifact_kind": "PH2_W04_PRIVATE_CASES",
        "case_count": len(cases),
        "cases": cases,
        "format_version": 1,
    })
    label_bytes = canonical_json_bytes({
        "artifact_kind": "PH2_W04_PRIVATE_LABELS",
        "format_version": 1,
        "labels": labels,
    })
    cluster_bytes = canonical_json_bytes({
        "artifact_kind": "PH2_W04_PRIVATE_CLUSTERS",
        "clusters": clusters,
        "format_version": 1,
    })
    return W04PrivateFamilyDocuments(
        source_bytes,
        schema_bytes,
        case_bytes,
        label_bytes,
...
def _validate_root(root: Path, forbidden_roots: tuple[str | Path, ...]) -> None:
    for forbidden in forbidden_roots:
        ...
        if (root == path or root.is_relative_to(path)
            ...
            raise W04PrivateEvaluationError("private family root 未隔离")


def publish_w04_private_family(
        artifact_root: str | Path,
        documents: W04PrivateFamilyDocuments,
        *,
        forbidden_roots: tuple[str | Path, ...] = (),
        ) -> tuple[Path, str]:
    """以排他写发布新 private family，不覆盖既有 family。"""
    if not isinstance(documents, W04PrivateFamilyDocuments):
    ...
    for name, payload in documents.files():
        ...
        try:
            ...
            raise W04PrivateEvaluationError("private family 文件不可覆盖") from exc
    inventory = [
        {"path": name, "sha256": _sha256(payload), "size_bytes": len(payload)}
        for name, payload in documents.files()
    ]
    freeze = {
        "artifact_kind": "PH2_W04_PRIVATE_FAMILY_FREEZE",
    ...
    target = root / W04_PRIVATE_FAMILY_FREEZE_NAME
    encoded = canonical_json_bytes(freeze)
    try:
        with target.open("xb") as handle:
            handle.write(encoded)
    except FileExistsError as exc:
        raise W04PrivateEvaluationError("private family freeze 不可覆盖") from exc
    return target, _sha256(encoded)


def consume_w04_private_first_run_guard(
        artifact_root: str | Path,
        *,
    ...
    freeze = root / W04_PRIVATE_FAMILY_FREEZE_NAME
    if (not freeze.is_file() or freeze.is_symlink()
            or _sha256(freeze.read_bytes()) != expected):
        raise W04PrivateEvaluationError("private family freeze SHA 漂移")
    try:
        value = json.loads(freeze.read_text(encoding="utf-8"))
    except (OSError, UnicodeError, json.JSONDecodeError) as exc:
        raise W04PrivateEvaluationError("private family freeze 无法读取") from exc
    if value.get("formal_run_count") != 0:
        raise W04PrivateEvaluationError("private family 已运行")
    payload = canonical_json_bytes({
        "artifact_kind": "PH2_W04_PRIVATE_FIRST_RUN_GUARD",
        "family_freeze_sha256": expected,
        "formal_run_count_after": 1,
        "formal_run_count_before": 0,
        "format_version": 1,
    })
    target = root / W04_PRIVATE_FIRST_RUN_GUARD_NAME
    try:
        with target.open("xb") as handle:
            handle.write(payload)
    except FileExistsError as exc:
        raise W04PrivateEvaluationError("private family guard 已消费，不可重跑") from exc
