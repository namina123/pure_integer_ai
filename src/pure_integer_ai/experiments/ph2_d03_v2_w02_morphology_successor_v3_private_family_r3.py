"""Metadata-only freeze and single-use guard for the R3 private family."""
...
W02_MORPH_V3_PRIVATE_R3_FAMILY_VERSION = (
    "PH2-D03-V2-W02-MORPHOLOGY-SUCCESSOR-V3-PRIVATE-R3-FAMILY-V1"
)
W02_MORPH_V3_PRIVATE_R3_FAMILY_FREEZE_PATH = (
...
W02_MORPH_V3_PRIVATE_R3_FAMILY_CODE_PATHS = (
    ...
    "src/pure_integer_ai/experiments/"
    "ph2_d03_v2_w02_morphology_successor_v3_private_owner_r3_contract.py",
    "src/pure_integer_ai/experiments/"
    "ph2_d03_v2_w02_morphology_successor_v3_private_owner_r3_audit.py",
    "src/pure_integer_ai/experiments/"
    "ph2_d03_v2_w02_morphology_successor_v3_private_owner_r3.py",
    "src/pure_integer_ai/experiments/"
    "ph2_d03_v2_w02_morphology_successor_v3_private_evaluator_r3.py",
    "src/pure_integer_ai/experiments/"
    "ph2_d03_v2_w02_morphology_successor_v3_private_family_r3.py",
    "src/pure_integer_ai/experiments/"
    "ph2_d03_v2_w02_morphology_successor_v3_private_publication_r3.py",
    "src/pure_integer_ai/experiments/"
    "run_ph2_d03_v2_w02_morphology_successor_v3_private_evaluation_r3.py",
    "tests/test_ph2_d03_v2_w02_morphology_successor_v3_private_owner_r3.py",
    "tests/test_ph2_d03_v2_w02_morphology_successor_v3_private_evaluation_r3.py",
    "tests/test_ph2_d03_v2_w02_morphology_successor_v3_private_family_r3.py",
)
W02_MORPH_V3_PRIVATE_R3_FORMAL_FAMILY_NAME = (
    ".d03-w02-morph-successor-v3-private-formal-r3-20260810-a"
)
W02_MORPH_V3_PRIVATE_R3_FAMILY_DOCUMENT = "private-family-freeze.json"
W02_MORPH_V3_PRIVATE_R3_REGISTRATION_DOCUMENT = (
    "private-family-registration.json"
)
W02_MORPH_V3_PRIVATE_R3_GUARD_AVAILABLE = "run-guard/available.guard.json"
W02_MORPH_V3_PRIVATE_R3_GUARD_CONSUMED = "run-guard/consumed.guard.json"
W02_MORPH_V3_PRIVATE_R3_RUN_INTENT = "run-guard/run-intent.json"
W02_MORPH_V3_PRIVATE_R3_EXPOSURE_LEDGER = "exposure-ledger"


# object-model: exception
class W02MorphologySuccessorV3PrivateR3FamilyError(RuntimeError):
    """The R3 public freeze, dependency chain, or single-use guard drifted."""


def _sha256(value: object, *, where: str) -> str:
    if (not isinstance(value, str) or len(value) != 64
            or any(char not in "0123456789abcdef" for char in value)):
        raise W02MorphologySuccessorV3PrivateR3FamilyError(
...
def _repository_file(repository: Path, relative: str) -> Path:
    ...
    if (not relative or "\\" in relative or pure.is_absolute()
        ...
        raise W02MorphologySuccessorV3PrivateR3FamilyError(
            "R3 private family repository file is invalid")
    return target
...
def _code_rows(repository: Path) -> tuple[list[dict[str, object]], str]:
    rows = [
        ...
        for relative in W02_MORPH_V3_PRIVATE_R3_FAMILY_CODE_PATHS
    ]
...
def _owner_budget(value: object) -> V2EvaluatorResourceBudget:
    if (not isinstance(value, dict) or set(value) != {
        ...
        raise W02MorphologySuccessorV3PrivateR3FamilyError(
            "R3 owner resource limits drifted")
    return V2EvaluatorResourceBudget(
        512,
        value["max_logic_operations"],
        value["max_payload_bytes"],
        value["max_payload_gets"],
        value["max_records"],
        4,
    )
...
def _family_root(value: str | Path, *, require_exists: bool) -> Path:
    ...
    if (root.name != W02_MORPH_V3_PRIVATE_R3_FORMAL_FAMILY_NAME
            or root.is_symlink()
            or (require_exists and not root.is_dir())):
        raise W02MorphologySuccessorV3PrivateR3FamilyError(
            "R3 private formal family root is invalid")
    return root
...
def _dependency_state(repository: Path) -> dict[str, Any]:
    ...
    source_extension = read_blind_private_source_extension_v4_manifest(repository)
    owner, files = read_w02_morphology_successor_v3_private_owner_r3_receipt(
        repository)
    paths = {
        ...
        "source_extension_v4": BLIND_PRIVATE_SOURCE_EXTENSION_V4_PATH,
        "owner_r3_receipt": W02_MORPH_V3_PRIVATE_R3_OWNER_RECEIPT_PATH,
    }
    identities = {
    ...
    if (candidate.get("status") != "W02_CANDIDATE_ARTIFACT_FROZEN"
            ...
            or source_extension.get("consumed_r2_owner_reuse_authorized") != 0
            or source_extension.get("previous_consumed_r2_family", {}).get(
            ...
            != W02_MORPH_V3_PRIVATE_LAYOUTS):
        raise W02MorphologySuccessorV3PrivateR3FamilyError(
            "R3 private family dependency chain did not close")
    zero_values = (
    ...
    if any(value != 0 for value in zero_values):
        raise W02MorphologySuccessorV3PrivateR3FamilyError(
            "R3 private dependency already consumed private data")
    chain = {
        ...
        "owner_r3_receipt_file_sha256":
            identities["owner_r3_receipt"]["sha256"],
        "source_extension_v4_file_sha256":
            identities["source_extension_v4"]["sha256"],
...
def w02_morphology_successor_v3_private_r3_guard_value(
        *, family_commitment: str, artifact_chain_sha256: str,
        code_freeze_sha256: str) -> dict[str, object]:
    return {
        "artifact_chain_sha256": _sha256(
            artifact_chain_sha256, where="R3 private artifact chain"),
        "artifact_kind": (
            "PH2_D03_V2_W02_MORPHOLOGY_SUCCESSOR_V3_PRIVATE_R3_"
            "FIRST_RUN_GUARD"),
        "code_freeze_sha256": _sha256(
            code_freeze_sha256, where="R3 private code freeze"),
        "family_commitment": _sha256(
            family_commitment, where="R3 private family commitment"),
        "formal_private_evaluation_runs": 0,
        "format_version": 1,
        "guard_consumed": 0,
        "guard_version": W02_MORPH_V3_PRIVATE_R3_FAMILY_VERSION,
        "owner_family_key": W02_MORPH_V3_PRIVATE_R3_OWNER_FAMILY_KEY,
...
def build_w02_morphology_successor_v3_private_r3_family_freeze(
    ...
    return {
        ...
        "artifact_kind": (
            "PH2_D03_V2_W02_MORPHOLOGY_SUCCESSOR_V3_PRIVATE_R3_"
            "FAMILY_FREEZE"),
        "artifact_version": W02_MORPH_V3_PRIVATE_R3_FAMILY_VERSION,
        "code_files": code_rows,
        "code_freeze_sha256": code_sha,
        "dimension_denominator_counts": owner["dimension_denominator_counts"],
        "evaluator_version": W02_MORPH_V3_PRIVATE_R3_EVALUATOR_VERSION,
        ...
        "ne_policy": "BLOCK",
        "next_action": "W02_FORMAL_BLIND_PRIVATE_R3_EVALUATION",
        ...
        "owner_receipt_file_sha256":
            dependency["identities"]["owner_r3_receipt"]["sha256"],
        ...
        "parent_evidence_files": [
            ...
            for key in (
                ...
                "v3_shadow_report", "source_extension_v4",
                "owner_r3_receipt",
            )
        ],
        ...
        "stage_key": "W-02",
        "status": "W02_SUCCESSOR_V3_R3_BLIND_PRIVATE_FAMILY_FROZEN",
...
def publish_w02_morphology_successor_v3_private_r3_family_freeze(
    ...
    target = repository / Path(*PurePosixPath(
        W02_MORPH_V3_PRIVATE_R3_FAMILY_FREEZE_PATH).parts)
    write_immutable_json(value, target)
    if target.read_bytes() != canonical_json_bytes(value) + b"\n":
        raise W02MorphologySuccessorV3PrivateR3FamilyError(
            "R3 private family freeze publication drifted")
    return target


def read_w02_morphology_successor_v3_private_r3_family_freeze(
        repository_root: str | Path) -> dict[str, object]:
    repository = Path(repository_root).resolve()
    target = _repository_file(
        repository, W02_MORPH_V3_PRIVATE_R3_FAMILY_FREEZE_PATH)
    value = read_canonical_object(target)
    expected = build_w02_morphology_successor_v3_private_r3_family_freeze(
        repository)
    if value != expected or target.read_bytes() != canonical_json_bytes(value) + b"\n":
        raise W02MorphologySuccessorV3PrivateR3FamilyError(
            "R3 private family freeze no longer matches live identities")
    return value


def w02_morphology_successor_v3_private_r3_registration_from_freeze(
    ...
    if not isinstance(registration, dict) or not isinstance(budget, dict):
        raise W02MorphologySuccessorV3PrivateR3FamilyError(
            "R3 private family registration fields are invalid")
    ...
    if rebuilt.to_dict() != registration:
        raise W02MorphologySuccessorV3PrivateR3FamilyError(
            "R3 private family registration canonical value drifted")
    return rebuilt


def publish_w02_morphology_successor_v3_private_r3_family_root(
        repository_root: str | Path, family_root: str | Path) -> str:
    repository = Path(repository_root).resolve()
    freeze = read_w02_morphology_successor_v3_private_r3_family_freeze(
        repository)
    registration = (
    ...
    if root.exists():
        raise W02MorphologySuccessorV3PrivateR3FamilyError(
            "R3 private formal family root already exists")
    root.mkdir(parents=False, exist_ok=False)
    write_immutable_json(
        registration.to_dict(),
        root / W02_MORPH_V3_PRIVATE_R3_REGISTRATION_DOCUMENT)
    write_immutable_json(freeze, root / W02_MORPH_V3_PRIVATE_R3_FAMILY_DOCUMENT)
    (root / W02_MORPH_V3_PRIVATE_R3_EXPOSURE_LEDGER).mkdir(exist_ok=False)
    guard = w02_morphology_successor_v3_private_r3_guard_value(
        family_commitment=registration.family_commitment,
    ...
    target = root / Path(*PurePosixPath(
        W02_MORPH_V3_PRIVATE_R3_GUARD_AVAILABLE).parts)
    write_immutable_json(guard, target)
    digest = hashlib.sha256(target.read_bytes()).hexdigest()
    if digest != freeze["first_run_guard_sha256"]:
        raise W02MorphologySuccessorV3PrivateR3FamilyError(
            "R3 private available guard SHA drifted")
    return digest


def w02_morphology_successor_v3_private_r3_run_identity(
        freeze: dict[str, object], family_freeze_sha256: str) -> str:
    registration = freeze.get("registration")
    if not isinstance(registration, dict):
        raise W02MorphologySuccessorV3PrivateR3FamilyError(
            "R3 private run registration is missing")
    return hashlib.sha256(canonical_json_bytes({
        ...
        "family_freeze_sha256": _sha256(
            family_freeze_sha256, where="R3 private family freeze"),
...
def _intent_value(
    ...
    return {
        "artifact_kind": (
            "PH2_D03_V2_W02_MORPHOLOGY_SUCCESSOR_V3_PRIVATE_R3_RUN_INTENT"),
        "formal_private_evaluation_runs": 1,
        "guard_sha256": _sha256(guard_sha256, where="R3 private guard"),
        "private_payload_reads_before_guard": 0,
        "run_id": 1,
        "run_identity_sha256": _sha256(
            run_identity_sha256, where="R3 private run identity"),
        "stage_key": "W-02",
        "status": "RUN_INTENT_COMMITTED_BEFORE_PRIVATE_READ",
    }


def consume_w02_morphology_successor_v3_private_r3_guard(
    ...
    consumed = root / Path(*PurePosixPath(
        W02_MORPH_V3_PRIVATE_R3_GUARD_CONSUMED).parts)
    intent = root / Path(*PurePosixPath(
        W02_MORPH_V3_PRIVATE_R3_RUN_INTENT).parts)
    if consumed.exists() or intent.exists() or not available.is_file():
        raise W02MorphologySuccessorV3PrivateR3FamilyError(
            "R3 private guard is unavailable or already consumed")
    if _sha256_file(available)[1] != _sha256(
            expected_guard_sha256, where="expected R3 private guard"):
        raise W02MorphologySuccessorV3PrivateR3FamilyError(
            "R3 private available guard bytes drifted")
...
def verify_w02_morphology_successor_v3_private_r3_consumed_guard(
        family_root: str | Path, *, expected_guard_sha256: str,
        run_identity_sha256: str) -> None:
    root = _family_root(family_root, require_exists=True)
    available = root / Path(*PurePosixPath(
    ...
    if available.exists() or not consumed.is_file() or not intent.is_file():
        raise W02MorphologySuccessorV3PrivateR3FamilyError(
            "R3 private consumed guard state is incomplete")
    if (_sha256_file(consumed)[1] != expected_guard_sha256
            or read_canonical_object(intent) != _intent_value(
                guard_sha256=expected_guard_sha256,
                run_identity_sha256=run_identity_sha256)):
        raise W02MorphologySuccessorV3PrivateR3FamilyError(
            "R3 private consumed guard or run intent drifted")


__all__ = [
    "W02_MORPH_V3_PRIVATE_R3_EXPOSURE_LEDGER",
    "W02_MORPH_V3_PRIVATE_R3_FAMILY_CODE_PATHS",
    "W02_MORPH_V3_PRIVATE_R3_FAMILY_DOCUMENT",
    "W02_MORPH_V3_PRIVATE_R3_FAMILY_FREEZE_PATH",
    "W02_MORPH_V3_PRIVATE_R3_FAMILY_VERSION",
    "W02_MORPH_V3_PRIVATE_R3_FORMAL_FAMILY_NAME",
    "W02_MORPH_V3_PRIVATE_R3_GUARD_AVAILABLE",
    "W02_MORPH_V3_PRIVATE_R3_GUARD_CONSUMED",
    "W02_MORPH_V3_PRIVATE_R3_REGISTRATION_DOCUMENT",
    "W02_MORPH_V3_PRIVATE_R3_RUN_INTENT",
    ...
    "publish_w02_morphology_successor_v3_private_r3_family_freeze",
    "publish_w02_morphology_successor_v3_private_r3_family_root",
    "read_w02_morphology_successor_v3_private_r3_family_freeze",
    "verify_w02_morphology_successor_v3_private_r3_consumed_guard",
    "w02_morphology_successor_v3_private_r3_guard_value",
    "w02_morphology_successor_v3_private_r3_registration_from_freeze",
    "w02_morphology_successor_v3_private_r3_run_identity",
]
