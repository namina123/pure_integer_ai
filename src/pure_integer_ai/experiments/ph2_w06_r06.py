...
from pure_integer_ai.experiments.ph2_w06_r06_generation import (
...
class W06R06RuntimeReport:
    """R06 truth、event-time state、三向消费和资源摘要。"""
...
class W06R06Runtime:
    """组合共享 truth view、独立 U/R 历史与 Generation runtime。"""
    ...
    def __init__(
            ...
            endpoint_resolver,
            *,
            protocol: W06R06ConsumerProtocol = W06R06ConsumerProtocol(),
            ) -> None:
        self.view = W06R06View(
            learning, adapter, protocol, endpoint_resolver)
        self.learning = learning
        self.adapter = adapter
        self.protocol = protocol
        self.candidates = self.view.candidates
        self.understanding = W06R06QueryRuntime(
            self.view, "UNDERSTANDING")
        self.reasoning = W06R06QueryRuntime(self.view, "REASONING")
        self.generation = W06R06GenerationRuntime(
    ...
    def query_for_candidate(self, candidate, *, request_key, budget=None):
        """用 runtime 绑定的 train-only endpoint projection 构造查询。"""
    ...
    def report(self) -> W06R06RuntimeReport:
        snapshots = tuple(
        ...
        active_value = [
            {
                ...
                "direction": candidate_event_time_qualifier(item).direction,
                "evidence": [list(key)
