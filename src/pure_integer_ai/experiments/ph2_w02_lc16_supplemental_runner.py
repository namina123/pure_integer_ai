"""W-02 LC-16 supplemental 安全结果 pack 的只读执行桥。"""
...
RESULT_PACK_KIND = "PH2_W02_LC16_SUPPLEMENTAL_SAFE_RESULT_PACK"
RESULT_PACK_VERSION = "PH2-W02-LC16-SAFE-RESULT-PACK-20260801-A"
PRODUCER_KEY = "PH2-W02-LC16-INDEPENDENT-PRIVATE-EVALUATOR"
...
class W02Lc16SupplementalRunnerError(RuntimeError):
...
def _exact(value: Any, expected: set[str], *, where: str) -> dict[str, Any]:
    ...
    if not isinstance(value, dict) or set(value) != expected:
        raise W02Lc16SupplementalRunnerError(f"{where} 字段不精确")
    return value
...
def _strict_int(value: Any, *, where: str, positive: bool = False) -> int:
    ...
    if type(value) is not int or value < int(positive):
        raise W02Lc16SupplementalRunnerError(f"{where} 整数非法")
    return value
...
def _sha256(value: Any, *, where: str) -> str:
    ...
    if (not isinstance(value, str) or len(value) != 64
        ...
        raise W02Lc16SupplementalRunnerError(f"{where} 必须是小写 SHA-256")
    return value
...
def _reject_private(value: Any) -> None:
    ...
    if isinstance(value, dict):
        if _PRIVATE_KEYS.intersection(value):
            raise W02Lc16SupplementalRunnerError("安全 pack 含 private 字段")
...
class W02Lc16SupplementalSafeResultPack:
    ...
    def __post_init__(self) -> None:
        ...
        if (not isinstance(self.direction_results, tuple)
            ...
            raise W02Lc16SupplementalRunnerError(
        ...
        if before != after:
            raise W02Lc16SupplementalRunnerError("安全 pack 改变 host digest")
        for name in (
        ...
        if self.private_path_reads < self.private_path_count:
            raise W02Lc16SupplementalRunnerError("private path read 计数不闭合")
        if (self.private_payload_bytes > MAX_PAYLOAD_BYTES
            ...
            raise W02Lc16SupplementalRunnerError("安全 pack 资源预算超限")
        if self.evaluator_label_writes != 0 or self.host_write_count != 0:
            raise W02Lc16SupplementalRunnerError("安全 pack 必须零写")
        if (self.independent_evaluator_module_separate != 1
                or self.consumer_result_builder_reused != 0):
            raise W02Lc16SupplementalRunnerError("安全 pack evaluator 不独立")
        if self.runtime_observed != 1:
            raise W02Lc16SupplementalRunnerError("安全 pack 必须来自已观察 runtime")
    ...
    def to_public_dict(self) -> dict[str, Any]:
        ...
        value = {
            ...
            "w02_parent_receipt_sha256": W02_PARENT_RECEIPT_SHA256,
            "writes": {
    ...
    def from_public_dict(
        ...
        if isinstance(value, dict) and (
                value.get("case_count") == 10
                or "V3" in str(value.get("artifact_version", "")).upper()
                or "V3" in str(value.get("producer_revision", "")).upper()):
            raise W02Lc16SupplementalRunnerError(
                "旧 W-02 v3/10-case pack 不属于 LC-16 supplemental")
        _reject_private(value)
        raw = _exact(value, {
            ...
            "resource_use", "runtime_observed", "w02_parent_receipt_sha256",
            "writes",
        ...
        if (raw["format_version"] != RESULT_PACK_FORMAT_VERSION
            ...
            raise W02Lc16SupplementalRunnerError("安全 pack producer/revision 非法")
        if (raw["parent_overlay_sha256"] != OVERLAY_SHA256
                or raw["w02_parent_receipt_sha256"]
                != W02_PARENT_RECEIPT_SHA256):
            raise W02Lc16SupplementalRunnerError("安全 pack parent SHA 漂移")
        if (raw["case_count"] != CASE_COUNT
                or raw["direction_evaluations"]
                != DIRECTION_EVALUATION_COUNT):
            raise W02Lc16SupplementalRunnerError("安全 pack 63/189 覆盖漂移")
...
@dataclass(frozen=True)
class W02Lc16SupplementalRunOutcome:
    """只读 runner 的资格状态或缺 pack 阻断状态。"""

    status: str
    blocker_code: str | None
    safe_pack_sha256: str | None
    report: W02Lc16SupplementalReport | None


def read_w02_lc16_supplemental_safe_result_pack(
        path: str | Path,
        ) -> tuple[W02Lc16SupplementalSafeResultPack, str]:
    """严格回读 canonical 安全 pack 并返回其逐字节摘要。"""
    target = Path(path)
    try:
        payload = target.read_bytes()
        if not payload.endswith(b"\n") or payload.endswith(b"\n\n"):
            raise W02Lc16SupplementalRunnerError("安全 pack newline 非法")
        value = parse_canonical_json_bytes(payload[:-1], require_object=True)
        pack = W02Lc16SupplementalSafeResultPack.from_public_dict(value)
    except W02Lc16SupplementalRunnerError:
        raise
    except Exception as error:
        raise W02Lc16SupplementalRunnerError("安全 pack 损坏") from error
    if pack.canonical_bytes() != payload:
        raise W02Lc16SupplementalRunnerError("安全 pack 非 canonical bytes")
    return pack, hashlib.sha256(payload).hexdigest()


def run_w02_lc16_supplemental_safe_pack(
        safe_pack_path: str | Path,
        *,
        repository_root: str | Path,
        ) -> W02Lc16SupplementalRunOutcome:
    """零写读取公开依赖和安全 pack，并交给独立聚合器。"""
    pack_path = Path(safe_pack_path)
    if not pack_path.is_file():
        return W02Lc16SupplementalRunOutcome(
            "BLOCKED", "SAFE_RESULT_PACK_MISSING", None, None)
    root = Path(repository_root).resolve()
    try:
        manifest = read_w02_lc16_supplemental_manifest(
            root / Path(*MANIFEST_PATH.split("/")))
        if (manifest.parent_overlay_sha256 != OVERLAY_SHA256
                or manifest.w02_parent_receipt_sha256
                != W02_PARENT_RECEIPT_SHA256):
            raise W02Lc16SupplementalRunnerError(
        ...
        if overlay.sha256() != OVERLAY_SHA256:
            raise W02Lc16SupplementalRunnerError("overlay SHA 漂移")
        pack, pack_sha256 = read_w02_lc16_supplemental_safe_result_pack(
            pack_path)
        report = aggregate_w02_lc16_supplemental(
            overlay,
...
__all__ = [
    ...
    "W02Lc16SupplementalRunOutcome", "W02Lc16SupplementalRunnerError",
    "W02Lc16SupplementalSafeResultPack",
