"""W-03 LC-16 九载体补充资格的只读合同与公开 receipt 结构。"""
...
FORMAT_VERSION = 1
ARTIFACT_KIND = "PH2_W03_LC16_SUPPLEMENTAL_QUALIFICATION"
ARTIFACT_VERSION = "PH2-W03-LC16-SUPPLEMENTAL-20260801-A"
ARTIFACT_STATUS = "W03_LC16_SUPPLEMENTAL_FROZEN_NOT_RUN"
MANIFEST_PATH = "data/ph2/manifests/w03_lc16_supplemental_v1.json"
...
CONCEPT_SPLIT = "W-03-CONCEPT_SPLIT_BY_CARRIER"
POLYSEMY_COMPETITION = "W-03-POLYSEMY_COMPETITION_BY_CARRIER"
SOURCE_CONFLICT = "W-03-SOURCE_CONFLICT_BY_CARRIER"
SUPERSEDE = "W-03-SUPERSEDE_BY_CARRIER"
GENERATION_HARD_CONJUNCT = (
    "W-03-LC16-GENERATION-SENSE-SELECTION-HARD-CONJUNCT")
BEARING_DIMENSIONS = (
    CONCEPT_SPLIT,
    POLYSEMY_COMPETITION,
    SOURCE_CONFLICT,
    SUPERSEDE,
)
EVALUATION_ORDER = (*BEARING_DIMENSIONS, GENERATION_HARD_CONJUNCT)
ABLATION_ORDER = tuple(f"{item}-ABLATION" for item in EVALUATION_ORDER)
...
MAX_WORKERS = 4
MAX_PAYLOAD_BYTES = 201326592
MAX_PAYLOAD_READS = 196608
MAX_LOGIC_OPERATIONS = 3000000
...
EXECUTION_STATE = {
    ...
    "runtime_observed": 0,
    "teacher_calls": 0,
    "w02_lc16_supplemental_qualified": 1,
...
class W03Lc16SupplementalError(RuntimeError):
    """补充资格输入、合取结果或发布身份不闭合。"""
...
def _exact(value: Any, expected: set[str], *, where: str) -> dict[str, Any]:
    ...
    if not isinstance(value, dict) or set(value) != expected:
        raise W03Lc16SupplementalError(f"{where} 字段不精确")
    return value
...
def _text(value: Any, *, where: str) -> str:
    ...
    if not isinstance(value, str) or not value or value.strip() != value:
        raise W03Lc16SupplementalError(f"{where} 文本非法")
    return value
...
def _sha256(value: Any, *, where: str) -> str:
    ...
    if len(checked) != 64 or any(item not in "0123456789abcdef" for item in checked):
        raise W03Lc16SupplementalError(f"{where} 必须是小写 SHA-256")
    return checked
...
def _nonnegative(value: Any, *, where: str) -> int:
    ...
    if type(value) is not int or value < 0:
        raise W03Lc16SupplementalError(f"{where} 必须是非负严格整数")
    return value
...
def _positive(value: Any, *, where: str) -> int:
    ...
    if checked == 0:
        raise W03Lc16SupplementalError(f"{where} 必须为正")
    return checked
...
def _status(value: Any, *, where: str, blocked: bool = False) -> str:
    ...
    if value not in allowed:
        raise W03Lc16SupplementalError(f"{where} 状态非法")
    return value
...
class SupplementalDimensionResult:
    ...
    def __post_init__(self) -> None:
        if self.dimension_key not in EVALUATION_ORDER:
            raise W03Lc16SupplementalError("补充维度未预注册")
        ...
        if self.case_count != self.pass_count + self.fail_count + self.ne_count:
            raise W03Lc16SupplementalError("补充维度计数不闭合")
        if self.case_count <= 0 or self.status != _dimension_status(self):
            raise W03Lc16SupplementalError("补充维度状态与计数不一致")
...
class SupplementalAblationResult:
    ...
    def __post_init__(self) -> None:
        if self.ablation_key not in ABLATION_ORDER:
            raise W03Lc16SupplementalError("补充消融未预注册")
        index = ABLATION_ORDER.index(self.ablation_key)
        if self.targeted_dimension != EVALUATION_ORDER[index]:
            raise W03Lc16SupplementalError("补充消融目标错位")
        if tuple(item[0] for item in self.dimension_statuses) != BEARING_DIMENSIONS:
            raise W03Lc16SupplementalError("补充消融维度顺序漂移")
        if any(
                not isinstance(item, tuple) or len(item) != 2
                or item[1] not in STATUSES
                for item in self.dimension_statuses):
            raise W03Lc16SupplementalError("补充消融状态非法")
...
class SupplementalDirectionResult:
    ...
    def __post_init__(self) -> None:
        if self.carrier_key not in IN_SCOPE_CARRIER_KEYS:
            raise W03Lc16SupplementalError("补充结果 carrier 未登记")
...
class SupplementalCarrierSummary:
    ...
    def __post_init__(self) -> None:
        if self.carrier_key not in IN_SCOPE_CARRIER_KEYS:
            raise W03Lc16SupplementalError("carrier summary 未登记")
        if self.direction_evaluations != len(SAMPLE_KINDS) * len(DIRECTIONS):
            raise W03Lc16SupplementalError("carrier summary direction 数量错误")
        if tuple(item.dimension_key for item in self.dimensions) != BEARING_DIMENSIONS:
            raise W03Lc16SupplementalError("carrier summary bearing 顺序漂移")
        if self.generation.dimension_key != GENERATION_HARD_CONJUNCT:
            raise W03Lc16SupplementalError("carrier summary generation 缺失")
...
@dataclass(frozen=True, order=True)
class W03Lc16SupplementalReport:
    """独立 supplemental family 的 append-only 公开 receipt 投影。"""

    status: str
    parent_overlay_sha256: str
    w03_parent_receipt_sha256: str
    w02_supplemental_receipt_sha256: str
    ...
    def __post_init__(self) -> None:
        ...
        if self.parent_overlay_sha256 != OVERLAY_SHA256:
            raise W03Lc16SupplementalError("parent overlay SHA 漂移")
        if self.w03_parent_receipt_sha256 != W03_PARENT_RECEIPT_SHA256:
            raise W03Lc16SupplementalError("W-03 parent receipt SHA 漂移")
        if (self.w02_supplemental_receipt_sha256
                != W02_SUPPLEMENTAL_RECEIPT_SHA256):
            raise W03Lc16SupplementalError("W-02 supplemental receipt SHA 漂移")
        ...
        if self.case_count != CASE_COUNT or self.direction_evaluations != DIRECTION_EVALUATION_COUNT:
            raise W03Lc16SupplementalError("supplemental case/direction 数量错误")
        if tuple(item.dimension_key for item in self.dimensions) != BEARING_DIMENSIONS:
            raise W03Lc16SupplementalError("report bearing 顺序漂移")
        if self.generation.dimension_key != GENERATION_HARD_CONJUNCT:
            raise W03Lc16SupplementalError("report generation 缺失")
        if tuple(item.ablation_key for item in self.ablations) != ABLATION_ORDER:
            raise W03Lc16SupplementalError("report ablation 顺序漂移")
        if tuple(item.carrier_key for item in self.carrier_summaries) != IN_SCOPE_CARRIER_KEYS:
            raise W03Lc16SupplementalError("report carrier summary 未闭合")
        self._verify_host()
        for name in (
        ...
        if (self.runtime_observed == 1
            ...
            raise W03Lc16SupplementalError(
        ...
        if self.private_payload_bytes > MAX_PAYLOAD_BYTES or self.private_payload_reads > MAX_PAYLOAD_READS:
            raise W03Lc16SupplementalError("supplemental payload budget 超限")
        if self.evaluator_label_writes != 0 or self.host_write_count != 0:
            raise W03Lc16SupplementalError("supplemental evaluator 必须零写")
        if self.independent_evaluator_module_separate != 1 or self.consumer_result_builder_reused != 0:
            raise W03Lc16SupplementalError("supplemental evaluator independence 失败")
        if self.runtime_observed not in (0, 1):
            raise W03Lc16SupplementalError("runtime_observed 非法")
        _status(
            self.independent_reveal_status,
            where="report independent reveal",
            blocked=True,
        )
        statuses = [item.status for item in self.dimensions]
        statuses.extend((self.generation.status, self.independent_reveal_status))
        if self.runtime_observed == 0:
        ...
        if self.status != expected_status:
            raise W03Lc16SupplementalError("report status 与硬合取不一致")
    ...
    def _verify_host(self) -> None:
        ...
        if (set(self.host_digests_before) != set(HOST_DIGEST_KEYS)
            ...
            raise W03Lc16SupplementalError("host digest 集合不完整")
        ...
        if self.host_digests_before != self.host_digests_after:
            raise W03Lc16SupplementalError("supplemental 改变 host digest")
    ...
    def to_public_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "status": self.status,
            "w02_supplemental_receipt_sha256": (
                self.w02_supplemental_receipt_sha256),
            "w03_parent_receipt_sha256": self.w03_parent_receipt_sha256,
        }
    ...
    @classmethod
    def from_public_dict(cls, value: Any) -> "W03Lc16SupplementalReport":
        ...
        }, where="W03Lc16SupplementalReport")
        if raw["artifact_kind"] != ARTIFACT_KIND or raw["artifact_version"] != ARTIFACT_VERSION:
            raise W03Lc16SupplementalError("supplemental receipt artifact identity 漂移")
        ...
        return cls(
            ...
            str(raw["w03_parent_receipt_sha256"]),
            str(raw["w02_supplemental_receipt_sha256"]),
            ...
            raw["evaluator_label_writes"], raw["host_write_count"],
            raw["independent_evaluator_module_separate"],
            raw["consumer_result_builder_reused"],
            str(raw["independent_reveal_status"]), raw["runtime_observed"],
            str(raw["evidence_sha256"]),
        )
...
class SupplementalManifestFile:
    ...
    def __post_init__(self) -> None:
        ...
        if path.is_absolute() or ".." in path.parts or "\\" in self.relative_path or path.as_posix() != self.relative_path:
            raise W03Lc16SupplementalError("manifest path 逃逸")
...
@dataclass(frozen=True, order=True)
class W03Lc16SupplementalManifest:
    """冻结 W-03 supplemental 课程、预算、边界和零执行状态。"""
    ...
    def __post_init__(self) -> None:
        if (self.format_version != FORMAT_VERSION
            ...
            raise W03Lc16SupplementalError("supplemental manifest artifact identity 漂移")
        if self.parent_overlay_path != OVERLAY_PATH or self.parent_overlay_sha256 != OVERLAY_SHA256:
            raise W03Lc16SupplementalError("supplemental overlay parent 漂移")
        _sha256(self.w03_parent_receipt_sha256, where="manifest W-03 receipt SHA")
        if self.w03_parent_receipt_sha256 != W03_PARENT_RECEIPT_SHA256:
            raise W03Lc16SupplementalError("manifest W-03 receipt 承诺漂移")
        if (self.w02_supplemental_receipt_sha256
                != W02_SUPPLEMENTAL_RECEIPT_SHA256):
            raise W03Lc16SupplementalError(
                "manifest W-02 supplemental receipt 承诺漂移")
        expected_scope = {
            ...
            "w03_bearing_dimension_count": len(BEARING_DIMENSIONS),
        }
        if self.scope.to_value() != expected_scope:
            raise W03Lc16SupplementalError("supplemental scope 漂移")
        expected_state = dict(EXECUTION_STATE)
        if self.execution_state.to_value() != expected_state:
            raise W03Lc16SupplementalError("supplemental execution state 漂移")
        if (self.dependencies != tuple(sorted(self.dependencies, key=lambda item: item.role))
                or tuple(item.role for item in self.dependencies) != (
                    "D03_LC16_OVERLAY", "W02_LC16_SUPPLEMENTAL_PASS_RECEIPT",
                    "W03_PARENT_RECEIPT_COMMITMENT")
            ...
            raise W03Lc16SupplementalError("supplemental 文件证据未闭合")
    ...
    @classmethod
    def from_dict(cls, value: Any) -> "W03Lc16SupplementalManifest":
        ...
        raw = _exact(value, {
            ...
            "w02_supplemental_receipt_sha256", "w03_parent_receipt_sha256",
        }, where="W03Lc16SupplementalManifest")
        if raw["artifact_kind"] != ARTIFACT_KIND:
            raise W03Lc16SupplementalError("supplemental manifest artifact_kind 非法")
        return cls(
            raw["format_version"], str(raw["artifact_version"]),
            str(raw["artifact_status"]), str(raw["parent_overlay_path"]),
            str(raw["parent_overlay_sha256"]), str(raw["w03_parent_receipt_sha256"]),
            str(raw["w02_supplemental_receipt_sha256"]),
...
def read_w03_lc16_supplemental_manifest(path: str | Path) -> W03Lc16SupplementalManifest:
    """严格回读 canonical supplemental manifest。"""
    try:
        payload = Path(path).read_bytes()
        if not payload.endswith(b"\n") or payload.endswith(b"\n\n"):
            raise W03Lc16SupplementalError("supplemental manifest newline 非法")
        manifest = W03Lc16SupplementalManifest.from_dict(
            parse_canonical_json_bytes(payload[:-1], require_object=True))
    except W03Lc16SupplementalError:
        raise
    except Exception as error:
        raise W03Lc16SupplementalError("supplemental manifest 损坏") from error
    if manifest.canonical_bytes() != payload:
        raise W03Lc16SupplementalError("supplemental manifest 非 canonical bytes")
    return manifest


def write_w03_lc16_supplemental_manifest(
        manifest: W03Lc16SupplementalManifest, path: str | Path,
        ) -> Path:
    """以逐字节幂等、异内容拒绝方式写入预注册 manifest。"""
    if not isinstance(manifest, W03Lc16SupplementalManifest):
        raise W03Lc16SupplementalError("supplemental manifest 类型非法")
    target = Path(path)
    payload = manifest.canonical_bytes()
    if target.exists():
        if target.read_bytes() != payload:
            raise W03Lc16SupplementalError("supplemental manifest 已存在且内容不同")
        return target
...
__all__ = [
    ...
    "W02_SUPPLEMENTAL_RECEIPT_SHA256",
    "W03Lc16SupplementalError", "W03Lc16SupplementalManifest",
    "W03Lc16SupplementalReport", "SupplementalAblationResult",
    "SupplementalCarrierSummary", "SupplementalDimensionResult",
    "SupplementalDirectionResult", "SupplementalManifestFile",
    "W03_PARENT_RECEIPT_SHA256", "read_w03_lc16_supplemental_manifest",
