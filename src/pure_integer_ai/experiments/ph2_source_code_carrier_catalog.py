"""LC-16 SOURCE_CODE payload、tree adapter 与冻结 parent pack 目录。"""
...
_EVIDENCE_PATHS = (
    ("src/pure_integer_ai/experiments/ph2_source_code_carrier_adapter.py", "ADAPTER"),
    ("src/pure_integer_ai/experiments/ph2_source_code_carrier_catalog.py", "CATALOG"),
    ("src/pure_integer_ai/experiments/ph2_source_code_carrier_contract.py", "CONTRACT"),
    ("pyproject.toml", "DEPENDENCY"),
    ("tests/test_lc16_source_code_carrier_adapter.py", "TEST"),
)


class SourceCodeCarrierCatalogError(RuntimeError):
...
def _path(root: Path, relative_path: str) -> Path:
    ...
    try:
        ...
        raise SourceCodeCarrierCatalogError("catalog 路径逃逸") from error
    return target


def _identity(path: Path) -> tuple[int, str]:
    if not path.is_file():
        raise SourceCodeCarrierCatalogError(f"catalog 文件缺失: {path.name}")
    payload = path.read_bytes()
    return len(payload), hashlib.sha256(payload).hexdigest()


def _evidence_files(root: Path) -> tuple[SourceCodeCarrierEvidenceFile, ...]:
    result = []
    for relative_path, role in _EVIDENCE_PATHS:
        byte_count, sha256 = _identity(_path(root, relative_path))
        result.append(SourceCodeCarrierEvidenceFile(
...
def _check_budget(
    ...
    if raw_unit_count > budget.max_raw_units:
        raise SourceCodeCarrierCatalogError("SOURCE_CODE raw 超出 parent budget")
    if previous_raw_unit_count > budget.max_raw_units:
        raise SourceCodeCarrierCatalogError("SOURCE_CODE previous raw 超出 parent budget")
    if structure_node_count > budget.max_structure_nodes:
        raise SourceCodeCarrierCatalogError("SOURCE_CODE structure 超出 parent budget")
    if revision_count > budget.max_edges:
        raise SourceCodeCarrierCatalogError("SOURCE_CODE revision 超出 parent budget")
    if envelope_count > budget.max_depth:
        raise SourceCodeCarrierCatalogError("SOURCE_CODE envelope 超出 parent budget")


def build_source_code_carrier_manifest(
    ...
    if parent_byte_count <= 0 or parent_sha256 != PARENT_PACK_SHA256:
        raise SourceCodeCarrierCatalogError("parent pack 文件身份漂移")
    try:
        parent = read_typed_carrier_pack_manifest(parent_path)
        verify_typed_carrier_pack_files(parent, repository_root=root)
    except Exception as error:
        raise SourceCodeCarrierCatalogError("parent pack 无法严格回验") from error
    if parent.sha256() != parent_sha256:
        raise SourceCodeCarrierCatalogError("parent pack canonical SHA 漂移")
    ...
    if len(budgets) != 1 or len(cases) != budgets[0].max_cases:
        raise SourceCodeCarrierCatalogError("parent SOURCE_CODE case/budget 漂移")
    budget = budgets[0]
    ...
    if any(item.anchor_kinds != expected_anchors for item in cases):
        raise SourceCodeCarrierCatalogError("parent SOURCE_CODE anchor 漂移")
    sample_path = _path(root, SOURCE_CODE_CARRIER_SAMPLE_PATH)
    _, sample_sha256 = _identity(sample_path)
    try:
        records = read_source_code_carrier_records(sample_path)
    except Exception as error:
        raise SourceCodeCarrierCatalogError("SOURCE_CODE sample 无法严格回读") from error
    if tuple(item.case_key for item in records) != tuple(item.case_key for item in cases):
        raise SourceCodeCarrierCatalogError("sample case_key 与 parent 不一致")
    if tuple(item.sample_kind for item in records) != tuple(item.sample_kind for item in cases):
        raise SourceCodeCarrierCatalogError("sample kind 与 parent 不一致")
    if any(item.payload_state != "UNMATERIALIZED" or item.runtime_state != "NOT_RUN"
           for item in cases):
        raise SourceCodeCarrierCatalogError("parent 已越过 payload/runtime 边界")
    _parser_version()
    materializations = []
    for record in records:
        try:
            adapted = adapt_source_code_carrier_record(record)
            payload = serialize_source_code_carrier_materialization(adapted)
            restored = deserialize_source_code_carrier_materialization(payload, record)
        except Exception as error:
            raise SourceCodeCarrierCatalogError(
                f"adapter 无法物化 {record.sample_kind}") from error
        if restored != adapted:
            raise SourceCodeCarrierCatalogError("adapter reload identity 漂移")
        _check_budget(
            budget,
        ...
        materializations.append(SourceCodeMaterializationIdentity(
            record.case_key,
            len(payload),
    ...
    return SourceCodeCarrierPayloadManifest(
        FORMAT_VERSION,
        ARTIFACT_VERSION,
        ARTIFACT_STATUS,
        PARENT_PACK_PATH,
