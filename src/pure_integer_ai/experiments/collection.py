...
**E10 local_dir 首选**：LocalDirSource 读 PURE_INTEGER_AI_LOCAL_DIR 本地裸文本文件（用户第三方下载器
...
def _resolve_local_dir(local_dir: str | None) -> str | None:
    """解析 local_dir（参数 > PURE_INTEGER_AI_LOCAL_DIR env > None·E10 首选）。"""
    d = local_dir or read_project_env("PURE_INTEGER_AI_LOCAL_DIR")
    if d is None:
        return None
...
class LocalDirSource:
    """本地目录源（E10 首选·读 PURE_INTEGER_AI_LOCAL_DIR 裸文本文件·纯本地读离线可复现）。
...
def resolve_is_a_facts(lang: int, local_dir: str | None = None) -> list[tuple[str, str]]:
    """解析某 lang 的 IS_A facts（E10 本地文件·PURE_INTEGER_AI_LOCAL_DIR·文件不存在→空）。

    优先序：local_dir 参数 > PURE_INTEGER_AI_LOCAL_DIR env > None。目录/文件不存在 / lang 无映射 → 返 []
...
def resolve_mereology_facts(lang: int, local_dir: str | None = None) -> list[tuple[str, str]]:
    """解析某 lang 的 MEREOLOGY facts（E10 本地文件·PURE_INTEGER_AI_LOCAL_DIR/mereology_facts_{lang}.txt·文件不存在→空·镜像 resolve_is_a_facts）。
...
def resolve_antonym_facts(lang: int, local_dir: str | None = None) -> list[tuple[str, str]]:
    """解析某 lang 的 ANTONYM facts（E10 本地文件·PURE_INTEGER_AI_LOCAL_DIR/antonym_facts_{lang}.txt·文件不存在→空·镜像 resolve_mereology_facts）。
...
def resolve_similar_facts(lang: int, local_dir: str | None = None) -> list[tuple[str, str]]:
    """解析某 lang 的 SIMILAR facts（E10 本地文件·PURE_INTEGER_AI_LOCAL_DIR/similar_facts_{lang}.txt·文件不存在→空·镜像 resolve_antonym_facts）。
...
def resolve_abstract_facts(lang: int, local_dir: str | None = None) -> list[tuple[str, str]]:
    """解析某 lang 的 ABSTRACT→IS_A 泛化 facts（E10 本地文件·PURE_INTEGER_AI_LOCAL_DIR/abstract_facts_{lang}.txt·文件不存在→空）。
    ...
    **异 source provenance**（关键·避 MED-1 式污染）：caller 传 SOURCE_CHINESE_KB（ChineseSemanticKB 抽象关系库·#1133）·
    ...
    优先序：local_dir > PURE_INTEGER_AI_LOCAL_DIR env > None。缺文件/lang 无映射 → []（E5·bit-identical·CI 无文件→空→零边）。
...
def resolve_sense_facts(lang: int, local_dir: str | None = None) -> list[tuple[str, list[str]]]:
    """解析某 lang 的 sense facts（E10 本地文件·PURE_INTEGER_AI_LOCAL_DIR·文件不存在→空·镜像 resolve_is_a_facts）。
...
def resolve_causes_facts(lang: int, local_dir: str | None = None) -> list[tuple[str, str]]:
    """解析某 lang 的 CAUSES facts（E10 本地文件·PURE_INTEGER_AI_LOCAL_DIR/causes_facts_{lang}.txt·镜像 resolve_is_a_facts）。
...
def resolve_struct_bind_pairs(local_dir: str | None = None
    ...
    """解析 STRUCT_BIND 绑定对（E10 本地文件·PURE_INTEGER_AI_LOCAL_DIR/struct_bind_pairs.txt·文件不存在→空·
    ...
    **非 lang-keyed**（与 resolve_is_a_facts/resolve_sense_facts 异）：STRUCT_BIND 跨模态绑定横跨模态不属某
    lang·单一 struct_bind_pairs.txt（无 lang 后缀·caller 不带 lang）。优先序：local_dir 参数 > PURE_INTEGER_AI_LOCAL_DIR
...
def resolve_alias_facts(local_dir: str | None = None
    ...
    """解析跨语言 alias facts（E10 本地文件·PURE_INTEGER_AI_LOCAL_DIR/alias_facts.txt·文件不存在→空·
    ...
    **非 lang-keyed**（与 resolve_is_a_facts/resolve_sense_facts 异·同 resolve_struct_bind_pairs）：aliases 横跨
    ...
    PURE_INTEGER_AI_LOCAL_DIR env > None。目录/文件不存在 → 返 []（E5 graceful·bit-identical 守·CI/生产 default
...
def resolve_number_facts(local_dir: str | None = None
    ...
    """解析数字词接地 facts（E10 本地文件·PURE_INTEGER_AI_LOCAL_DIR/number_facts.txt·文件不存在→空·镜像 resolve_alias_facts）。

    **非 lang-keyed**（同 alias_facts·数字词横跨语言不属某 lang）：单一 number_facts.txt（无 lang 后缀）。
    优先序：local_dir 参数 > PURE_INTEGER_AI_LOCAL_DIR env > None。目录/文件不存在 → 返 []（E5 graceful·bit-identical 守·
...
def resolve_degree_facts(lang: int, local_dir: str | None = None
    ...
    """解析 degree cues（E10 本地文件·PURE_INTEGER_AI_LOCAL_DIR/degree_cues_{suffix}.txt·文件不存在→{}·镜像 resolve_*_facts lang-keyed·#1134）。
...
def load_transform_rules_file(path: str) -> list[TransformSpec]:
    ...
    **bit-identical**：CI/生产 default 无 PURE_INTEGER_AI_LOCAL_DIR→resolve 返 []→不 inject→零 specs→bit-identical。
...
def resolve_transform_rules(local_dir: str | None = None
    ...
    """解析符号变换规则（E10 本地文件·PURE_INTEGER_AI_LOCAL_DIR/transform_rules.txt·文件不存在→空·
    镜像 resolve_alias_facts 范式·Phase 0.1）。

    **非 lang-keyed**（同 resolve_alias_facts/resolve_struct_bind_pairs）：符号数学规则横跨域不属某 lang·
    单一 transform_rules.txt（无 lang 后缀）。优先序：local_dir 参数 > PURE_INTEGER_AI_LOCAL_DIR env > None。
...
def resolve_inverse_relations(local_dir: str | None = None
    ...
    """解析逆关系（E10 本地文件·PURE_INTEGER_AI_LOCAL_DIR/inverse_relations.txt·文件不存在→空·
...
# **bit-identical**：CI 无 PURE_INTEGER_AI_LOCAL_DIR→resolve_*_facts 返 []→filter 空 list 返空（filter_pairs_to_vocab
