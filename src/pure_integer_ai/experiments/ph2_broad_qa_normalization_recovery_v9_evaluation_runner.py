"""执行 recovery-v9 GIMP fixed-denominator的唯一formal run。"""
...
from pure_integer_ai.experiments.ph2_broad_qa_normalization_recovery_v9_evaluation_family import (
...
NORMALIZATION_RECOVERY_V9_FORMAL_GUARD_KIND = (
    "PH2_BROAD_QA_NORMALIZATION_RECOVERY_V9_FORMAL_RUN_GUARD_V1")
NORMALIZATION_RECOVERY_V9_FORMAL_REPORT_KIND = (
    "PH2_BROAD_QA_NORMALIZATION_RECOVERY_V9_FORMAL_REPORT_V1")
NORMALIZATION_RECOVERY_V9_FORMAL_FAILURE_KIND = (
    "PH2_BROAD_QA_NORMALIZATION_RECOVERY_V9_FORMAL_FAILURE_V1")


def _sha256(payload: bytes) -> str:
    """返回guard、report或failure publication SHA-256。"""
    return hashlib.sha256(payload).hexdigest()


def _write_immutable(path: Path, value: dict[str, object]) -> None:
    """以exclusive create写入规范publication。"""
    with path.open("xb") as handle:
        handle.write(canonical_json_line(value))


def _family_arguments(arguments: dict[str, object]) -> dict[str, object]:
    """选择family strict reader的全部冻结参数。"""
...
def _overlap(left: Path, right: Path) -> bool:
    """判断publication与冻结artifact根是否存在包含关系。"""
...
def run_normalization_recovery_v9_formal_evaluation(
        **arguments: object,
        ) -> dict[str, object]:
    """先消费唯一guard，再物化全GIMP labels并发布六维aggregate。"""
    root = require_normalization_recovery_v9_k_root(arguments["run_root"])
    family_dir = normalization_recovery_v9_path_within(
        root, arguments["family_freeze_dir"], label="family_freeze_dir")
    publication = normalization_recovery_v9_path_within(
        root, arguments["publication_dir"], label="publication_dir")
    if not family_dir.is_dir():
        raise BroadQaExternalDataError("v9 formal family缺失")
    ...
    if (publication.exists()
        ...
        raise BroadQaExternalDataError(
            "v9 formal publication已消费或artifact混淆")
    ...
    try:
        guard = {
            "artifact_kind": NORMALIZATION_RECOVERY_V9_FORMAL_GUARD_KIND,
            ...
            "format_version": 1,
            "gimp_source_payload_read_count": 0,
            "overwrite_allowed": 0,
            "retry_allowed": 0,
            "run_id": 1,
            "status": "CONSUMED_BEFORE_GIMP_LABEL_MATERIALIZATION",
        }
        ...
        materialization, records = (
            materialize_normalization_recovery_v9_labels_after_guard(
                guard_consumed=1,
                gimp_source_pack_dir=arguments["gimp_source_pack_dir"],
                expected_gimp_source_manifest_sha256=arguments[
        ...
        evaluation = evaluate_normalization_recovery_v9_candidate(
        ...
        formal = {
            "artifact_kind": NORMALIZATION_RECOVERY_V9_FORMAL_REPORT_KIND,
