...
def _event_key(binding_key: tuple[int, ...]) -> tuple[int, ...]:
    """把完整绑定内容转成固定长度、域分离的幂等 event key。"""
    return _content_ref(
...
class GenerationChoiceAssessmentConsumer:
    ...
    def prepare(
        ...
        for item in assessment.inputs:
            ...
            event_key = _event_key(binding_key)
            trace = _content_ref(
                binding_key,
    ...
    def apply(
        ...
        _definitions, registrations, existing = self._definitions(
            tuple(pending))
        if registrations:
            self.learning.register_many(registrations)
        timestamp_values = self.learning.next_timestamps(3 * len(pending))
        ...
        for index, item in enumerate(pending):
            ...
            supported = (
                ...
                if item.stance == EVIDENCE_SUPPORT else ())
            refuted = (
                (choice.selected_object,)
                if item.stance == EVIDENCE_REFUTE else ())
            recognition_requests.append(CandidateRecognitionRequest(
                self.learning.hypothesis_for_candidate(
                    self.mapper.candidate_identity(choice)),
