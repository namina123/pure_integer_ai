"""W06-R02 train endpoint_id 到跨命题 canonical identity 的追加投影。"""
...
W06_R02_SOURCE_OVERLAY_PATH = (
    "data/ph2/manifests/w06_source_semantic_overlay_v1.json"
)
W06_R02_STAGE_PATH = (
    "data/ph2/manifests/d03_v1/stages/w06_stage_manifest_v1.json"
)
W06_R02_EXPECTED_PARENT_SHA256 = {
    W06_R02_SAMPLE_PATH:
        "f46bfb2cdefaa0ad9503fa656cb8b59f5a98794a01f1eb8f1a81b1189db1cb10",
    W06_R02_SOURCE_OVERLAY_PATH:
...
class W06R02EndpointProjectionError(RuntimeError):
    """R02 endpoint overlay、parent 或映射发生漂移。"""
...
def _sha256(path: Path) -> str:
    ...
    try:
        ...
        raise W06R02EndpointProjectionError(
...
def _canonical_endpoint(endpoint_id: str, object_kind: int) -> ObjectIdentity:
    ...
    if not isinstance(endpoint_id, str) or not endpoint_id:
        raise W06R02EndpointProjectionError("endpoint_id 必须是非空文本")
    if type(object_kind) is not int or object_kind <= 0:
        raise W06R02EndpointProjectionError("endpoint object kind 非法")
...
class W06R02EndpointProjectionEntry:
    """一条来源局部 endpoint 到 canonical endpoint 的有损禁止映射。"""
...
class W06R02EndpointProjection:
    """只按已发布完整 local identity 查询，不读取 surface。"""

    def __init__(
            self, entries: tuple[W06R02EndpointProjectionEntry, ...],
            ) -> None:
        if (not isinstance(entries, tuple) or not entries
                or any(not isinstance(item, W06R02EndpointProjectionEntry)
                       for item in entries)):
            raise W06R02EndpointProjectionError("endpoint projection entries 非法")
        normalized = tuple(sorted(
            entries, key=W06R02EndpointProjectionEntry.stable_key))
        if entries != normalized:
            raise W06R02EndpointProjectionError("endpoint projection 未规范排序")
        ...
        for item in entries:
            ...
            if prior is not None and prior != item.canonical_endpoint:
                raise W06R02EndpointProjectionError("同一 local endpoint 映射冲突")
            mapping[item.local_endpoint] = item.canonical_endpoint
        if len(mapping) != len(entries):
            raise W06R02EndpointProjectionError("endpoint projection local key 重复")
    ...
    def clone_for_evaluation(self) -> "W06R02EndpointProjection":
        return W06R02EndpointProjection(self.entries)

    def state_key(self) -> tuple[int, ...]:
        values = [50602, 1, len(self.entries)]
...
def build_w06_r02_endpoint_projection(repo_root: str | Path) -> dict[str, Any]:
    """从公开 authored train seed 重建无 surface canonical endpoint overlay。"""
    root = Path(repo_root)
    parents = {}
    for relative, expected in W06_R02_EXPECTED_PARENT_SHA256.items():
        actual = _sha256(root / relative)
        if actual != expected:
            raise W06R02EndpointProjectionError(
                f"R02 endpoint projection parent 漂移：{relative}")
        parents[relative] = actual

    seeds = read_authored_subset_member_seeds(root / W06_R02_SAMPLE_PATH)
    train = tuple(seed for seed in seeds if seed.split == "train")
    accepted = tuple(
        seed for seed in train if seed.perturbation_kind != "TYPE_MISMATCH")
    rejected = tuple(
        seed for seed in train if seed.perturbation_kind == "TYPE_MISMATCH")
    if len(train) != 6 or len(accepted) != 5 or len(rejected) != 1:
        raise W06R02EndpointProjectionError("R02 train/rejection inventory 漂移")
    ...
    for seed in accepted:
        ...
        if not isinstance(endpoints, list) or len(endpoints) != len(seed.endpoints):
            raise W06R02EndpointProjectionError("compiled endpoint inventory 漂移")
        for source, compiled_endpoint in zip(
                seed.endpoints, endpoints, strict=True):
            if not isinstance(compiled_endpoint, dict):
                raise W06R02EndpointProjectionError("compiled endpoint 非 object")
            ...
            if (local.object_kind != source.object_kind
                ...
                raise W06R02EndpointProjectionError("compiled endpoint kind 漂移")
    ...
    if len({tuple(item["local_endpoint_key"]) for item in rows}) != len(rows):
        raise W06R02EndpointProjectionError("local endpoint key 重复")

    return {
        "artifact_kind": "PH2_W06_R02_ENDPOINT_PROJECTION",
        "artifact_version": "PH2-W06-R02-ENDPOINT-PROJECTION-V1",
        "entries": rows,
        "execution_state": {
        ...
        "projection_policy": {
            ...
            "rejected_type_mismatch_count": len(rejected),
            "train_seed_count": len(accepted),
        },
        "status": "W06_R02_ENDPOINT_PROJECTION_PASS",
    }


def canonical_w06_r02_endpoint_projection_bytes(
        repo_root: str | Path,
        ) -> bytes:
    return canonical_json_bytes(
        build_w06_r02_endpoint_projection(repo_root)) + b"\n"


def _projection_from_value(value: dict[str, Any]) -> W06R02EndpointProjection:
    rows = value.get("entries")
    if not isinstance(rows, list) or not rows:
        raise W06R02EndpointProjectionError("endpoint projection entries 缺失")
    entries = []
    for row in rows:
        if not isinstance(row, dict) or set(row) != {
                "canonical_endpoint_key", "endpoint_id", "local_endpoint_key",
                "object_kind", "seed_id"}:
            raise W06R02EndpointProjectionError("endpoint projection entry 漂移")
        try:
        ...
        if (canonical != _canonical_endpoint(
            ...
            raise W06R02EndpointProjectionError("endpoint projection 内容漂移")
        entries.append(W06R02EndpointProjectionEntry(local, canonical))
    entries.sort(key=W06R02EndpointProjectionEntry.stable_key)
    return W06R02EndpointProjection(tuple(entries))


def read_w06_r02_endpoint_projection(
        path: str | Path,
        ) -> W06R02EndpointProjection:
    """严格回读 canonical overlay 并返回可注入 R-02 的 resolver。"""
    target = Path(path)
    try:
        payload = target.read_bytes()
    except OSError as error:
        raise W06R02EndpointProjectionError(
            "无法读取 R02 endpoint projection") from error
    if not payload.endswith(b"\n") or payload == b"\n":
        raise W06R02EndpointProjectionError("endpoint projection 换行非法")
    try:
    ...
    if canonical_json_bytes(value) + b"\n" != payload:
        raise W06R02EndpointProjectionError("endpoint projection 字节漂移")
    assert isinstance(value, dict)
    if (value.get("artifact_kind") != "PH2_W06_R02_ENDPOINT_PROJECTION"
            or value.get("status") != "W06_R02_ENDPOINT_PROJECTION_PASS"):
        raise W06R02EndpointProjectionError("endpoint projection 合同字段漂移")
    policy = value.get("projection_policy")
    if (not isinstance(policy, dict)
            or policy.get("held_out_mapping_count") != 0
            or policy.get("train_seed_count") != 5
            or policy.get("rejected_type_mismatch_count") != 1):
        raise W06R02EndpointProjectionError("endpoint projection policy 漂移")
    ...
    if (len(projection.entries) != policy.get("local_endpoint_count")
        ...
        raise W06R02EndpointProjectionError("endpoint projection count 漂移")
    return projection


def publish_w06_r02_endpoint_projection(
        repo_root: str | Path,
        output_path: str | Path | None = None,
        ) -> Path:
    """以排他创建发布 overlay，已有路径一律拒绝覆盖。"""
    root = Path(repo_root)
    target = root / (output_path or W06_R02_ENDPOINT_PROJECTION_PATH)
    target.parent.mkdir(parents=True, exist_ok=True)
    payload = canonical_w06_r02_endpoint_projection_bytes(root)
    try:
        with target.open("xb") as handle:
            handle.write(payload)
    except FileExistsError as error:
        raise W06R02EndpointProjectionError(
            "R02 endpoint projection 已存在，禁止覆盖") from error
    return target


__all__ = [
    "W06R02EndpointProjection",
    "W06R02EndpointProjectionEntry",
    "W06R02EndpointProjectionError",
    "W06_R02_ENDPOINT_PROJECTION_PATH",
    "build_w06_r02_endpoint_projection",
