"""W-03 LC-16 九载体补充资格的独立聚合 evaluator。"""
...
def _dimension(
    ...
    if not values:
        raise W03Lc16SupplementalError(f"{where} 没有结果")
    ...
    for course in overlay.carrier_courses:
        for case in course.cases:
            ...
            for direction in DIRECTIONS:
                ...
                if key in result:
                    raise W03Lc16SupplementalError("overlay 结果键重复")
                result[key] = owner_key
    if len(result) != DIRECTION_EVALUATION_COUNT:
        raise W03Lc16SupplementalError("overlay 方向覆盖不是 189")
    return result
...
def _verify_ablation_orthogonality(
    ...
    baseline_status[GENERATION_HARD_CONJUNCT] = generation.status
    for ablation in ablations:
        statuses = dict(ablation.dimension_statuses)
        statuses[GENERATION_HARD_CONJUNCT] = ablation.generation_status
        target_status = statuses[ablation.targeted_dimension]
        if baseline_status[ablation.targeted_dimension] == "PASS" \
                and target_status == "PASS":
            raise W03Lc16SupplementalError("消融未击穿目标维度")
        for dimension, status in statuses.items():
            if dimension != ablation.targeted_dimension \
                    and baseline_status[dimension] != status:
                raise W03Lc16SupplementalError("消融改变非目标维度")
...
def aggregate_w03_lc16_supplemental(
        overlay: D03Lc16SuccessorOverlay,
    ...
    if overlay.sha256() != OVERLAY_SHA256:
        raise W03Lc16SupplementalError("overlay SHA-256 漂移")
    expected = _expected_overlay_cases(overlay)
    if (not isinstance(results, tuple)
            or len(results) != DIRECTION_EVALUATION_COUNT):
        raise W03Lc16SupplementalError("supplemental 结果必须精确 189 条")
    ...
    for result in results:
        ...
        if key in seen or key not in expected or result.owner_key != expected[key]:
            raise W03Lc16SupplementalError("supplemental carrier/case/owner/direction 漂移")
        _verify_direction_ablation(result)
        seen.add(key)
    if seen != set(expected):
        raise W03Lc16SupplementalError("supplemental 189 方向覆盖不完整")
    ...
    _verify_ablation_orthogonality(dimensions, generation, ablations)
    ...
    if private_path_reads < 0 or private_payload_bytes < 0 \
        ...
        raise W03Lc16SupplementalError("supplemental read count 非法")
    return W03Lc16SupplementalReport(
        report_status,
        OVERLAY_SHA256,
        W03_PARENT_RECEIPT_SHA256,
        W02_SUPPLEMENTAL_RECEIPT_SHA256,
        safe_result_pack_sha256,
