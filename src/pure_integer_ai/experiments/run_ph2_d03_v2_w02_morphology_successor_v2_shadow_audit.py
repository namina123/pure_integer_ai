"""运行唯一 W-02 morphology successor V2 shadow audit。"""
...
def _failure_seal(output_root: Path, error: BaseException) -> None:
    write_immutable_json({
        "artifact_kind": (
            "PH2_D03_V2_W02_MORPHOLOGY_SUCCESSOR_V2_SHADOW_AUDIT_FAILURE_SEAL"),
...
def main() -> int:
    ...
    if target.exists() or failure.exists():
        raise RuntimeError("successor V2 shadow formal family 已消费，不得重跑")
    try:
