"""Run the single formal W-02 V3 source-route shadow probe."""
...
def _failure_seal(output: Path, error: BaseException) -> None:
    write_immutable_json({
        "artifact_kind": (
            "PH2_D03_V2_W02_MORPHOLOGY_SUCCESSOR_V3_ROUTE_SHADOW_FAILURE_SEAL"),
        ...
        "run_id": 1,
        "stage_key": "W-02",
        "status": "FAILED_OR_NE_NO_NEW_BLIND_FAMILY",
...
def main() -> int:
    ...
    if arguments.run_id != 1:
        raise RuntimeError("successor V3 route shadow formal run_id must be 1")
    ...
    if report_path.exists() or failure_path.exists():
        raise RuntimeError("successor V3 route shadow formal family is consumed")
    try:
        freeze = read_w02_morphology_successor_v3_shadow_freeze(
            arguments.repository_root)
        preflight = run_w02_morphology_successor_v3_shadow_preflight(
            arguments.repository_root,
        ...
        assert_w02_morphology_successor_v3_shadow_preflight(preflight, freeze)
        freeze_path = (
        ...
        report = {
            **preflight,
            ...
            "next_action": "W02_SUCCESSOR_V3_NEW_BLIND_OWNER_REQUIRED",
            "run_id": 1,
            "run_scope": "FORMAL",
            "shadow_started": 1,
        }
