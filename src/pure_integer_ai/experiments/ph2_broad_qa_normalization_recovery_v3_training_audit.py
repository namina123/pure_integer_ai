"""发布并严格回读 recovery-v3 TRAIN-only audit artifact。"""
...
from pure_integer_ai.experiments.ph2_broad_qa_normalization_recovery_v3_training_audit_records import (
...
NORMALIZATION_RECOVERY_V3_TRAINING_AUDIT_KIND = (
    "PH2_BROAD_QA_NORMALIZATION_RECOVERY_V3_TRAINING_AUDIT_V1")
NORMALIZATION_RECOVERY_V3_TRAINING_AUDIT_STATUS = (
    "TRAIN_ONLY_COMPLETE_NOT_FORMAL_NOT_DEPLOYED")

AUDIT_FILES = (
    ("runtime-audit.jsonl", "TRAIN_ONLY_RUNTIME_AUDIT", "case_id"),
    ("loso-audit.jsonl", "TRAIN_ONLY_LOSO_AUDIT", "loso_id"),
)
...
def _require_k_root(value: str | Path) -> Path:
    ...
    if not root.is_dir() or root.drive.upper() != "K:":
        raise BroadQaExternalDataError(
            "normalization recovery v3 audit run root 必须是 K 盘目录")
    return root
...
def _derive(
    ...
    """严格回读 protocol/pack 后重派生唯一 audit material。"""
    protocol_values = read_normalization_recovery_v3_learner_input(
        protocol_dir,
    ...
    pack_manifest, learned_outputs = read_normalization_recovery_v3_rule_pack(
        pack_dir,
    ...
    manifest = {
        "artifact_kind": NORMALIZATION_RECOVERY_V3_TRAINING_AUDIT_KIND,
        ...
        "status": NORMALIZATION_RECOVERY_V3_TRAINING_AUDIT_STATUS,
        "summary": summary,
...
def publish_normalization_recovery_v3_training_audit(
        *,
        run_root: str | Path,
    ...
    """不可覆盖发布 v3 TRAIN-only audit，并以 manifest-last 封口。"""
    ...
    if (not protocol_root.is_dir() or not pack_root.is_dir()
        ...
        raise BroadQaExternalDataError(
            "v3 audit 输入缺失、artifact 混淆或 target 已存在")
...
def read_normalization_recovery_v3_training_audit(
        audit_dir: str | Path,
        *,
        protocol_dir: str | Path,
        expected_protocol_manifest_sha256: str,
    ...
    """以三外部 SHA 重派生并严格回读完整 TRAIN-only audit。"""
    ...
    if (_overlap(root, protocol_root) or _overlap(root, pack_root)
            or _overlap(protocol_root, pack_root)):
        raise BroadQaExternalDataError("v3 audit artifact 根混淆")
    protocol_sha = _sha_value(
        expected_protocol_manifest_sha256,
        label="v3 audit expected protocol manifest")
    pack_sha = _sha_value(
        expected_pack_manifest_sha256,
        label="v3 audit expected pack manifest")
    audit_sha = _sha_value(
        expected_audit_manifest_sha256,
        label="v3 audit expected manifest")
    ...
    try:
        ...
        raise BroadQaExternalDataError("v3 audit manifest 不可读") from error
    if (_sha256(encoded) != audit_sha or not isinstance(stored, dict)
            or canonical_json_line(stored) != encoded
            or not _strict_equal(stored, expected)):
        raise BroadQaExternalDataError(
            "v3 audit manifest identity/encoding/material 漂移")
    for name, _role, _identity in AUDIT_FILES:
        try:
            payload = (root / name).read_bytes()
        except OSError as error:
            raise BroadQaExternalDataError(
                f"v3 audit {name} 不可读") from error
        if payload != payloads[name]:
            raise BroadQaExternalDataError(
                f"v3 audit {name} 与 protocol/pack 重派生漂移")
