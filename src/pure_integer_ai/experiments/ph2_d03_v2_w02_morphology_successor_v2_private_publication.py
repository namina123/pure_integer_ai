"""PASS-only public receipt for the successor V2 blind private run."""
...
from pure_integer_ai.experiments.ph2_d03_v2_w02_morphology_successor_v2_private_family import (
...
W02_MORPH_V2_PRIVATE_RECEIPT_VERSION = (
    "PH2-D03-V2-W02-MORPHOLOGY-SUCCESSOR-V2-PRIVATE-RECEIPT-V1"
)
W02_MORPH_V2_PRIVATE_RECEIPT_PATH = (
    "data/ph2/manifests/d03_v2/stages/"
    "ph2_d03_v2_w02_morphology_successor_v2_private_evaluation_receipt_v1.json"
)
W02_MORPH_V2_PRIVATE_REPORT = "publication/run-000001.report.json"
W02_MORPH_V2_PRIVATE_ZERO_WRITE_KEYS = frozenset({
    ...
    "v1_overlay_writes", "v2_overlay_writes",
})
W02_MORPH_V2_PRIVATE_ZERO_CALL_WINDOWS = (
    "BEFORE_PRIVATE_READ",
...
def _repository_file(repository: Path, relative: str) -> Path:
    ...
    if (not relative or "\\" in relative or pure.is_absolute()
        ...
        raise W02MorphologySuccessorV2PrivateFamilyError(
            "private publication repository file is invalid")
    return target
...
def _family_root(value: str | Path) -> Path:
    ...
    if (root.name != W02_MORPH_V2_PRIVATE_FORMAL_FAMILY_NAME
        ...
        raise W02MorphologySuccessorV2PrivateFamilyError(
            "private publication family root is invalid")
    return root
...
def _pass_dimension_map(
    ...
    for key, denominator in expected.items():
        ...
        if (row.get("denominator") != denominator
            ...
            raise W02MorphologySuccessorV2PrivateFamilyError(
                "private dimension did not pass its frozen denominator")
        _sha256(row.get("evidence_sha256"), where="private dimension evidence")
    return by_key
...
def _validate_formal_pass_report(
    ...
    if (report.get("artifact_kind")
            ...
            or report.get("validated_layout_count") != len(W02_MORPH_V2_PRIVATE_LAYOUTS)
            ...
            != W02_MORPH_V2_PRIVATE_SOURCE_COUNT
            + W02_MORPH_V2_PRIVATE_PAIR_COUNT * 2
            ...
            or report.get("formal_shadow_audit_attempts") != 2
            or report.get("formal_shadow_audit_passes") != 1
            or report.get("formal_shadow_audit_runs") != 1
            or report.get("formal_shadow_recovery_runs") != 1
        ...
        raise W02MorphologySuccessorV2PrivateFamilyError(
            "private formal PASS report identity or resource state drifted")
    _sha256(report.get("source_identity_sha256"), where="private source identity")
    if set(support) != set(W02_MORPH_V2_PRIVATE_SUPPORT_KEYS):
        raise W02MorphologySuccessorV2PrivateFamilyError(
            "private support gate inventory drifted")


def build_w02_morphology_successor_v2_private_receipt(
    ...
    run_identity = w02_morphology_successor_v2_private_run_identity(
        freeze, freeze_sha)
    verify_w02_morphology_successor_v2_private_consumed_guard(
        root,
    ...
    report_path = root / Path(*PurePosixPath(W02_MORPH_V2_PRIVATE_REPORT).parts)
    if report_path.is_symlink() or not report_path.is_file():
        raise W02MorphologySuccessorV2PrivateFamilyError(
            "private formal report is missing")
    report = read_canonical_object(report_path)
    if (not isinstance(report, dict)
            or report_path.read_bytes() != canonical_json_bytes(report) + b"\n"):
        raise W02MorphologySuccessorV2PrivateFamilyError(
            "private formal report is not canonical")
    ...
    return {
        ...
        "artifact_kind": (
            "PH2_D03_V2_W02_MORPHOLOGY_SUCCESSOR_V2_PRIVATE_RECEIPT"),
        "artifact_version": W02_MORPH_V2_PRIVATE_RECEIPT_VERSION,
        ...
        "run_id": 1,
        "stage_key": "W-02",
        "status": "W02_SUCCESSOR_V2_RUNTIME_EVIDENCED",
...
__all__ = [
    "W02_MORPH_V2_PRIVATE_RECEIPT_PATH",
    "W02_MORPH_V2_PRIVATE_RECEIPT_VERSION",
    "W02_MORPH_V2_PRIVATE_REPORT",
    "build_w02_morphology_successor_v2_private_receipt",
    "publish_w02_morphology_successor_v2_private_receipt",
]
