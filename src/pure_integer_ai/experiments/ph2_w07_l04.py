"""W07-L04 EXISTS 的 typed facade 与 bounded public report。

EXISTS 本身由共享 S-04 ``LogicExecutor`` 执行；本模块只负责冻结
L04 的 prefix、请求边界、量词 trace 和 U/R/G consumer 的公开装配。
"""
...
W07_L04_PREFIX = ("NOT", "AND_OR", "CONDITION", "EXISTS")
W07_L04_SUBSTAGE = "EXISTS"


@dataclass(frozen=True)
class W07L04RuntimeReport:
    """EXISTS 的 operator/content 分账、分支和 consumer 摘要。"""
...
def logic_request_for_proposal(
    ...
    """从来源化 EXISTS proposal 构造不携 expected state 的执行请求。"""
    if (not isinstance(proposal, W07LogicProposal)
            or proposal.observation.substage != W07_L04_SUBSTAGE):
        raise TypeError("W07-L04 request 只接受 EXISTS proposal")
...
def _quantifier(proposal: W07LogicProposal):
    if len(proposal.quantifiers) != 1:
        raise W07LogicContractError(
            "EXISTS proposal 必须包含唯一 Binder/Variable/domain")
    quantifier = proposal.quantifiers[0]
    if quantifier.definition.body_slot != proposal.specs[0].definition.slots[0]:
        raise W07LogicContractError("EXISTS body Role 与 operator slot 漂移")
    return quantifier


class W07L04Runtime:
    """共享 prefix learning owner，显式暴露 EXISTS 分支和 U/R/G。"""
    ...
    def __init__(
            ...
            *,
            protocol: W07LogicConsumerProtocol = (
                W07LogicConsumerProtocol(W07_L04_PREFIX)),
            ) -> None:
    ...
    def proposals(self) -> tuple[W07LogicProposal, ...]:
        return tuple(
            ...
            if item.observation.substage == W07_L04_SUBSTAGE)

    def quantifier_for(self, proposal: W07LogicProposal):
        """返回 proposal 的完整量词定义，拒绝模糊或错向绑定。"""
        if proposal not in self.proposals:
            raise W07LogicContractError("EXISTS quantifier proposal 不属于当前 runtime")
        return _quantifier(proposal)

    def execute(self, request: W07LogicRequest) -> W07LogicExecution | None:
        """执行一次真实 S-04 EXISTS，并保留完整 branch trace。"""
        if request.substage != W07_L04_SUBSTAGE:
            raise TypeError("W07-L04 execute 只接受 EXISTS request")
    ...
    def report(self) -> W07L04RuntimeReport:
        ...
        witness_support_count = sum(
            any(branch.state.support for branch in item.evaluation.branches)
...
def build_w07_l04_runtime(
    ...
    """构建严格 L01..L04 prefix 的共享学习/执行 owner。"""
    sliced = slice_w07_adapter(adapter, W07_L04_PREFIX)
    learning = build_w07_learning_runtime(backend, sliced)
    return W07L04Runtime(learning, sliced, protocol=protocol)
