"""W06-R06 train endpoint_id 到跨命题 canonical event identity 的追加投影。"""
...
W06_R06_EXPECTED_PARENT_SHA256 = {
    W06_R06_SAMPLE_PATH:
...
class W06R06EndpointProjectionError(RuntimeError):
    """R06 endpoint overlay、parent 或映射发生漂移。"""
...
def _sha256(path: Path) -> str:
    ...
    try:
        ...
        raise W06R06EndpointProjectionError(
            f"无法读取 endpoint projection 依赖：{path}") from error
    return digest.hexdigest()


def _canonical_endpoint(endpoint_id: str, object_kind: int) -> ObjectIdentity:
    """以 authored endpoint_id 构造不含 surface 的 canonical identity。"""
    if not isinstance(endpoint_id, str) or not endpoint_id:
        raise W06R06EndpointProjectionError("endpoint_id 必须是非空文本")
    if type(object_kind) is not int or object_kind <= 0:
        raise W06R06EndpointProjectionError("endpoint object kind 非法")
...
class W06R06EndpointProjectionEntry:
    """一条来源局部 event endpoint 到 canonical endpoint 的映射。"""
...
class W06R06EndpointProjection:
    """只按完整 local identity 查询的不可变 train projection。"""

    def __init__(
            self, entries: tuple[W06R06EndpointProjectionEntry, ...],
            ) -> None:
        if (not isinstance(entries, tuple) or not entries
                or any(not isinstance(item, W06R06EndpointProjectionEntry)
                       for item in entries)):
            raise W06R06EndpointProjectionError("endpoint projection entries 非法")
        normalized = tuple(sorted(
            entries, key=W06R06EndpointProjectionEntry.stable_key))
        if entries != normalized:
            raise W06R06EndpointProjectionError("endpoint projection 未规范排序")
        ...
        for item in entries:
            ...
            if prior is not None and prior != item.canonical_endpoint:
                raise W06R06EndpointProjectionError("local endpoint 映射冲突")
            mapping[item.local_endpoint] = item.canonical_endpoint
        if len(mapping) != len(entries):
            raise W06R06EndpointProjectionError("endpoint projection local key 重复")
    ...
    def clone_for_evaluation(self) -> "W06R06EndpointProjection":
        return W06R06EndpointProjection(self.entries)

    def state_key(self) -> tuple[int, ...]:
        values = [W06_R06_RUNTIME_NAMESPACE, 1, len(self.entries)]
...
def build_w06_r06_endpoint_projection(repo_root: str | Path) -> dict[str, Any]:
    """从公开 PRECEDES/train seed 重建 canonical event endpoint overlay。"""
    root = Path(repo_root)
    parents = {}
    for relative, expected in W06_R06_EXPECTED_PARENT_SHA256.items():
        actual = _sha256(root / relative)
        if actual != expected:
            raise W06R06EndpointProjectionError(
                f"R06 endpoint projection parent 漂移：{relative}")
        parents[relative] = actual

    seeds = read_authored_precedes_seeds(root / W06_R06_SAMPLE_PATH)
    ...
    for seed in train:
        ...
        for source, compiled_endpoint in zip(
            ...
            if not isinstance(compiled_endpoint, dict):
                raise W06R06EndpointProjectionError("compiled endpoint 非 object")
            ...
            if (local.object_kind != source.object_kind
                ...
                raise W06R06EndpointProjectionError("compiled endpoint kind 漂移")
    ...
    if len({tuple(item["local_endpoint_key"]) for item in rows}) != len(rows):
        raise W06R06EndpointProjectionError("local endpoint key 重复")

    return {
        "artifact_kind": "PH2_W06_R06_ENDPOINT_PROJECTION",
        "artifact_version": "PH2-W06-R06-ENDPOINT-PROJECTION-V1",
        "entries": rows,
        "execution_state": {
        ...
        "status": "W06_R06_ENDPOINT_PROJECTION_PASS",
    }


def canonical_w06_r06_endpoint_projection_bytes(
        repo_root: str | Path,
        ) -> bytes:
    return canonical_json_bytes(
        build_w06_r06_endpoint_projection(repo_root)) + b"\n"


def _projection_from_value(value: dict[str, Any]) -> W06R06EndpointProjection:
    rows = value.get("entries")
    if not isinstance(rows, list) or not rows:
        raise W06R06EndpointProjectionError("endpoint projection entries 缺失")
    entries = []
    for row in rows:
        if not isinstance(row, dict) or set(row) != {
                "canonical_endpoint_key", "endpoint_id", "local_endpoint_key",
                "object_kind", "seed_id"}:
            raise W06R06EndpointProjectionError("endpoint projection entry 漂移")
        try:
        ...
        if (canonical != _canonical_endpoint(
            ...
            raise W06R06EndpointProjectionError("endpoint projection 内容漂移")
        entries.append(W06R06EndpointProjectionEntry(local, canonical))
    entries.sort(key=W06R06EndpointProjectionEntry.stable_key)
    return W06R06EndpointProjection(tuple(entries))


def read_w06_r06_endpoint_projection(
        path: str | Path,
        ) -> W06R06EndpointProjection:
    target = Path(path)
    try:
        payload = target.read_bytes()
    except OSError as error:
        raise W06R06EndpointProjectionError(
            "无法读取 R06 endpoint projection") from error
    if not payload.endswith(b"\n") or payload == b"\n":
        raise W06R06EndpointProjectionError("endpoint projection 换行非法")
    try:
    ...
    if canonical_json_bytes(value) + b"\n" != payload:
        raise W06R06EndpointProjectionError("endpoint projection 字节漂移")
    assert isinstance(value, dict)
    if (value.get("artifact_kind") != "PH2_W06_R06_ENDPOINT_PROJECTION"
            or value.get("status") != "W06_R06_ENDPOINT_PROJECTION_PASS"):
        raise W06R06EndpointProjectionError("endpoint projection 合同字段漂移")
    policy = value.get("projection_policy")
    if (not isinstance(policy, dict)
            or policy.get("held_out_mapping_count") != 0
            or policy.get("train_seed_count") != 9
            or policy.get("rejected_type_mismatch_count") != 0):
        raise W06R06EndpointProjectionError("endpoint projection policy 漂移")
    ...
    if (len(projection.entries) != policy.get("local_endpoint_count")
        ...
        raise W06R06EndpointProjectionError("endpoint projection count 漂移")
    return projection


def publish_w06_r06_endpoint_projection(
        repo_root: str | Path,
        output_path: str | Path | None = None,
        ) -> Path:
    root = Path(repo_root)
    target = root / (output_path or W06_R06_ENDPOINT_PROJECTION_PATH)
    target.parent.mkdir(parents=True, exist_ok=True)
    payload = canonical_w06_r06_endpoint_projection_bytes(root)
    try:
        with target.open("xb") as handle:
            handle.write(payload)
    except FileExistsError as error:
        raise W06R06EndpointProjectionError(
            "R06 endpoint projection 已存在，禁止覆盖") from error
    return target


__all__ = [
    "W06R06EndpointProjection",
    "W06R06EndpointProjectionEntry",
    "W06R06EndpointProjectionError",
    "W06_R06_ENDPOINT_PROJECTION_PATH",
    "build_w06_r06_endpoint_projection",
    "canonical_w06_r06_endpoint_projection_bytes",
    "publish_w06_r06_endpoint_projection",
