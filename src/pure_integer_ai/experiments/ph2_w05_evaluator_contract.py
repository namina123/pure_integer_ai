"""PH2 W-05 独立 private evaluator 的文档、结果和安全 aggregate 合同。"""
...
W05_PRIVATE_OWNER_KEY = "PH2_W05_PRIVATE_EVALUATOR_OWNER"
W05_PRIVATE_FAMILY_FREEZE_NAME = "private_family_freeze.json"
W05_PRIVATE_FIRST_RUN_GUARD_NAME = "formal_first_run_guard.json"
W05_PRIVATE_AGGREGATE_NAME = "private_evaluation_aggregate.json"
W05_PRIVATE_RECOMMENDATION_NAME = "runtime_receipt_recommendation.json"
W05_PRIVATE_SOURCE_NAME = "private_source.json"
W05_PRIVATE_SCHEMA_NAME = "private_schema.json"
W05_PRIVATE_CASE_NAME = "private_cases.json"
W05_PRIVATE_LABEL_NAME = "private_labels.json"
W05_PRIVATE_CLUSTER_NAME = "private_clusters.json"
W05_EVALUATOR_PHASES = (
    ...
    "CLONE_COMPARE",
    "BASELINE",
    "ABLATION_OCCURRENCE_IDENTITY",
    "ABLATION_PROPOSITION_CONSUMER",
    "ABLATION_ROLE_SWAP",
    "ABLATION_SCOPE",
    "ABLATION_GENERATION",
    "INTEGRITY",
...
W05_EVALUATOR_FAILURE_PHASES = ("NONE", *W05_EVALUATOR_PHASES)


class W05PrivateEvaluationError(RuntimeError):
    """W-05 private 文档、结果或安全报告违反冻结合同。"""
...
def _strict_key(value: object, *, label: str) -> tuple[int, ...]:
    if (not isinstance(value, list) or not value
        ...
        raise W05PrivateEvaluationError(f"{label} 不是非空整数键")
    return tuple(value)


def _decode(payload: bytes, *, label: str) -> dict[str, Any]:
    if not isinstance(payload, bytes) or not payload:
        raise W05PrivateEvaluationError(f"{label} payload 为空")
    try:
        value = json.loads(payload.decode("utf-8"))
    except (UnicodeError, json.JSONDecodeError) as exc:
        raise W05PrivateEvaluationError(f"{label} payload 无法解析") from exc
    if not isinstance(value, dict) or canonical_json_bytes(value) != payload:
        raise W05PrivateEvaluationError(f"{label} payload 非 canonical object")
    return value


@dataclass(frozen=True)
class W05PrivateSource:
    ...
    def __post_init__(self) -> None:
        ...
        if self.owner_key != W05_PRIVATE_OWNER_KEY or self.license_id != "CC0-1.0":
            raise W05PrivateEvaluationError("private source owner/license 漂移")
        if not self.source_key:
            raise W05PrivateEvaluationError("private source key 为空")
...
class W05PrivateCase:
    ...
    def __post_init__(self) -> None:
        ...
        if self.dimension_key not in W05_EVALUATION_ORDER:
            raise W05PrivateEvaluationError("private case dimension 非法")
        if not self.challenge_key:
            raise W05PrivateEvaluationError("private challenge key 为空")


@dataclass(frozen=True)
class W05PrivateLabel:
    ...
    def __post_init__(self) -> None:
        ...
        if (self.dimension_key not in W05_EVALUATION_ORDER
            ...
            raise W05PrivateEvaluationError("private label threshold 漂移")


@dataclass(frozen=True)
class W05PrivateClusterBinding:
...
@dataclass(frozen=True)
class W05PrivatePayload:
    """五份 private 文档解码后的合同。"""

    source: W05PrivateSource
    cases: tuple[W05PrivateCase, ...]
    labels: tuple[W05PrivateLabel, ...]
    cluster_bindings: tuple[W05PrivateClusterBinding, ...]
    schema_key: str
    candidate_contract_sha256: str
    candidate_host_freeze_sha256: str
...
@dataclass(frozen=True)
class W05PrivateDimensionResult:
    ...
    def __post_init__(self) -> None:
        ...
        if (self.passed, self.fail_count, self.ne_count) != expected:
            raise W05PrivateEvaluationError("dimension result 状态/计数漂移")
...
def decode_w05_private_documents(
        source_bytes: bytes,
        schema_bytes: bytes,
        case_bytes: bytes,
        label_bytes: bytes,
    ...
    if (set(source) != {
            ...
            or source.get("artifact_kind") != "PH2_W05_PRIVATE_SOURCE"
            or source.get("format_version") != 1):
        raise W05PrivateEvaluationError("private source 版本漂移")
    source_value = W05PrivateSource(
        source["family_key"],
    ...
    if (set(schema) != {
            ...
            "ablation_order", "evaluation_order", "failure_phases", "fault_registry",
            "format_version", "label_fields", "schema_key"}
            or schema.get("artifact_kind") != "PH2_W05_PRIVATE_SCHEMA"
            or schema.get("format_version") != 1
            or schema.get("ablation_order") != list(W05_PRIVATE_ABLATION_KEYS)
            or schema.get("evaluation_order") != list(W05_EVALUATION_ORDER)
            or schema.get("failure_phases") != list(W05_EVALUATOR_FAILURE_PHASES)):
        raise W05PrivateEvaluationError("private schema 漂移")
    ...
    if (set(cases_doc) != {
            ...
            or cases_doc.get("artifact_kind") != "PH2_W05_PRIVATE_CASES"
            or labels_doc.get("artifact_kind") != "PH2_W05_PRIVATE_LABELS"
            or clusters_doc.get("artifact_kind") != "PH2_W05_PRIVATE_CLUSTERS"
            or not isinstance(raw_cases, list)
            or not isinstance(raw_labels, list)
            or not isinstance(raw_clusters, list)):
        raise W05PrivateEvaluationError("private case/label/cluster 文档漂移")
    if cases_doc.get("case_count") != len(W05_EVALUATION_ORDER):
        raise W05PrivateEvaluationError("private case count 漂移")
    cases = tuple(W05PrivateCase(
        item["case_key"],
        item["dimension_key"],
    ...
    clusters = tuple(W05PrivateClusterBinding(
        item["case_key"],
        item["source_cluster"],
    ...
    if (tuple(item.dimension_key for item in cases) != W05_EVALUATION_ORDER
        ...
        raise W05PrivateEvaluationError("private case 顺序或引用漂移")
    return W05PrivatePayload(
        source=source_value,
        cases=cases,
        labels=labels,
...
def public_safe_w05_aggregate(
        results: tuple[W05PrivateDimensionResult, ...],
        *,
    ...
    if failure_phase not in W05_EVALUATOR_FAILURE_PHASES:
        raise W05PrivateEvaluationError("failure phase 非法")
    if type(formal_run_count) is not int or formal_run_count != 1:
        raise W05PrivateEvaluationError("formal run count 必须为 1")
    if host_writes not in {0, 1} or label_writes not in {0, 1}:
        raise W05PrivateEvaluationError("host/label write count 非法")
    if failure_phase != "NONE":
        safe_results: tuple[W05PrivateDimensionResult, ...] = ()
        status = "NE"
        passed = fails = 0
        ne = 1
    else:
        if (not isinstance(results, tuple)
                ...
                != W05_EVALUATION_ORDER):
            raise W05PrivateEvaluationError("W-05 五维结果顺序漂移")
        safe_results = results
        passed = sum(item.passed for item in results)
        fails = sum(item.fail_count for item in results)
        ne = sum(item.ne_count for item in results)
        status = "PASS" if passed == len(W05_EVALUATION_ORDER) and not fails and not ne else (
    ...
    return {
        "ablation_results": [],
        "artifact_kind": "PH2_W05_PRIVATE_EVALUATION_AGGREGATE",
