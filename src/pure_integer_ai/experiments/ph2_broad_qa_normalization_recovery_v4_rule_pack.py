"""发布 recovery-v4 双运行等价后的禁用态 scoped phrase rule pack。"""
...
from pure_integer_ai.experiments.ph2_broad_qa_normalization_recovery_v4_learner import (
...
NORMALIZATION_RECOVERY_V4_RULE_PACK_KIND = (
    "PH2_BROAD_QA_NORMALIZATION_RECOVERY_V4_RULE_PACK_V1")
NORMALIZATION_RECOVERY_V4_RULE_PACK_STATUS = (
...
def _learner_reader(
    ...
    """把共享 pack runtime 的位置参数适配到 v4 reader。"""
    return read_normalization_recovery_v4_learner(
        run_dir,
...
def publish_normalization_recovery_v4_rule_pack(
        *,
        run_root: str | Path,
    ...
    return publish_materialized_rule_pack(
        ...
        label="normalization recovery v4 rule pack",
        artifact_kind=NORMALIZATION_RECOVERY_V4_RULE_PACK_KIND,
        status=NORMALIZATION_RECOVERY_V4_RULE_PACK_STATUS,
        format_version=1,
        output_file_roles=NORMALIZATION_RECOVERY_V4_OUTPUT_FILE_ROLES,
        learner_reader=_learner_reader,
        payload_builder=normalization_recovery_v4_output_payloads,
        fixed_manifest_fields=_FIXED_MANIFEST_FIELDS,
    )


def read_normalization_recovery_v4_rule_pack(
        pack_dir: str | Path,
        *,
        protocol_dir: str | Path,
        expected_protocol_manifest_sha256: str,
        expected_pack_manifest_sha256: str,
                   ...
                   dict[str, tuple[dict[str, object], ...]]]:
    """以双外部 SHA 与 protocol 重派生严格回读 v4 disabled pack。"""
