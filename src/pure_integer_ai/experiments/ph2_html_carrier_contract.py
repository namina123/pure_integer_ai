"""LC-16 HTML 七类 payload 与 DOM adapter evidence 合同。"""
...
FORMAT_VERSION = 1
ARTIFACT_KIND = "PH2_LC16_HTML_CARRIER_PAYLOAD"
ARTIFACT_VERSION = "LC16-HTML-CARRIER-20260731-A"
ARTIFACT_STATUS = "PAYLOAD_FROZEN_DOM_ADAPTER_ONLY"
CARRIER_KEY = "HTML"
LICENSE_ID = "CC0-1.0"
PARSER_PACKAGE = "lxml"
PARSER_VERSION = "6.1.1"
EMPTY_SHA256 = hashlib.sha256(b"").hexdigest()
CASE_KEYS = tuple(
    StableRecordKey((16616000, 1, 2, ordinal))
    for ordinal in range(1, len(SAMPLE_KINDS) + 1)
)
ADAPTER_OBLIGATIONS = {
    "POSITIVE": "DOM_ATTRIBUTE_AND_TEXT_ROUND_TRIP",
    "NEGATIVE": "RAW_SOURCE_NOT_DOM_NORMALIZED",
    "AMBIGUOUS": "REPAIRED_STRUCTURE_WITHOUT_SEMANTIC_PRESELECTION",
    "UNKNOWN": "CUSTOM_ELEMENT_AND_ATTRIBUTE_PRESERVATION",
    "REVISION": "DOM_REVISION_MAPPING",
    "GENERATION": "EXACT_HTML_SERIALIZATION",
    "RETENTION": "NESTED_DOM_AND_REFERENCE_RELOAD_IDENTITY",
}
...
class HtmlCarrierContractError(RuntimeError):
    """HTML payload、manifest 或 evidence 身份不闭合。"""


def _exact(value: Any, expected: set[str], *, where: str) -> dict[str, Any]:
    if not isinstance(value, dict) or set(value) != expected:
        raise HtmlCarrierContractError(f"{where} 字段不精确")
    return value
...
def _text(
    ...
    if not isinstance(value, str) or (not allow_empty and not value):
        raise HtmlCarrierContractError(f"{where} 必须是文本")
    if strip and value and value.strip() != value:
        raise HtmlCarrierContractError(f"{where} 含首尾空白")
    return value


def _positive(value: Any, *, where: str) -> int:
    if type(value) is not int or value <= 0:
        raise HtmlCarrierContractError(f"{where} 必须是正严格整数")
    return value


def _nonnegative(value: Any, *, where: str) -> int:
    if type(value) is not int or value < 0:
        raise HtmlCarrierContractError(f"{where} 必须是非负严格整数")
    return value
...
def _sha256(value: Any, *, where: str) -> str:
    ...
    if (len(text) != 64
        ...
        raise HtmlCarrierContractError(f"{where} 必须是小写 SHA-256")
    return text
...
def _relative_path(value: Any, *, where: str) -> str:
    ...
    if (not path.parts or path.is_absolute() or ".." in path.parts
            ...
            or ":" in path.parts[0]):
        raise HtmlCarrierContractError(f"{where} 必须是安全 POSIX 相对路径")
    return text
...
def _raw_identity(
    ...
    if text:
        try:
            ...
            raise HtmlCarrierContractError(
                f"{where} 含非法 Unicode scalar") from error
    if _nonnegative(unit_count, where=f"{where} unit_count") != len(text):
        raise HtmlCarrierContractError(f"{where} unit_count 漂移")
    expected = hashlib.sha256(text.encode("utf-8")).hexdigest()
    if _sha256(sha256, where=f"{where} utf8_sha256") != expected:
        raise HtmlCarrierContractError(f"{where} UTF-8 SHA-256 漂移")


@dataclass(frozen=True, order=True)
class HtmlCarrierRecord:
    """一个绑定 parent case 且不含语义 expected label 的 HTML payload。"""
    ...
    def __post_init__(self) -> None:
        if self.format_version != FORMAT_VERSION:
            raise HtmlCarrierContractError("record format_version 非法")
        if not isinstance(self.case_key, StableRecordKey):
            raise HtmlCarrierContractError("record case_key 非法")
        if self.case_key not in CASE_KEYS:
            raise HtmlCarrierContractError("record case_key 不属于 HTML 七类 case")
        if self.sample_kind not in SAMPLE_KINDS:
            raise HtmlCarrierContractError("record sample_kind 未登记")
        expected_index = SAMPLE_KINDS.index(self.sample_kind)
        if self.case_key != CASE_KEYS[expected_index]:
            raise HtmlCarrierContractError("record case_key/sample_kind 错配")
        if self.adapter_obligation != ADAPTER_OBLIGATIONS[self.sample_kind]:
            raise HtmlCarrierContractError("record adapter obligation 漂移")
        if self.license_id != LICENSE_ID:
            raise HtmlCarrierContractError("HTML payload 必须为 CC0-1.0")
        _raw_identity(
        ...
        if self.sample_kind == "REVISION":
            if not self.previous_text or self.previous_text == self.raw_text:
                raise HtmlCarrierContractError(
                    "REVISION 必须携带不同的 previous HTML")
    ...
    @classmethod
    def from_dict(cls, value: dict[str, Any]) -> "HtmlCarrierRecord":
        raw = _exact(value, {
        ...
        }, where="HtmlCarrierRecord")
        try:
            case_key = StableRecordKey.from_value(
                raw["case_key"], where="record case_key")
        except Exception as error:
            raise HtmlCarrierContractError("record case_key 损坏") from error
        return cls(
            raw["format_version"],
            case_key,
            str(raw["sample_kind"]),
...
def read_html_carrier_records(
        path: str | Path,
        ) -> tuple[HtmlCarrierRecord, ...]:
    """严格读取七行 canonical JSONL，不容忍 BOM、空行或尾随字节。"""
    try:
        payload = Path(path).read_bytes()
        if not payload or not payload.endswith(b"\n"):
            raise HtmlCarrierContractError("payload 缺少终止换行")
        lines = payload.splitlines(keepends=True)
        if any(not line.endswith(b"\n") or line == b"\n" for line in lines):
            raise HtmlCarrierContractError("payload 含坏行或空行")
        records = []
        for line in lines:
            value = parse_canonical_json_bytes(line[:-1], require_object=True)
            if canonical_json_line(value) != line:
                raise HtmlCarrierContractError("payload 行不是 canonical JSON")
            records.append(HtmlCarrierRecord.from_dict(value))
    except HtmlCarrierContractError:
        raise
    except Exception as error:
        raise HtmlCarrierContractError("HTML payload 损坏") from error
    result = tuple(records)
    if len(result) != len(SAMPLE_KINDS):
        raise HtmlCarrierContractError("payload 必须精确包含七行")
    if tuple(item.sample_kind for item in result) != SAMPLE_KINDS:
        raise HtmlCarrierContractError("payload sample_kind 顺序或覆盖漂移")
    if tuple(item.case_key for item in result) != CASE_KEYS:
        raise HtmlCarrierContractError("payload HTML case_key 顺序或覆盖漂移")
    return result


def write_html_carrier_records(
        records: tuple[HtmlCarrierRecord, ...],
        path: str | Path,
        ) -> Path:
    """排他或幂等写七行 payload，禁止异内容覆盖。"""
    if (not isinstance(records, tuple)
            or any(not isinstance(item, HtmlCarrierRecord)
                   for item in records)
            or tuple(item.sample_kind for item in records) != SAMPLE_KINDS
            or tuple(item.case_key for item in records) != CASE_KEYS):
        raise HtmlCarrierContractError("records 类型、顺序或覆盖非法")
    payload = b"".join(item.canonical_line() for item in records)
    target = Path(path)
    if target.exists():
        if not target.is_file() or target.read_bytes() != payload:
            raise HtmlCarrierContractError("payload 已存在且内容不同")
        return target
    ...
    try:
        ...
        raise HtmlCarrierContractError("payload 无法写入") from error
    return target


@dataclass(frozen=True, order=True)
class HtmlMaterializationIdentity:
    """一个 HTML case 的 adapter 输出摘要及对象计数。"""
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.case_key, StableRecordKey):
            raise HtmlCarrierContractError("materialization case_key 非法")
        if self.case_key not in CASE_KEYS:
            raise HtmlCarrierContractError("materialization case_key 非 HTML case")
        ...
        if self.anchor_count <= self.envelope_count * 2:
            raise HtmlCarrierContractError(
                "materialization 未保留 DOM tree anchors")
        if self.structure_node_count <= 0:
            raise HtmlCarrierContractError(
                "materialization 缺少 DOM structure nodes")
        if self.revision_count == 0:
            ...
            raise HtmlCarrierContractError("revision_count 必须为 0/1")
    ...
    @classmethod
    def from_dict(cls, value: dict[str, Any]) -> "HtmlMaterializationIdentity":
        raw = _exact(value, {
        ...
        }, where="HtmlMaterializationIdentity")
        return cls(
...
class HtmlCarrierEvidenceFile:
    ...
    def __post_init__(self) -> None:
        ...
        if self.role not in EVIDENCE_ROLES:
            raise HtmlCarrierContractError("evidence role 未登记")
    ...
    @classmethod
    def from_dict(cls, value: dict[str, Any]) -> "HtmlCarrierEvidenceFile":
        raw = _exact(value, {
            "byte_count", "relative_path", "role", "sha256",
        }, where="HtmlCarrierEvidenceFile")
        return cls(
...
@dataclass(frozen=True, order=True)
class HtmlCarrierPayloadManifest:
    ...
    materializations: tuple[HtmlMaterializationIdentity, ...]
    execution_state: CanonicalJsonObject
    evidence_files: tuple[HtmlCarrierEvidenceFile, ...]

    def __post_init__(self) -> None:
        if self.format_version != FORMAT_VERSION:
            raise HtmlCarrierContractError("manifest format_version 非法")
        if (self.artifact_version != ARTIFACT_VERSION
                or self.artifact_status != ARTIFACT_STATUS):
            raise HtmlCarrierContractError("manifest artifact identity 漂移")
        _relative_path(
        ...
        if self.carrier_key != CARRIER_KEY or self.license_id != LICENSE_ID:
            raise HtmlCarrierContractError("manifest carrier/license 非法")
        if (self.parser_package != PARSER_PACKAGE
                or self.parser_version != PARSER_VERSION):
            raise HtmlCarrierContractError("manifest parser identity 漂移")
        if (not isinstance(self.budget, TypedCarrierBudget)
            ...
            raise HtmlCarrierContractError("manifest budget 非法")
        if self.case_keys != CASE_KEYS:
            raise HtmlCarrierContractError("manifest HTML case_keys 漂移")
        if self.sample_kinds != SAMPLE_KINDS:
            raise HtmlCarrierContractError("manifest sample_kinds 漂移")
        if (not isinstance(self.materializations, tuple)
                or any(not isinstance(item, HtmlMaterializationIdentity)
                       for item in self.materializations)
                or tuple(item.case_key for item in self.materializations)
                != self.case_keys):
            raise HtmlCarrierContractError("manifest materializations 漂移")
        if (not isinstance(self.execution_state, CanonicalJsonObject)
                or self.execution_state.to_value() != EXECUTION_STATE):
            raise HtmlCarrierContractError("manifest execution_state 必须全零")
        if (not isinstance(self.evidence_files, tuple)
                or any(not isinstance(item, HtmlCarrierEvidenceFile)
            ...
            raise HtmlCarrierContractError("manifest evidence_files 未闭合")
    ...
    @classmethod
    def from_dict(cls, value: dict[str, Any]) -> "HtmlCarrierPayloadManifest":
        raw = _exact(value, {
            ...
            "sample_sha256",
        }, where="HtmlCarrierPayloadManifest")
        if raw["artifact_kind"] != ARTIFACT_KIND:
            raise HtmlCarrierContractError("manifest artifact_kind 非法")
        try:
            return cls(
                raw["format_version"],
                ...
                tuple(HtmlMaterializationIdentity.from_dict(item)
                      for item in raw["materializations"]),
                CanonicalJsonObject.from_value(raw["execution_state"]),
                tuple(HtmlCarrierEvidenceFile.from_dict(item)
                      for item in raw["evidence_files"]),
            )
        except HtmlCarrierContractError:
            raise
        except Exception as error:
            raise HtmlCarrierContractError("manifest 嵌套字段损坏") from error


def read_html_carrier_manifest(
        path: str | Path,
        ) -> HtmlCarrierPayloadManifest:
    """严格回读 canonical HTML manifest。"""
    try:
        payload = Path(path).read_bytes()
        if not payload.endswith(b"\n") or payload.endswith(b"\n\n"):
            raise HtmlCarrierContractError("manifest newline 非法")
        value = parse_canonical_json_bytes(payload[:-1], require_object=True)
        manifest = HtmlCarrierPayloadManifest.from_dict(value)
    except HtmlCarrierContractError:
        raise
    except Exception as error:
        raise HtmlCarrierContractError("manifest 损坏") from error
    if manifest.canonical_bytes() != payload:
        raise HtmlCarrierContractError("manifest 不是 canonical 字节")
    return manifest


def write_html_carrier_manifest(
        manifest: HtmlCarrierPayloadManifest,
        path: str | Path,
        ) -> Path:
    """排他或幂等写 manifest，不允许异内容覆盖。"""
    if not isinstance(manifest, HtmlCarrierPayloadManifest):
        raise HtmlCarrierContractError("manifest 类型非法")
    target = Path(path)
    payload = manifest.canonical_bytes()
    if target.exists():
        if not target.is_file() or target.read_bytes() != payload:
            raise HtmlCarrierContractError("manifest 已存在且内容不同")
        return target
    target.parent.mkdir(parents=True, exist_ok=True)
    try:
        with target.open("xb") as handle:
        ...
        raise HtmlCarrierContractError("manifest 无法写入") from error
    return target


def verify_html_carrier_files(
        manifest: HtmlCarrierPayloadManifest,
        *,
    ...
    for relative_path, sha256, byte_count in paths:
        ...
        try:
            ...
            raise HtmlCarrierContractError("evidence 路径逃逸") from error
        if not target.is_file():
            raise HtmlCarrierContractError("evidence 文件缺失")
        payload = target.read_bytes()
        if ((byte_count is not None and len(payload) != byte_count)
                or hashlib.sha256(payload).hexdigest() != sha256):
            raise HtmlCarrierContractError("evidence 文件身份漂移")
