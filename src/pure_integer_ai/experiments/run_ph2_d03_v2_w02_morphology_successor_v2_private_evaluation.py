"""Run the only successor V2 blind private evaluation."""
...
def _failure_seal(
    ...
    write_immutable_json({
        "artifact_kind": (
            "PH2_D03_V2_W02_MORPHOLOGY_SUCCESSOR_V2_PRIVATE_FAILURE_SEAL"),
...
def _private_owner_root(value: str | Path, repository: Path) -> Path:
    ...
    if (root.is_symlink() or not root.is_dir()
        ...
        raise RuntimeError("private owner root is invalid")
    return root
...
def _validate_pre_guard_inputs(
    ...
    """Reject path or public-artifact mistakes before consuming the guard."""
    ...
    for identity in files:
        ...
        relative = W02_MORPH_V2_PRIVATE_PATHS.get(layout_key)
        ...
        if (not relative or target.is_symlink() or not target.is_file()
            ...
            raise RuntimeError("private owner transport metadata drifted")
    ...
    if (candidate.artifact_manifest_sha256
        ...
        raise RuntimeError("formal artifact roots do not match the public freeze")
...
def main() -> int:
    ...
    if family_root.name != W02_MORPH_V2_PRIVATE_FORMAL_FAMILY_NAME:
        raise RuntimeError("private formal family name drifted")
    ...
    if report_path.exists() or failure_path.exists():
        raise RuntimeError("private formal family has already been consumed")

    phase = "PRE_GUARD"
    report: dict[str, object] | None = None
    try:
        freeze = read_w02_morphology_successor_v2_private_family_freeze(
            repository)
        freeze_path = repository / Path(
            *W02_MORPH_V2_PRIVATE_FAMILY_FREEZE_PATH.split("/"))
        family_freeze_sha = _sha256_file(freeze_path)[1]
        if (read_canonical_object(
                family_root / W02_MORPH_V2_PRIVATE_FAMILY_DOCUMENT) != freeze
                or read_canonical_object(
                    family_root / W02_MORPH_V2_PRIVATE_REGISTRATION_DOCUMENT)
                != freeze["registration"]):
            raise RuntimeError("private formal family documents drifted")
        registration = (
            w02_morphology_successor_v2_private_registration_from_freeze(
                freeze))
        _, files = read_w02_morphology_successor_v2_private_owner_receipt(
            repository)
        if [row.to_dict() for row in files] != freeze["owner_input_files"]:
            raise RuntimeError("private owner inventory drifted before guard")
        ...
        roots = V2PhysicalRoots.from_paths(
            ...
            owner_root,
            family_root / W02_MORPH_V2_PRIVATE_EXPOSURE_LEDGER,
        )
        report = run_w02_morphology_successor_v2_private_evaluation(
        ...
        if report["status"] != "PASS":
            error = RuntimeError(
                "formal private result was FAIL or NE; public receipt blocked")
            _failure_seal(
