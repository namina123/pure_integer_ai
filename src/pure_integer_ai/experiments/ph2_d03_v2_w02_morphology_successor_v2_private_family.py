"""Metadata-only freeze for the successor V2 blind private family."""
...
W02_MORPH_V2_PRIVATE_FAMILY_VERSION = (
    "PH2-D03-V2-W02-MORPHOLOGY-SUCCESSOR-V2-PRIVATE-FAMILY-V1"
)
W02_MORPH_V2_PRIVATE_FAMILY_FREEZE_PATH = (
    "data/ph2/manifests/d03_v2/stages/"
    "ph2_d03_v2_w02_morphology_successor_v2_private_family_freeze_v1.json"
)
W02_MORPH_V2_PRIVATE_FAMILY_CODE_PATHS = (
    "src/pure_integer_ai/experiments/"
    "ph2_d03_v2_w02_morphology_successor_v2_private_evaluator.py",
    "src/pure_integer_ai/experiments/"
    "ph2_d03_v2_w02_morphology_successor_v2_private_family.py",
    "src/pure_integer_ai/experiments/"
    "ph2_d03_v2_w02_morphology_successor_v2_private_publication.py",
    "src/pure_integer_ai/experiments/"
    "run_ph2_d03_v2_w02_morphology_successor_v2_private_evaluation.py",
    "tests/test_ph2_d03_v2_w02_morphology_successor_v2_private_evaluation.py",
    "tests/test_ph2_d03_v2_w02_morphology_successor_v2_private_family.py",
)
W02_MORPH_V2_PRIVATE_FORMAL_FAMILY_NAME = (
    ".d03-w02-morph-successor-v2-private-formal-20260809-a"
)
W02_MORPH_V2_PRIVATE_FAMILY_DOCUMENT = "private-family-freeze.json"
W02_MORPH_V2_PRIVATE_REGISTRATION_DOCUMENT = "private-family-registration.json"
W02_MORPH_V2_PRIVATE_GUARD_AVAILABLE = "run-guard/available.guard.json"
W02_MORPH_V2_PRIVATE_GUARD_CONSUMED = "run-guard/consumed.guard.json"
W02_MORPH_V2_PRIVATE_RUN_INTENT = "run-guard/run-intent.json"
W02_MORPH_V2_PRIVATE_EXPOSURE_LEDGER = "exposure-ledger"


# object-model: exception
class W02MorphologySuccessorV2PrivateFamilyError(RuntimeError):
    """The public freeze, dependency chain, guard, or receipt drifted."""


def _sha256(value: object, *, where: str) -> str:
    if (not isinstance(value, str) or len(value) != 64
            or any(char not in "0123456789abcdef" for char in value)):
        raise W02MorphologySuccessorV2PrivateFamilyError(
...
def _repository_file(repository: Path, relative: str) -> Path:
    ...
    if (not relative or "\\" in relative or pure.is_absolute()
        ...
        raise W02MorphologySuccessorV2PrivateFamilyError(
            "private family repository file is invalid")
    return target
...
def _code_rows(repository: Path) -> tuple[list[dict[str, object]], str]:
    rows = [
        ...
        for relative in W02_MORPH_V2_PRIVATE_FAMILY_CODE_PATHS
    ]
...
def _family_root(value: str | Path, *, require_exists: bool) -> Path:
    ...
    if (root.name != W02_MORPH_V2_PRIVATE_FORMAL_FAMILY_NAME
            or root.is_symlink()
            or (require_exists and not root.is_dir())):
        raise W02MorphologySuccessorV2PrivateFamilyError(
            "private formal family root is invalid")
    return root
...
def _dependency_state(repository: Path) -> dict[str, Any]:
    ...
    dev_freeze = read_w02_morphology_successor_v2_dev_calibration_freeze(
        repository)
    shadow_freeze = read_w02_morphology_successor_v2_shadow_audit_freeze(
        repository)
    recovery_freeze = read_w02_morphology_successor_v2_shadow_recovery_freeze(
        repository)
    owner, files = read_w02_morphology_successor_v2_private_owner_receipt(
        repository)
    paths = {
        ...
        "v2_dev_freeze": W02_MORPH_SUCCESSOR_V2_DEV_FREEZE_PATH,
        "v2_dev_report": W02_MORPH_SUCCESSOR_V2_DEV_REPORT_PATH,
        "v2_shadow_freeze": W02_MORPH_SUCCESSOR_V2_SHADOW_FREEZE_PATH,
        "v2_shadow_recovery_freeze": W02_MORPH_V2_SHADOW_RECOVERY_FREEZE_PATH,
        "v2_shadow_report": W02_MORPH_SUCCESSOR_V2_SHADOW_REPORT_PATH,
        "owner_receipt": W02_MORPH_V2_PRIVATE_OWNER_RECEIPT_PATH,
    }
    identities = {
    ...
    dev = read_canonical_object(
        _repository_file(repository, paths["v2_dev_report"]))
    shadow = read_canonical_object(
        _repository_file(repository, paths["v2_shadow_report"]))
    if (candidate.get("status") != "W02_CANDIDATE_ARTIFACT_FROZEN"
            ...
            or dev.get("dev_freeze_file_sha256")
            != identities["v2_dev_freeze"]["sha256"]
            or dev.get("candidate_artifact_manifest_sha256")
            != candidate.get("candidate_artifact_manifest_sha256")
            or dev.get("v1_overlay_artifact_manifest_sha256")
            != v1.get("overlay_artifact_manifest_sha256")
            or dev.get("v2_overlay_artifact_manifest_sha256")
            != v2.get("v2_overlay_artifact_manifest_sha256")
            or shadow.get("status") != "PASS"
            or shadow.get("formal_shadow_audit_runs") != 1
            or shadow.get("formal_shadow_audit_attempts") != 2
            or shadow.get("formal_shadow_audit_passes") != 1
            or shadow.get("formal_shadow_recovery_runs") != 1
            or shadow.get("interrupted_parent_formal_shadow_runs") != 1
            or shadow.get("label_reads") != 0
            or shadow.get("code_freeze_sha256")
            != shadow_freeze.get("code_freeze_sha256")
            or shadow.get("shadow_freeze_file_sha256")
            != identities["v2_shadow_freeze"]["sha256"]
            or shadow.get("recovery_code_freeze_sha256")
            != recovery_freeze.get("code_freeze_sha256")
            or shadow.get("recovery_freeze_file_sha256")
            != identities["v2_shadow_recovery_freeze"]["sha256"]
            or shadow.get("dev_pass_report_file_sha256")
            != identities["v2_dev_report"]["sha256"]
            or shadow.get("candidate_artifact_manifest_sha256")
            != candidate.get("candidate_artifact_manifest_sha256")
            or shadow.get("v1_overlay_artifact_manifest_sha256")
            != v1.get("overlay_artifact_manifest_sha256")
            or shadow.get("v2_overlay_artifact_manifest_sha256")
            != v2.get("v2_overlay_artifact_manifest_sha256")
            ...
            != W02_MORPH_V2_PRIVATE_OWNER_METADATA_SHA256
            or owner.get("pair_count") != W02_MORPH_V2_PRIVATE_PAIR_COUNT
            or owner.get("source_count") != W02_MORPH_V2_PRIVATE_SOURCE_COUNT
            or tuple(row.layout_key for row in files)
            != W02_MORPH_V2_PRIVATE_LAYOUTS):
        raise W02MorphologySuccessorV2PrivateFamilyError(
            "private family dependency chain did not close")
    zero_values = (
    ...
    if any(value != 0 for value in zero_values):
        raise W02MorphologySuccessorV2PrivateFamilyError(
            "private family dependency already consumed private data")
    chain = {
        ...
        "v2_dev_freeze_file_sha256": identities["v2_dev_freeze"]["sha256"],
        "v2_dev_pass_report_file_sha256":
            identities["v2_dev_report"]["sha256"],
        ...
        "v2_shadow_freeze_file_sha256":
            identities["v2_shadow_freeze"]["sha256"],
        "v2_shadow_pass_report_file_sha256":
            identities["v2_shadow_report"]["sha256"],
        "v2_shadow_recovery_freeze_file_sha256":
            identities["v2_shadow_recovery_freeze"]["sha256"],
    }
    return {
...
def w02_morphology_successor_v2_private_guard_value(
    ...
    return {
        "artifact_chain_sha256": _sha256(
            artifact_chain_sha256, where="private artifact chain"),
        "artifact_kind": (
            "PH2_D03_V2_W02_MORPHOLOGY_SUCCESSOR_V2_PRIVATE_FIRST_RUN_GUARD"),
        "code_freeze_sha256": _sha256(
            code_freeze_sha256, where="private code freeze"),
        "family_commitment": _sha256(
            family_commitment, where="private family commitment"),
        "formal_private_evaluation_runs": 0,
        "format_version": 1,
        "guard_consumed": 0,
        "guard_version": W02_MORPH_V2_PRIVATE_FAMILY_VERSION,
        "owner_family_key": W02_MORPH_V2_PRIVATE_OWNER_FAMILY_KEY,
...
def build_w02_morphology_successor_v2_private_family_freeze(
        repository_root: str | Path) -> dict[str, object]:
    """Freeze the family from public metadata and public PASS evidence only."""
    repository = Path(repository_root).resolve()
    dependency = _dependency_state(repository)
    owner = dependency["owner"]
    code_rows, code_sha = _code_rows(repository)
    budget = V2EvaluatorResourceBudget.from_dict(owner["resource_budget"])
    ...
    guard = w02_morphology_successor_v2_private_guard_value(
    ...
    return {
        ...
        "artifact_chain_sha256": dependency["artifact_chain_sha256"],
        "artifact_kind": (
            "PH2_D03_V2_W02_MORPHOLOGY_SUCCESSOR_V2_PRIVATE_FAMILY_FREEZE"),
        "artifact_version": W02_MORPH_V2_PRIVATE_FAMILY_VERSION,
        "code_files": code_rows,
        "code_freeze_sha256": code_sha,
        "dimension_denominator_counts": owner["dimension_denominator_counts"],
        "evaluator_version": W02_MORPH_V2_PRIVATE_EVALUATOR_VERSION,
        ...
        "parent_evidence_files": [
            ...
            for key in (
                ...
                "v2_dev_freeze", "v2_dev_report", "v2_shadow_freeze",
                "v2_shadow_recovery_freeze", "v2_shadow_report",
                "owner_receipt",
            )
        ],
        ...
        "stage_key": "W-02",
        "status": "W02_SUCCESSOR_V2_BLIND_PRIVATE_FAMILY_FROZEN",
        "support_hard_conjunct_keys": list(W02_MORPH_V2_PRIVATE_SUPPORT_KEYS),
        "teacher_calls": 0,
...
def publish_w02_morphology_successor_v2_private_family_freeze(
        repository_root: str | Path) -> Path:
    repository = Path(repository_root).resolve()
    value = build_w02_morphology_successor_v2_private_family_freeze(repository)
    target = repository / Path(*PurePosixPath(
        W02_MORPH_V2_PRIVATE_FAMILY_FREEZE_PATH).parts)
    write_immutable_json(value, target)
    if target.read_bytes() != canonical_json_bytes(value) + b"\n":
        raise W02MorphologySuccessorV2PrivateFamilyError(
            "private family freeze publication drifted")
    return target


def read_w02_morphology_successor_v2_private_family_freeze(
        repository_root: str | Path) -> dict[str, object]:
    repository = Path(repository_root).resolve()
    target = _repository_file(
        repository, W02_MORPH_V2_PRIVATE_FAMILY_FREEZE_PATH)
    value = read_canonical_object(target)
    expected = build_w02_morphology_successor_v2_private_family_freeze(
        repository)
    if value != expected or target.read_bytes() != canonical_json_bytes(value) + b"\n":
        raise W02MorphologySuccessorV2PrivateFamilyError(
            "private family freeze no longer matches live identities")
    return value


def w02_morphology_successor_v2_private_registration_from_freeze(
    ...
    if not isinstance(registration, dict) or not isinstance(budget, dict):
        raise W02MorphologySuccessorV2PrivateFamilyError(
            "private family registration fields are invalid")
    ...
    if rebuilt.to_dict() != registration:
        raise W02MorphologySuccessorV2PrivateFamilyError(
            "private family registration canonical value drifted")
    return rebuilt


def publish_w02_morphology_successor_v2_private_family_root(
        repository_root: str | Path, family_root: str | Path) -> str:
    """Publish registration, freeze, ledger, and an available guard off Git."""
    repository = Path(repository_root).resolve()
    freeze = read_w02_morphology_successor_v2_private_family_freeze(repository)
    registration = w02_morphology_successor_v2_private_registration_from_freeze(
        freeze)
    root = _family_root(family_root, require_exists=False)
    if root.exists():
        raise W02MorphologySuccessorV2PrivateFamilyError(
            "private formal family root already exists")
    root.mkdir(parents=False, exist_ok=False)
    write_immutable_json(
        registration.to_dict(), root / W02_MORPH_V2_PRIVATE_REGISTRATION_DOCUMENT)
    write_immutable_json(
        freeze, root / W02_MORPH_V2_PRIVATE_FAMILY_DOCUMENT)
    (root / W02_MORPH_V2_PRIVATE_EXPOSURE_LEDGER).mkdir(exist_ok=False)
    guard = w02_morphology_successor_v2_private_guard_value(
        family_commitment=registration.family_commitment,
...
def w02_morphology_successor_v2_private_run_identity(
    ...
    return hashlib.sha256(canonical_json_bytes({
        ...
        "family_freeze_sha256": _sha256(
            family_freeze_sha256, where="private family freeze"),
...
def _intent_value(
    ...
    return {
        "artifact_kind": (
            "PH2_D03_V2_W02_MORPHOLOGY_SUCCESSOR_V2_PRIVATE_RUN_INTENT"),
        "formal_private_evaluation_runs": 1,
        "guard_sha256": _sha256(guard_sha256, where="private guard"),
        "private_payload_reads_before_guard": 0,
        "run_id": 1,
        "run_identity_sha256": _sha256(
            run_identity_sha256, where="private run identity"),
        "stage_key": "W-02",
        "status": "RUN_INTENT_COMMITTED_BEFORE_PRIVATE_READ",
    }


def consume_w02_morphology_successor_v2_private_guard(
    ...
    consumed = root / Path(*PurePosixPath(
        W02_MORPH_V2_PRIVATE_GUARD_CONSUMED).parts)
    intent = root / Path(*PurePosixPath(
        W02_MORPH_V2_PRIVATE_RUN_INTENT).parts)
    if consumed.exists() or intent.exists() or not available.is_file():
        raise W02MorphologySuccessorV2PrivateFamilyError(
            "private guard is unavailable or already consumed")
    if _sha256_file(available)[1] != _sha256(
            expected_guard_sha256, where="expected private guard"):
        raise W02MorphologySuccessorV2PrivateFamilyError(
            "private available guard bytes drifted")
...
def verify_w02_morphology_successor_v2_private_consumed_guard(
        family_root: str | Path, *, expected_guard_sha256: str,
        run_identity_sha256: str) -> None:
    root = _family_root(family_root, require_exists=True)
    available = root / Path(*PurePosixPath(
    ...
    if available.exists() or not consumed.is_file() or not intent.is_file():
        raise W02MorphologySuccessorV2PrivateFamilyError(
            "private consumed guard state is incomplete")
    if (_sha256_file(consumed)[1] != expected_guard_sha256
            or read_canonical_object(intent) != _intent_value(
                guard_sha256=expected_guard_sha256,
                run_identity_sha256=run_identity_sha256)):
        raise W02MorphologySuccessorV2PrivateFamilyError(
            "private consumed guard or run intent drifted")


__all__ = [
    "W02_MORPH_V2_PRIVATE_EXPOSURE_LEDGER",
    "W02_MORPH_V2_PRIVATE_FAMILY_CODE_PATHS",
    "W02_MORPH_V2_PRIVATE_FAMILY_DOCUMENT",
    "W02_MORPH_V2_PRIVATE_FAMILY_FREEZE_PATH",
    "W02_MORPH_V2_PRIVATE_FAMILY_VERSION",
    "W02_MORPH_V2_PRIVATE_FORMAL_FAMILY_NAME",
    "W02_MORPH_V2_PRIVATE_GUARD_AVAILABLE",
    "W02_MORPH_V2_PRIVATE_GUARD_CONSUMED",
    "W02_MORPH_V2_PRIVATE_REGISTRATION_DOCUMENT",
    "W02_MORPH_V2_PRIVATE_RUN_INTENT",
    "W02MorphologySuccessorV2PrivateFamilyError",
    "build_w02_morphology_successor_v2_private_family_freeze",
    "consume_w02_morphology_successor_v2_private_guard",
    "publish_w02_morphology_successor_v2_private_family_freeze",
    "publish_w02_morphology_successor_v2_private_family_root",
    "read_w02_morphology_successor_v2_private_family_freeze",
    "verify_w02_morphology_successor_v2_private_consumed_guard",
    "w02_morphology_successor_v2_private_guard_value",
    "w02_morphology_successor_v2_private_registration_from_freeze",
    "w02_morphology_successor_v2_private_run_identity",
]
