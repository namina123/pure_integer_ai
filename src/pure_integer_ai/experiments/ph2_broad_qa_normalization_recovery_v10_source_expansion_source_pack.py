"""发布并严格回读 recovery-v10 单个新 TRAIN source family pack。"""
...
from pure_integer_ai.experiments.ph2_broad_qa_normalization_recovery_v10_source_expansion_source_pack_records import (
...
V10_SOURCE_EXPANSION_SOURCE_PACK_KIND = (
    "PH2_BROAD_QA_NORMALIZATION_RECOVERY_V10_TRAIN_SOURCE_PACK_V1")
V10_SOURCE_EXPANSION_SOURCE_PACK_STATUS = (
    "SEALED_NEW_TRAIN_SOURCE_ONLY_NOT_OBSERVED_NOT_LEARNED")

_OUTPUT_FILES = (
    ("source-files.jsonl", "V10_SOURCE_FILE_RECORDS"),
    ("translation-pairs.jsonl", "V10_TRANSLATION_PAIR_RECORDS"),
    ("source-census.jsonl", "V10_SOURCE_PACK_CENSUS"),
)
...
def _require_k_root(value: str | Path) -> Path:
    ...
    if not root.is_dir() or root.drive.upper() != "K:":
        raise BroadQaExternalDataError(
            "v10 source pack run root 必须在K盘")
    return root
...
def _within(root: Path, value: str | Path, *, label: str) -> Path:
    ...
    try:
        ...
        raise BroadQaExternalDataError(
            f"v10 source pack {label} 越出run root") from error
    return path
    ...
    try:
        ...
        for line in lines:
            ...
            if not isinstance(value, dict):
                raise BroadQaExternalDataError(
                    f"v10 source pack {label} record 非对象")
            values.append(value)
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as error:
        raise BroadQaExternalDataError(
            f"v10 source pack {label} 不可读") from error
    if (not lines or b"".join(lines) != payload
            or b"".join(canonical_json_line(item) for item in values)
            != payload):
        raise BroadQaExternalDataError(
            f"v10 source pack {label} JSONL 非规范")
    return tuple(values)
...
def _artifact(
    ...
    try:
        ...
        raise BroadQaExternalDataError(
            "v10 source pack artifact path 越界") from error
    return {
        "bytes": len(payload),
        "record_count": count,
...
def _raw_target(target: Path, relative: str) -> Path:
    ...
    try:
        ...
        raise BroadQaExternalDataError(
            "v10 source pack raw path 越界") from error
    return path
...
def _raw_artifacts(
    ...
    for relative in sorted(payloads):
        ...
        role = (
            "V10_SOURCE_RAW_LICENSE_BLOB" if relative in license_paths
            else "V10_SOURCE_RAW_LOCALE_BLOB")
...
def _require_physical_inventory(
    ...
    try:
        ...
        raise BroadQaExternalDataError(
            "v10 source pack physical inventory 不可读") from error
    if actual != expected:
        raise BroadQaExternalDataError(
            "v10 source pack physical inventory 漂移")
...
def _manifest(
    ...
    """构造单family、未观察且不增加全局family vote的manifest。"""
    return {
        "artifact_kind": V10_SOURCE_EXPANSION_SOURCE_PACK_KIND,
        "files": files,
        "format_version": 1,
        "inputs": {
        ...
        "status": V10_SOURCE_EXPANSION_SOURCE_PACK_STATUS,
...
def publish_normalization_recovery_v10_source_expansion_source_pack(
        *,
        run_root: str | Path,
        source_family: str,
        roster_dir: str | Path,
    ...
    """不可覆盖发布一个v10新TRAIN source family pack。"""
    ...
    roster_path, content_path, source, target = paths
    if (target.exists()
            or any(not path.is_dir() for path in paths[:-1])
            or any(_overlap(target, path) for path in paths[:-1])):
        raise BroadQaExternalDataError(
            "v10 source pack input/target path 非法")
    roster, content = (
        read_normalization_recovery_v10_source_expansion_source_pack_state(
            source_family=source_family,
            roster_dir=roster_path,
            content_dir=content_path,
        ))
    (payloads, source_files, pairs, census, parser_summary,
     source_policy_scope) = (
        derive_normalization_recovery_v10_source_expansion_source_pack_records(
            roster=roster,
            content=content,
...
def read_normalization_recovery_v10_source_expansion_source_pack(
    ...
    try:
        ...
        raise BroadQaExternalDataError(
            "v10 source pack manifest 不可读") from error
    if (_sha256(encoded) != expected_manifest_sha256
            ...
            != V10_SOURCE_EXPANSION_SOURCE_PACK_KIND):
        raise BroadQaExternalDataError(
            "v10 source pack manifest identity 漂移")
    family = stored.get("source_family")
    if not isinstance(family, str):
        raise BroadQaExternalDataError(
            "v10 source pack family identity 漂移")
    roster, content = (
        read_normalization_recovery_v10_source_expansion_source_pack_state(
