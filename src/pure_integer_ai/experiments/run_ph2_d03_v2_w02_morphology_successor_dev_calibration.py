"""运行唯一 PH2-D03-V2 W-02 morphology successor dev calibration。"""
...
def _failure_seal(output_root: Path, error: BaseException) -> None:
    ...
    write_immutable_json({
        "artifact_kind": (
            "PH2_D03_V2_W02_MORPHOLOGY_SUCCESSOR_DEV_CALIBRATION_FAILURE_SEAL"),
...
def main() -> int:
    ...
    if target.exists() or failure.exists():
        raise RuntimeError("successor dev formal family 已消费，不得重跑")
    try:
        report = run_w02_morphology_successor_dev_calibration(
