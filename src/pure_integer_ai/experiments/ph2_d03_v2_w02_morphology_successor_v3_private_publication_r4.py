"""PASS-only public receipt for the successor V3 R4 blind private run."""
...
W02_MORPH_V3_PRIVATE_R4_RECEIPT_VERSION = (
    "PH2-D03-V2-W02-MORPHOLOGY-SUCCESSOR-V3-PRIVATE-R4-RECEIPT-V1"
)
W02_MORPH_V3_PRIVATE_R4_RECEIPT_PATH = (
    "data/ph2/manifests/d03_v2/stages/"
    "ph2_d03_v2_w02_morphology_successor_v3_private_r4_"
    "evaluation_receipt_v1.json"
)
W02_MORPH_V3_PRIVATE_R4_REPORT = "publication/run-000001.report.json"
W02_MORPH_V3_PRIVATE_R4_ZERO_WRITE_KEYS = frozenset({
...
W02_MORPH_V3_PRIVATE_R4_ZERO_CALL_WINDOWS = (
    "BEFORE_PRIVATE_READ",
...
def _repository_file(repository: Path, relative: str) -> Path:
    ...
    if (not relative or "\\" in relative or pure.is_absolute()
        ...
        raise W02MorphologySuccessorV3PrivateR4FamilyError(
            "R4 private publication repository file is invalid")
    return target
...
def _family_root(value: str | Path) -> Path:
    ...
    if (root.name != W02_MORPH_V3_PRIVATE_R4_FORMAL_FAMILY_NAME
        ...
        raise W02MorphologySuccessorV3PrivateR4FamilyError(
            "R4 private publication family root is invalid")
    return root
...
def _pass_dimension_map(
    ...
    if not isinstance(rows, list):
        raise W02MorphologySuccessorV3PrivateR4FamilyError(
            "R4 private dimension results are not a list")
    by_key: dict[str, dict[str, object]] = {}
    for row in rows:
        if not isinstance(row, dict) or not isinstance(row.get("dimension_key"), str):
            raise W02MorphologySuccessorV3PrivateR4FamilyError(
                "R4 private dimension result is invalid")
        by_key[str(row["dimension_key"])] = row
    if set(by_key) != set(expected):
        raise W02MorphologySuccessorV3PrivateR4FamilyError(
            "R4 private dimension inventory drifted")
    for key, denominator in expected.items():
        ...
        if (row.get("denominator") != denominator
            ...
            raise W02MorphologySuccessorV3PrivateR4FamilyError(
                "R4 private dimension did not pass its frozen denominator")
        _sha256(row.get("evidence_sha256"), where="R4 private dimension evidence")
    return by_key
...
def _validate_formal_pass_report(
    ...
    if (not isinstance(hard, list)
            ...
            or len(hard) != 9):
        raise W02MorphologySuccessorV3PrivateR4FamilyError(
            "R4 private hard-conjunct projection drifted")
    ...
    if unregistered != {
            ...
            "evidence_sha256": _sha256(
                ...
                where="R4 empty unregistered evidence")
        ...
        raise W02MorphologySuccessorV3PrivateR4FamilyError(
            "R4 private label registration did not close")
    ...
    if (report.get("artifact_kind")
        ...
        raise W02MorphologySuccessorV3PrivateR4FamilyError(
            "R4 private formal PASS report identity or resource state drifted")
    for name in (
            "source_identity_sha256", "route_semantic_sha256",
            "routed_index_semantic_sha256"):
        _sha256(report.get(name), where=f"R4 private {name}")
    if set(support) != set(W02_MORPH_V3_PRIVATE_SUPPORT_KEYS):
        raise W02MorphologySuccessorV3PrivateR4FamilyError(
            "R4 private support gate inventory drifted")


def build_w02_morphology_successor_v3_private_r4_receipt(
    ...
    report_path = root / Path(*PurePosixPath(
        W02_MORPH_V3_PRIVATE_R4_REPORT).parts)
    if report_path.is_symlink() or not report_path.is_file():
        raise W02MorphologySuccessorV3PrivateR4FamilyError(
            "R4 private formal report is missing")
    report = read_canonical_object(report_path)
    if (not isinstance(report, dict)
            or report_path.read_bytes() != canonical_json_bytes(report) + b"\n"):
        raise W02MorphologySuccessorV3PrivateR4FamilyError(
            "R4 private formal report is not canonical")
    ...
    return {
        ...
        "artifact_kind": (
            "PH2_D03_V2_W02_MORPHOLOGY_SUCCESSOR_V3_PRIVATE_R4_RECEIPT"),
        "artifact_version": W02_MORPH_V3_PRIVATE_R4_RECEIPT_VERSION,
        ...
        "stage_key": "W-02",
        "status": "W02_SUCCESSOR_V3_R4_RUNTIME_EVIDENCED",
...
__all__ = [
    "W02_MORPH_V3_PRIVATE_R4_RECEIPT_PATH",
    "W02_MORPH_V3_PRIVATE_R4_RECEIPT_VERSION",
    "W02_MORPH_V3_PRIVATE_R4_REPORT",
    "build_w02_morphology_successor_v3_private_r4_receipt",
    "publish_w02_morphology_successor_v3_private_r4_receipt",
]
