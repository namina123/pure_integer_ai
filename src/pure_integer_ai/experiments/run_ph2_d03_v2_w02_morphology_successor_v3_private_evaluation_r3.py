"""Run the only successor V3 R3 blind private evaluation."""
...
def _failure_seal(
    ...
    write_immutable_json({
        "artifact_kind": (
            "PH2_D03_V2_W02_MORPHOLOGY_SUCCESSOR_V3_PRIVATE_R3_FAILURE_SEAL"),
...
def _private_owner_root(value: str | Path, repository: Path) -> Path:
    ...
    if (root.is_symlink() or not root.is_dir()
        ...
        raise RuntimeError("R3 private owner root is invalid")
    return root
...
def _validate_pre_guard_inputs(
    ...
    for identity in files:
        ...
        if (not relative or target.is_symlink() or not target.is_file()
            ...
            raise RuntimeError("R3 private owner transport metadata drifted")
    ...
    if (candidate.artifact_manifest_sha256
        ...
        raise RuntimeError("formal artifact roots do not match the R3 freeze")
...
def main() -> int:
    ...
    if family_root.name != W02_MORPH_V3_PRIVATE_R3_FORMAL_FAMILY_NAME:
        raise RuntimeError("R3 private formal family name drifted")
    ...
    if report_path.exists() or failure_path.exists():
        raise RuntimeError("R3 private formal family has already been consumed")
    ...
    try:
        ...
        freeze_path = repository / Path(
            *W02_MORPH_V3_PRIVATE_R3_FAMILY_FREEZE_PATH.split("/"))
        family_freeze_sha = _sha256_file(freeze_path)[1]
        if (read_canonical_object(
                family_root / W02_MORPH_V3_PRIVATE_R3_FAMILY_DOCUMENT) != freeze
                or read_canonical_object(
                    family_root /
                    W02_MORPH_V3_PRIVATE_R3_REGISTRATION_DOCUMENT)
                != freeze["registration"]):
            raise RuntimeError("R3 private formal family documents drifted")
        registration = (
            w02_morphology_successor_v3_private_r3_registration_from_freeze(
                freeze))
        ...
        if [row.to_dict() for row in files] != freeze["owner_input_files"]:
            raise RuntimeError("R3 private owner inventory drifted before guard")
        ...
        roots = V2PhysicalRoots.from_paths(
            ...
            owner_root,
            family_root / W02_MORPH_V3_PRIVATE_R3_EXPOSURE_LEDGER,
        )
        report = run_w02_morphology_successor_v3_private_r3_evaluation(
        ...
        if report["status"] != "PASS":
            error = RuntimeError(
                "formal R3 private result was FAIL or NE; receipt blocked")
            _failure_seal(
