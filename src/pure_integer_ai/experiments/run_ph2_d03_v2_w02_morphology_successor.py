"""运行唯一 W-02 formal morphology successor transform。"""
...
def _failure_seal(successor_root: Path, run_id: int, error: BaseException) -> None:
    """guard 已消费时封存失败；未消费的只读拒绝不计 formal run。"""
    consumed = successor_root / Path(*W02_MORPH_SUCCESSOR_GUARD_CONSUMED.split("/"))
    if not consumed.is_file():
        return
    target = (
        successor_root / "morphology-overlay-store" / "failures"
        / f"run-{run_id:06d}.json")
    write_immutable_json({
        "artifact_kind": "PH2_D03_V2_W02_MORPHOLOGY_SUCCESSOR_FAILURE_SEAL",
...
def main() -> int:
    ...
    try:
        result = run_w02_morphology_overlay_formal(
