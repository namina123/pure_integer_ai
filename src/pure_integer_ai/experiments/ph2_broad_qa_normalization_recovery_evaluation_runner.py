"""运行唯一一次冻结的 normalization recovery evaluation family。

runner 在读取 Firefox evaluation inventory 前先发布不可覆盖 guard。PASS、FAIL
或 NE 均只发布报告，不启用 pack、不声明 mastery；异常封存 failure seal 并禁止重跑。
"""
...
NORMALIZATION_RECOVERY_FORMAL_RUN_GUARD_KIND = (
    "PH2_BROAD_QA_NORMALIZATION_RECOVERY_FORMAL_RUN_GUARD_V2")
NORMALIZATION_RECOVERY_FORMAL_REPORT_KIND = (
    "PH2_BROAD_QA_NORMALIZATION_RECOVERY_FORMAL_REPORT_V2")
NORMALIZATION_RECOVERY_FORMAL_FAILURE_KIND = (
    "PH2_BROAD_QA_NORMALIZATION_RECOVERY_FORMAL_FAILURE_SEAL_V2")
...
def _failure_seal(
    ...
    _write_immutable(path, {
        "artifact_kind": NORMALIZATION_RECOVERY_FORMAL_FAILURE_KIND,
...
def run_normalization_recovery_formal_evaluation(
        *,
        run_root: str | Path,
        repository_root: str | Path,
    ...
    family_dir = normalization_recovery_path_within(
        root, family_freeze_dir, label="normalization recovery family freeze")
    if not family_dir.is_dir():
        raise BroadQaExternalDataError(
            "normalization recovery family freeze 目录不存在")
    publication = normalization_recovery_path_within(
        root, publication_dir, label="normalization recovery publication")
    if publication.exists():
        raise BroadQaExternalDataError(
            "normalization recovery formal family 已消费或 publication 已存在")
    freeze = read_normalization_recovery_evaluation_family_freeze(
    ...
    try:
        guard = {
            "artifact_kind": NORMALIZATION_RECOVERY_FORMAL_RUN_GUARD_KIND,
        ...
        if (protocol_manifest["manifest_sha256"]
            ...
            raise BroadQaExternalDataError(
                "normalization recovery formal protocol/freeze 漂移")
        pack_manifest, outputs = read_normalization_recovery_rule_pack(
            rule_pack_dir,
        ...
        program = compile_normalization_recovery_candidate(
            evaluation_protocol_manifest=protocol_manifest,
            rule_pack_manifest=pack_manifest,
            outputs=outputs,
        )
        if (program.sha256()
                != freeze["candidate_freeze"]["candidate_program_sha256"]
                or program.transfer_profile.sha256()
                != freeze["candidate_freeze"]["transfer_profile_sha256"]):
            raise BroadQaExternalDataError(
                "normalization recovery formal clone/freeze 漂移")
        ...
        formal = {
            "artifact_kind": NORMALIZATION_RECOVERY_FORMAL_REPORT_KIND,
...
def main(argv: list[str] | None = None) -> int:
    """解析显式 K 盘路径并运行唯一 recovery formal evaluation。"""
    ...
    report = run_normalization_recovery_formal_evaluation(**vars(arguments))
