"""LC-16 SOURCE_CODE 七类 payload 与 tree adapter evidence 合同。"""
...
FORMAT_VERSION = 1
ARTIFACT_KIND = "PH2_LC16_SOURCE_CODE_CARRIER_PAYLOAD"
ARTIFACT_VERSION = "LC16-SOURCE_CODE-CARRIER-20260731-A"
ARTIFACT_STATUS = "PAYLOAD_FROZEN_TREE_ADAPTER_ONLY"
CARRIER_KEY = "SOURCE_CODE"
LICENSE_ID = "CC0-1.0"
PARSER_PACKAGE = "python-stdlib"
PARSER_VERSION = "tokenize-ast-v1"
EMPTY_SHA256 = hashlib.sha256(b"").hexdigest()
ADAPTER_OBLIGATIONS = {
    "POSITIVE": "TOKEN_AST_COMMENT_STRING_IMPORT_CALL_ROUND_TRIP",
    "NEGATIVE": "RAW_SOURCE_AND_PARSER_FAILURE_PRESERVATION",
    "AMBIGUOUS": "STRUCTURE_WITHOUT_SEMANTIC_PRESELECTION",
    "UNKNOWN": "UNKNOWN_SYNTAX_PRESERVATION",
    "REVISION": "TOKEN_AST_REVISION_MAPPING",
    "GENERATION": "EXACT_SOURCE_CODE_SERIALIZATION",
    "RETENTION": "TOKEN_AST_RELOAD_IDENTITY",
}
...
class SourceCodeCarrierContractError(RuntimeError):
    """SOURCE_CODE payload、manifest 或 evidence 身份不闭合。"""


def _exact(value: Any, expected: set[str], *, where: str) -> dict[str, Any]:
    if not isinstance(value, dict) or set(value) != expected:
        raise SourceCodeCarrierContractError(f"{where} 字段不精确")
    return value
...
def _text(
    ...
    if not isinstance(value, str) or (not allow_empty and not value):
        raise SourceCodeCarrierContractError(f"{where} 必须是文本")
    if strip and value and value.strip() != value:
        raise SourceCodeCarrierContractError(f"{where} 含首尾空白")
    return value


def _positive(value: Any, *, where: str) -> int:
    if type(value) is not int or value <= 0:
        raise SourceCodeCarrierContractError(f"{where} 必须是正严格整数")
    return value


def _nonnegative(value: Any, *, where: str) -> int:
    if type(value) is not int or value < 0:
        raise SourceCodeCarrierContractError(f"{where} 必须是非负严格整数")
    return value
...
def _sha256(value: Any, *, where: str) -> str:
    ...
    if (len(text) != 64
        ...
        raise SourceCodeCarrierContractError(f"{where} 必须是小写 SHA-256")
    return text
...
def _relative_path(value: Any, *, where: str) -> str:
    ...
    if (not path.parts or path.is_absolute() or ".." in path.parts
            ...
            or ":" in path.parts[0]):
        raise SourceCodeCarrierContractError(f"{where} 必须是安全 POSIX 相对路径")
    return text
...
def _raw_identity(
    ...
    if text:
        try:
            ...
            raise SourceCodeCarrierContractError(
                f"{where} 含非法 Unicode scalar") from error
    if _nonnegative(unit_count, where=f"{where} unit_count") != len(text):
        raise SourceCodeCarrierContractError(f"{where} unit_count 漂移")
    expected = hashlib.sha256(text.encode("utf-8")).hexdigest()
    if _sha256(sha256, where=f"{where} utf8_sha256") != expected:
        raise SourceCodeCarrierContractError(f"{where} UTF-8 SHA-256 漂移")


@dataclass(frozen=True, order=True)
class SourceCodeCarrierRecord:
    """一个绑定 parent case 且不含语义 expected label 的 SourceCode payload。"""
    ...
    def __post_init__(self) -> None:
        if self.format_version != FORMAT_VERSION:
            raise SourceCodeCarrierContractError("record format_version 非法")
        if not isinstance(self.case_key, StableRecordKey):
            raise SourceCodeCarrierContractError("record case_key 非法")
        if self.sample_kind not in SAMPLE_KINDS:
            raise SourceCodeCarrierContractError("record sample_kind 未登记")
        if self.adapter_obligation != ADAPTER_OBLIGATIONS[self.sample_kind]:
            raise SourceCodeCarrierContractError("record adapter obligation 漂移")
        if self.license_id != LICENSE_ID:
            raise SourceCodeCarrierContractError("SOURCE_CODE payload 必须为 CC0-1.0")
        _raw_identity(
        ...
        if self.sample_kind == "REVISION":
            if not self.previous_text or self.previous_text == self.raw_text:
                raise SourceCodeCarrierContractError(
                    "REVISION 必须携带不同的 previous SourceCode")
    ...
    @classmethod
    def from_dict(cls, value: dict[str, Any]) -> "SourceCodeCarrierRecord":
        raw = _exact(value, {
        ...
        }, where="SourceCodeCarrierRecord")
        try:
            case_key = StableRecordKey.from_value(
                raw["case_key"], where="record case_key")
        except Exception as error:
            raise SourceCodeCarrierContractError("record case_key 损坏") from error
        return cls(
            raw["format_version"],
            case_key,
            str(raw["sample_kind"]),
...
def read_source_code_carrier_records(
        path: str | Path,
        ) -> tuple[SourceCodeCarrierRecord, ...]:
    """严格读取七行 canonical JSONL，不容忍 BOM、空行或尾随字节。"""
    try:
        payload = Path(path).read_bytes()
        if not payload or not payload.endswith(b"\n"):
            raise SourceCodeCarrierContractError("payload 缺少终止换行")
        lines = payload.splitlines(keepends=True)
        if any(not line.endswith(b"\n") or line == b"\n" for line in lines):
            raise SourceCodeCarrierContractError("payload 含坏行或空行")
        records = []
        for line in lines:
            value = parse_canonical_json_bytes(line[:-1], require_object=True)
            if canonical_json_line(value) != line:
                raise SourceCodeCarrierContractError("payload 行不是 canonical JSON")
            records.append(SourceCodeCarrierRecord.from_dict(value))
    except SourceCodeCarrierContractError:
        raise
    except Exception as error:
        raise SourceCodeCarrierContractError("SOURCE_CODE payload 损坏") from error
    result = tuple(records)
    if len(result) != len(SAMPLE_KINDS):
        raise SourceCodeCarrierContractError("payload 必须精确包含七行")
    if tuple(item.sample_kind for item in result) != SAMPLE_KINDS:
        raise SourceCodeCarrierContractError("payload sample_kind 顺序或覆盖漂移")
    if result != tuple(sorted(result, key=lambda item: item.case_key)):
        raise SourceCodeCarrierContractError("payload 必须按 case_key 排序")
    if len({item.case_key for item in result}) != len(result):
        raise SourceCodeCarrierContractError("payload case_key 不得重复")
    return result


def write_source_code_carrier_records(
        records: tuple[SourceCodeCarrierRecord, ...],
        path: str | Path,
        ) -> Path:
    """排他或幂等写七行 payload，禁止异内容覆盖。"""
    if (not isinstance(records, tuple)
            or any(not isinstance(item, SourceCodeCarrierRecord)
                   for item in records)):
        raise SourceCodeCarrierContractError("records 类型非法")
    payload = b"".join(item.canonical_line() for item in records)
    target = Path(path)
    if target.exists():
        if not target.is_file() or target.read_bytes() != payload:
            raise SourceCodeCarrierContractError("payload 已存在且内容不同")
        return target
    ...
    try:
        ...
        raise SourceCodeCarrierContractError("payload 无法写入") from error
    return target


@dataclass(frozen=True, order=True)
class SourceCodeMaterializationIdentity:
    """一个 SourceCode case 的 adapter 输出摘要及对象计数。"""
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.case_key, StableRecordKey):
            raise SourceCodeCarrierContractError("materialization case_key 非法")
        ...
        if self.revision_count == 0:
            ...
            raise SourceCodeCarrierContractError("revision_count 必须为 0/1")
    ...
    @classmethod
    def from_dict(cls, value: dict[str, Any]) -> "SourceCodeMaterializationIdentity":
        raw = _exact(value, {
        ...
        }, where="SourceCodeMaterializationIdentity")
        return cls(
...
@dataclass(frozen=True, order=True)
class SourceCodeCarrierEvidenceFile:
    ...
    def __post_init__(self) -> None:
        ...
        if self.role not in EVIDENCE_ROLES:
            raise SourceCodeCarrierContractError("evidence role 未登记")
    ...
    @classmethod
    def from_dict(cls, value: dict[str, Any]) -> "SourceCodeCarrierEvidenceFile":
        raw = _exact(value, {
            "byte_count", "relative_path", "role", "sha256",
        }, where="SourceCodeCarrierEvidenceFile")
        return cls(
...
@dataclass(frozen=True, order=True)
class SourceCodeCarrierPayloadManifest:
    ...
    materializations: tuple[SourceCodeMaterializationIdentity, ...]
    execution_state: CanonicalJsonObject
    evidence_files: tuple[SourceCodeCarrierEvidenceFile, ...]

    def __post_init__(self) -> None:
        if self.format_version != FORMAT_VERSION:
            raise SourceCodeCarrierContractError("manifest format_version 非法")
        if (self.artifact_version != ARTIFACT_VERSION
                or self.artifact_status != ARTIFACT_STATUS):
            raise SourceCodeCarrierContractError("manifest artifact identity 漂移")
        _relative_path(
        ...
        if self.carrier_key != CARRIER_KEY or self.license_id != LICENSE_ID:
            raise SourceCodeCarrierContractError("manifest carrier/license 非法")
        if (self.parser_package != PARSER_PACKAGE
                or self.parser_version != PARSER_VERSION):
            raise SourceCodeCarrierContractError("manifest parser identity 漂移")
        if (not isinstance(self.budget, TypedCarrierBudget)
            ...
            raise SourceCodeCarrierContractError("manifest budget 非法")
        if (not isinstance(self.case_keys, tuple)
            ...
            raise SourceCodeCarrierContractError("manifest case_keys 非法")
        if self.sample_kinds != SAMPLE_KINDS:
            raise SourceCodeCarrierContractError("manifest sample_kinds 漂移")
        if (not isinstance(self.materializations, tuple)
                or any(not isinstance(item, SourceCodeMaterializationIdentity)
                       for item in self.materializations)
                or tuple(item.case_key for item in self.materializations)
                != self.case_keys):
            raise SourceCodeCarrierContractError("manifest materializations 漂移")
        if (not isinstance(self.execution_state, CanonicalJsonObject)
                or self.execution_state.to_value() != EXECUTION_STATE):
            raise SourceCodeCarrierContractError("manifest execution_state 必须全零")
        if (not isinstance(self.evidence_files, tuple)
                or any(not isinstance(item, SourceCodeCarrierEvidenceFile)
            ...
            raise SourceCodeCarrierContractError("manifest evidence_files 未闭合")
    ...
    @classmethod
    def from_dict(cls, value: dict[str, Any]) -> "SourceCodeCarrierPayloadManifest":
        raw = _exact(value, {
            ...
            "sample_sha256",
        }, where="SourceCodeCarrierPayloadManifest")
        if raw["artifact_kind"] != ARTIFACT_KIND:
            raise SourceCodeCarrierContractError("manifest artifact_kind 非法")
        try:
            return cls(
                raw["format_version"],
                ...
                tuple(SourceCodeMaterializationIdentity.from_dict(item)
                      for item in raw["materializations"]),
                CanonicalJsonObject.from_value(raw["execution_state"]),
                tuple(SourceCodeCarrierEvidenceFile.from_dict(item)
                      for item in raw["evidence_files"]),
            )
        except SourceCodeCarrierContractError:
            raise
        except Exception as error:
            raise SourceCodeCarrierContractError("manifest 嵌套字段损坏") from error


def read_source_code_carrier_manifest(
        path: str | Path,
        ) -> SourceCodeCarrierPayloadManifest:
    """严格回读 canonical SOURCE_CODE manifest。"""
    try:
        payload = Path(path).read_bytes()
        if not payload.endswith(b"\n") or payload.endswith(b"\n\n"):
            raise SourceCodeCarrierContractError("manifest newline 非法")
        value = parse_canonical_json_bytes(payload[:-1], require_object=True)
        manifest = SourceCodeCarrierPayloadManifest.from_dict(value)
    except SourceCodeCarrierContractError:
        raise
    except Exception as error:
        raise SourceCodeCarrierContractError("manifest 损坏") from error
    if manifest.canonical_bytes() != payload:
        raise SourceCodeCarrierContractError("manifest 不是 canonical 字节")
    return manifest


def write_source_code_carrier_manifest(
        manifest: SourceCodeCarrierPayloadManifest,
        path: str | Path,
        ) -> Path:
    """排他或幂等写 manifest，不允许异内容覆盖。"""
    if not isinstance(manifest, SourceCodeCarrierPayloadManifest):
        raise SourceCodeCarrierContractError("manifest 类型非法")
    target = Path(path)
    payload = manifest.canonical_bytes()
    if target.exists():
        if not target.is_file() or target.read_bytes() != payload:
            raise SourceCodeCarrierContractError("manifest 已存在且内容不同")
        return target
    target.parent.mkdir(parents=True, exist_ok=True)
    try:
        with target.open("xb") as handle:
        ...
        raise SourceCodeCarrierContractError("manifest 无法写入") from error
    return target


def verify_source_code_carrier_files(
        manifest: SourceCodeCarrierPayloadManifest,
        *,
    ...
    for relative_path, sha256, byte_count in paths:
        ...
        try:
            ...
            raise SourceCodeCarrierContractError("evidence 路径逃逸") from error
        if not target.is_file():
            raise SourceCodeCarrierContractError("evidence 文件缺失")
        payload = target.read_bytes()
        if ((byte_count is not None and len(payload) != byte_count)
                or hashlib.sha256(payload).hexdigest() != sha256):
            raise SourceCodeCarrierContractError("evidence 文件身份漂移")
...
__all__ = [
    ...
    "SourceCodeCarrierEvidenceFile",
    "SourceCodeCarrierPayloadManifest",
    "SourceCodeCarrierRecord",
    "SourceCodeMaterializationIdentity",
