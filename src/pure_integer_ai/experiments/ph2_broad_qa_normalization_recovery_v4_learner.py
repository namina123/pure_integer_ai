"""Recovery-v4 的 K 盘 deterministic learner 与严格恢复 reader。"""
...
from pure_integer_ai.experiments.ph2_broad_qa_normalization_recovery_v4_training_protocol import (
    read_normalization_recovery_v4_learner_input,
)


NORMALIZATION_RECOVERY_V4_LEARNER_KIND = (
    "PH2_BROAD_QA_NORMALIZATION_RECOVERY_V4_LEARNER_V1")
NORMALIZATION_RECOVERY_V4_LEARNER_STATUS = (
    "DEVELOPMENT_COMPLETE_PACK_DISABLED")
NORMALIZATION_RECOVERY_V4_CHECKPOINT_OPEN = "RECOVERY_V4_CHECKPOINT_OPEN"
NORMALIZATION_RECOVERY_V4_RESUME_MARKER_KIND = (
    "NORMALIZATION_RECOVERY_V4_RESUME_MARKER_V1")
NORMALIZATION_RECOVERY_V4_OPERATOR_FAMILY = NORMALIZATION_CONTRASTIVE_FAMILY
...
def normalization_recovery_v4_learning_material(
        protocol_dir: Path,
        expected_manifest_sha256: str,
        ) -> dict[str, object]:
    """只从 v4 protocol 读取四份 TRAIN 文件并纯派生唯一输出。"""
    values = read_normalization_recovery_v4_learner_input(
        protocol_dir,
    ...
    return {
        "manifest": manifest,
        "outputs": outputs,
        "payloads": normalization_recovery_v4_output_payloads(outputs),
...
def _prefix_counter(
    ...
    """把 v4 family emission 映射到共享 checkpoint 双计数。"""
    if (not isinstance(context, dict)
            or set(context) != {"emission_counts"}
            or not isinstance(context["emission_counts"], tuple)):
        raise BroadQaExternalDataError("v4 learner prefix context 漂移")
    return normalization_recovery_v4_prefix_output_counts(
        work=work,
...
def run_normalization_recovery_v4_learner(
        *,
        run_root: str | Path,
    ...
    """在 K 盘 fresh/resume 完整扫描 v4 TRAIN 并 manifest-last 封口。"""
    return run_materialized_learner(
        ...
        label="normalization recovery v4 learner",
        artifact_kind=NORMALIZATION_RECOVERY_V4_LEARNER_KIND,
        status=NORMALIZATION_RECOVERY_V4_LEARNER_STATUS,
        checkpoint_open_status=NORMALIZATION_RECOVERY_V4_CHECKPOINT_OPEN,
        resume_marker_kind=NORMALIZATION_RECOVERY_V4_RESUME_MARKER_KIND,
        format_version=1,
        marker_format_version=1,
        operator_family=NORMALIZATION_RECOVERY_V4_OPERATOR_FAMILY,
        output_file_roles=NORMALIZATION_RECOVERY_V4_OUTPUT_FILE_ROLES,
        material_loader=normalization_recovery_v4_learning_material,
        prefix_counter=_prefix_counter,
        fixed_manifest_fields=_FIXED_MANIFEST_FIELDS,
    )


def read_normalization_recovery_v4_learner(
        run_dir: str | Path,
        *,
        protocol_dir: str | Path,
        expected_protocol_manifest_sha256: str,
    ...
    """从 protocol 重派生并严格回读完整 v4 learner artifact。"""
...
def validate_normalization_recovery_v4_checkpoint_chain(
        *,
        chain_path: Path,
        run_id: str,
    ...
    """供审计和测试逐 revision 重算 v4 checkpoint。"""
...
__all__ = [
    "NORMALIZATION_RECOVERY_V4_CHECKPOINT_OPEN",
    "NORMALIZATION_RECOVERY_V4_LEARNER_KIND",
    "NORMALIZATION_RECOVERY_V4_LEARNER_STATUS",
    "normalization_recovery_v4_learning_material",
    "read_normalization_recovery_v4_learner",
    "run_normalization_recovery_v4_learner",
    "validate_normalization_recovery_v4_checkpoint_chain",
]
