"""冻结 normalization successor evaluation 的候选、代码与读取边界。

family freeze 只打开 evaluation protocol manifest，不读取 evaluation inventory、
reserve payload 或独立 evaluation source。候选身份由冻结 TRAIN protocol、两条
deterministic learner lineage、禁用态 rule pack 和 live code 共同决定。
"""
...
NORMALIZATION_SUCCESSOR_EVALUATION_FAMILY_KIND = (
    "PH2_BROAD_QA_NORMALIZATION_SUCCESSOR_EVALUATION_FAMILY_FREEZE_V1")
NORMALIZATION_SUCCESSOR_EVALUATION_FAMILY_STATUS = (
    "FROZEN_CANDIDATE_AND_CODE_UNCONSUMED")
NORMALIZATION_SUCCESSOR_EVALUATION_CODE_FILES = (
    "src/pure_integer_ai/experiments/"
    "ph2_broad_qa_normalization_successor_source_pack.py",
    "src/pure_integer_ai/experiments/"
    "ph2_broad_qa_normalization_successor_evaluation_protocol.py",
    "src/pure_integer_ai/experiments/"
    "ph2_broad_qa_normalization_successor_training_records.py",
    "src/pure_integer_ai/experiments/"
    "ph2_broad_qa_normalization_successor_training_protocol.py",
    "src/pure_integer_ai/experiments/"
    "ph2_broad_qa_normalization_successor_learning_records.py",
    "src/pure_integer_ai/experiments/"
    "ph2_broad_qa_normalization_successor_learner.py",
    "src/pure_integer_ai/experiments/"
    "ph2_broad_qa_normalization_successor_rule_pack.py",
    "src/pure_integer_ai/experiments/"
    "ph2_broad_qa_normalization_successor_candidate_clone.py",
    "src/pure_integer_ai/experiments/"
    "ph2_broad_qa_normalization_successor_evaluator.py",
    "src/pure_integer_ai/experiments/"
    "ph2_broad_qa_normalization_successor_evaluation_family.py",
    "src/pure_integer_ai/experiments/"
    "ph2_broad_qa_normalization_successor_evaluation_runner.py",
)
NORMALIZATION_SUCCESSOR_EVALUATION_DATA_ARGUMENTS = (
...
NORMALIZATION_SUCCESSOR_EVALUATION_IDENTITY_ARGUMENTS = (
...
def require_normalization_successor_k_run_root(value: str | Path) -> Path:
...
def normalization_successor_path_within(
        root: Path,
        value: str | Path,
        *,
        label: str,
...
def validate_normalization_successor_evaluation_paths(
        root: Path,
    ...
    if (not isinstance(root, Path) or not root.is_dir()
        ...
        raise BroadQaExternalDataError(
            "normalization successor evaluation path 参数非法")
    paths = {}
    for name in NORMALIZATION_SUCCESSOR_EVALUATION_DATA_ARGUMENTS:
        if name not in arguments:
            raise BroadQaExternalDataError(
                f"normalization successor evaluation 缺少路径参数: {name}")
        path = normalization_successor_path_within(
            root, arguments[name], label=name)
        if not path.is_dir():
            raise BroadQaExternalDataError(
                f"normalization successor evaluation 输入目录不存在: {name}")
        paths[name] = path
    for name in NORMALIZATION_SUCCESSOR_EVALUATION_IDENTITY_ARGUMENTS:
        if name not in arguments:
            raise BroadQaExternalDataError(
                f"normalization successor evaluation 缺少 identity: {name}")
        _strict_sha(arguments[name], label=name)
    return paths


def _code_identity(repository: Path) -> tuple[list[dict[str, object]], str]:
    """冻结 successor source/training/learning/evaluator 的完整 live code。"""
    files = []
    for relative in NORMALIZATION_SUCCESSOR_EVALUATION_CODE_FILES:
        path = (repository / Path(*relative.split("/"))).resolve()
        if (not path.is_relative_to(repository) or not path.is_file()):
            raise BroadQaExternalDataError(
                f"normalization successor code file 缺失: {relative}")
...
def _validate_disabled_lineage(
    ...
    for role, manifest in (
        ...
        if not _strict_equal(lineage, expected):
            raise BroadQaExternalDataError(
                "normalization successor pack/learner lineage 漂移")


def build_normalization_successor_evaluation_family_freeze(
        *,
    ...
    if not repository.is_dir():
        raise BroadQaExternalDataError(
            "normalization successor repository root 非法")
    expected_evaluation_sha = _strict_sha(
        expected_evaluation_protocol_manifest_sha256,
        label="successor expected evaluation manifest")
    expected_training_sha = _strict_sha(
        expected_training_protocol_manifest_sha256,
        label="successor expected training manifest")
    expected_fresh_sha = _strict_sha(
        expected_fresh_learner_manifest_sha256,
        label="successor expected fresh learner manifest")
    expected_resumed_sha = _strict_sha(
        expected_resumed_learner_manifest_sha256,
        label="successor expected resumed learner manifest")
    expected_pack_sha = _strict_sha(
        expected_rule_pack_manifest_sha256,
        label="successor expected rule pack manifest")
    evaluation_manifest = (
        read_normalization_successor_evaluation_manifest_only(
            evaluation_protocol_dir,
            expected_manifest_sha256=expected_evaluation_sha,
        ))
    training_values = read_normalization_successor_learner_input(
        training_protocol_dir,
    ...
    pack_manifest, pack_outputs = read_normalization_successor_rule_pack(
        rule_pack_dir,
        protocol_dir=training_protocol_dir,
        expected_protocol_manifest_sha256=expected_training_sha,
    ...
    if (training_manifest["manifest_sha256"] != expected_training_sha
        ...
        raise BroadQaExternalDataError(
            "normalization successor external manifest identity 漂移")
    ...
    program = compile_normalization_successor_candidate(
        rule_pack_manifest=pack_manifest, outputs=pack_outputs)
    code_files, code_sha = _code_identity(repository)
    candidate_identity = {
        "candidate_clone_sha256": program.sha256(),
        "conflict_count": len(program.declared_conflict_ids),
        "context_rule_count": len(program.context_rules),
        ...
        "target_rule_count": len(program.target_rules),
    ...
    freeze_core = {
        "artifact_kind": NORMALIZATION_SUCCESSOR_EVALUATION_FAMILY_KIND,
        ...
        "status": NORMALIZATION_SUCCESSOR_EVALUATION_FAMILY_STATUS,
...
def publish_normalization_successor_evaluation_family_freeze(
        *,
        run_root: str | Path,
        target_dir: str | Path,
    ...
    validate_normalization_successor_evaluation_paths(root, build_arguments)
    target = normalization_successor_path_within(
        root, target_dir, label="normalization successor family target")
    if target.exists():
        raise BroadQaExternalDataError(
            "normalization successor evaluation family target 已存在")
    freeze = build_normalization_successor_evaluation_family_freeze(
        **build_arguments)
...
def read_normalization_successor_evaluation_family_freeze(
        target_dir: str | Path,
        **build_arguments: object,
        ) -> dict[str, object]:
    ...
    try:
        ...
        raise BroadQaExternalDataError(
            "normalization successor evaluation family freeze 不可读") from error
    expected = build_normalization_successor_evaluation_family_freeze(
        **build_arguments)
    if (not isinstance(value, dict) or canonical_json_line(value) != payload
            or not _strict_equal(value, expected)):
        raise BroadQaExternalDataError(
            "normalization successor family freeze 与 live identity 漂移")
    return {**value, "manifest_sha256": _sha256(payload)}


def main(argv: list[str] | None = None) -> int:
    """不可覆盖发布或严格回读 successor evaluation family freeze。"""
    ...
    target = normalization_successor_path_within(
        root, target, label="normalization successor family target")
    if command == "publish":
        report = publish_normalization_successor_evaluation_family_freeze(
            run_root=root, target_dir=target, **arguments)
    else:
        report = read_normalization_successor_evaluation_family_freeze(
            target, **arguments)
