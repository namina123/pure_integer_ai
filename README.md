...
项目现在提供首个可独立回读的受限对话版本 `public-dialogue-v0.1-20260825`。它由公开课程
训练出的确定性整数状态 artifact 和同一来源/证据/拒答/恢复链组成，并提供 UTF-8 JSONL
机器交流入口。版本清单见
[public dialogue model v0.1 release](data/ph2/public_dialogue_model_v0_1_release.json)，
协议和 K 盘部署方式见[独立对话版本说明](docs/dialogue_protocol_v0_1.md)。这是一个可运行的
开发发布版，不是神经网络权重包；它不宣称自由生成、无限记忆或通用问答。
