...
PIDSLCA（Pure-Integer Deterministic Self-Learning Cognitive Architecture）是一个公开的探索性研究项目。它研究一个具体问题：能否在不依赖浮点运算的前提下，构造一个可在普通个人电脑上运行、可逐位复现、可审计，并能从经验中更新的认知系统。

本仓库公开参考实现、当前测试、持续集成、格式样例、开发脚本和论文材料。项目中的“自学习”描述研究目标与机制，不表示系统已经获得通用智能、自主理解、成熟对话能力或生产可用性。
...
- **轻量运行时**：主源码只依赖 Python 标准库，测试依赖单独安装。

## 公开进度

截至 2026-07-26：

- 代码已采用标准 `src/pure_integer_ai/` 包布局，可进行可编辑安装。
- 当前测试、样例、跨平台 CI、源码守卫和凭据扫描均已公开。
- 已实现纯整数基础、确定性工具、图存储、记忆与恢复、关系机制、认知流程、训练编排和评估设施。
- CPython 3.14.3 本地完整回归结果为 `3708 passed`。
- 当前研究仍在推进效率、长文本与长期上下文、正式训练资料、用户交互和真实语义泛化。

## 支持项目

这是一个没有机构经费或商业赞助的独立研究项目。捐赠主要用于公开测试与 CI、实验计算和存储、代码与论文存档，以及长期维护。

**[查看支持方式：微信、支付宝或 Ko-fi](DONATE.md)**

支持完全自愿，不改变 MIT 许可，不换取路线图优先级、私有版本或排他访问；无论是否捐赠，公开代码、测试和论文都按相同方式提供。
...
以上命令均从仓库根目录运行。项目支持 CPython 3.11 及以上版本，公开 CI 覆盖 Linux 和 Windows。
