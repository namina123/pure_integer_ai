...
**bit-identical（gate OFF·CI）**：无 number_facts.txt（CI/生产 default 无 pure_integer_ai_LOCAL_DIR）→ resolve_number_facts
...
def bootstrap_number_grounding(concept_index, edge_store: EdgeStore, backend,
    ...
    **无文件零副作用硬守（bit-identical·P0）**：facts 空 → 立即 return 0·**绝不调 concept_index.ensure /
      record_numeric / set_mark / build_refers_stable_edge / query_from**（无 pure_integer_ai_LOCAL_DIR → resolve 返 []
