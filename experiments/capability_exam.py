...
FOOTNOTE_MATH_MEASURES = ("math_measures：S5-S8 符号数学统计层产出度量（#1124·镜像 lang_measures·xform_verified/total + "
                          ...
                          "CI 无 symbolic specs（无 pure_integer_ai_LOCAL_DIR transform/inverse 文件）→ 全 0·无信号（诚实非伪装）")
...
class CapabilityReport:
    ...
    # CI 无 symbolic specs（无 pure_integer_ai_LOCAL_DIR transform/inverse）→ 0/0 → bit-identical。
...
def project_symbolic_measures(result: FormalTrainResult) -> dict[str, int]:
    ...
    **signal 非 criterion**（全统计层·非 can_ween/truth 决策层·同 lang_measures）：symbolic 路径已活（SYMBOLIC_TRANSFORM/
    ...
    CI 无 symbolic specs（无 pure_integer_ai_LOCAL_DIR transform/inverse 文件）→ generate=None 或全 0 → 返 0 分母 0 rate
