...
class DefaultRoundRunner:
    ...
    def run_round_full(self, ctx: TrainContext, item: CollectedItem,
        ...
        # （非 bit-identical）·try/finally 强翻会破测试。断奶/perf 跑设 pure_integer_ai_DAG_PATH_REACHABLE_SCOPE_MODE=1 + pure_integer_ai_DAG_HOPS=2
        ...
        try:
            ...
            # gate OFF → 全 dag_edges（既有 bit-identical）。gate ON → _hotzone_dag_edges query_from k-hop（pure_integer_ai_DAG_HOPS
            ...
            if getattr(gates, "DAG_PATH_REACHABLE_SCOPE_MODE", False):
                _h = os.environ.get("pure_integer_ai_DAG_HOPS")
...
def formal_train(config: FormalTrainConfig,
  ...
  终 dump（dump_run·per-space·新 run_id·E1 权威 base）。
    ...
    # 守「不写死」）。按 corpus 唯一 lang 集循环 resolve_is_a_facts（env pure_integer_ai_LOCAL_DIR/is_a_facts_{lang}.txt）。
    # **无文件（CI/生产 default 无 pure_integer_ai_LOCAL_DIR）→ resolve 返 [] → bootstrap 零副作用返 0 → ancestor_map 空
    ...
    # 按 corpus 唯一 lang 集循环 resolve_abstract_facts（env pure_integer_ai_LOCAL_DIR/abstract_facts_{lang}.txt）。
    # **无文件（CI/生产 default 无 pure_integer_ai_LOCAL_DIR）→ resolve 返 [] → bootstrap 零副作用返 0 → bit-identical 守回归**
    ...
    # （env pure_integer_ai_LOCAL_DIR/sense_facts_{lang}.txt）→ bootstrap_sense_candidates 种 (word→[senses]) base_count=1。
    # **无文件（CI/生产 default 无 pure_integer_ai_LOCAL_DIR）→ resolve 返 [] → bootstrap 空 short-circuit 返 0 →
    ...
    # core enum·守「不写死」）。按 corpus 唯一 lang 集循环 resolve_causes_facts（env pure_integer_ai_LOCAL_DIR/causes_facts_{lang}.txt）。
    # **无文件（CI/生产 default 无 pure_integer_ai_LOCAL_DIR）→ resolve 返 [] → bootstrap 零副作用返 0 → bit-identical 守回归**
    ...
    # resolve_mereology_facts（env pure_integer_ai_LOCAL_DIR/mereology_facts_{lang}.txt）。
    # **无文件（CI/生产 default 无 pure_integer_ai_LOCAL_DIR）→ resolve 返 [] → bootstrap 零副作用返 0 → bit-identical 守回归**
    ...
    # 按 corpus 唯一 lang 集循环 resolve_antonym_facts（env pure_integer_ai_LOCAL_DIR/antonym_facts_{lang}.txt）。
    # **无文件（CI/生产 default 无 pure_integer_ai_LOCAL_DIR）→ resolve 返 [] → bootstrap 零副作用返 0 → bit-identical 守回归**
    ...
    # 按 corpus 唯一 lang 集循环 resolve_similar_facts（env pure_integer_ai_LOCAL_DIR/similar_facts_{lang}.txt）。
    # **无文件（CI/生产 default 无 pure_integer_ai_LOCAL_DIR）→ resolve 返 [] → bootstrap 零副作用返 0 → bit-identical 守回归**
    ...
    # 镜像 struct_bind_pairs 范式·caller 不带 lang）。resolve_alias_facts（env pure_integer_ai_LOCAL_DIR/alias_facts.txt）
    ...
    # number_facts 本地文件（pure_integer_ai_LOCAL_DIR/number_facts.txt·来源① 结构化·EPI_STRUCTURED·**守「不写死」**：
    # 三↔3 来自外部数据非代码·**关联在图中**：bootstrap 种 整数概念 __int_{value} + CORR_NUMERIC 值 + 词 PURE_ALIAS 图边）。
    # **无文件（CI/生产 default 无 pure_integer_ai_LOCAL_DIR）→ resolve 返 [] → bootstrap 空 short-circuit →
    ...
    # ON + 有文件 → resolve_struct_bind_pairs 读 pure_integer_ai_LOCAL_DIR/struct_bind_pairs.txt（来源 a 教师标注·
    ...
    try:
        ...
        # 文件不存在（CI/生产 default 无 pure_integer_ai_LOCAL_DIR）→ resolve 返 [] → 不 append → bit-identical。
        ...
        # CI 无 specs（boot-inject 不进·无 pure_integer_ai_LOCAL_DIR）→ block `_xform_items`/`_rel_items` 空 → no-op → bit-identical。
