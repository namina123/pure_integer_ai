...
A new joint development evaluation now connects retrieval and evidence selection in one chain. On a 20,428-page index consisting of the random 20k slice plus frozen source pages, the 200-question development split reached 88.0% Recall@20 and 85.5% top1 source hit, with valid citations for all 159 ANSWER results. Gold-answer evidence hit was only 43.0%, below the frozen 60% gate, so the result is honestly `FAIL` and the 300-question held-out split has not been run. This narrows the next gap to page-local fact/evidence selection rather than index expansion. See the [joint evaluation report](docs/broad_qa_joint_retrieval_eval.md).
