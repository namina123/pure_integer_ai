...
A successor joint evaluation uses a completely new title domain and reaches `99.5%` Recall@20 and `99.0%` top1 source hit, with verifiable citations for all 190 ANSWER results. Full-denominator evidence hit is `107/200 = 53.5%`, below the frozen 60% gate, so the status remains `FAIL` and the 300-question held-out split has not been run. A separate audit shows that the legacy reading-comprehension gold answer exists in only `126/200` current frozen terminal pages; evidence hit within that source-covered subset is `107/126 = 84.92%`. The next gap is therefore source-version alignment between old dataset contexts and the current Wikipedia snapshot, not a lower gate or more tuning on the same development split. See the [joint evaluation report](docs/broad_qa_joint_retrieval_eval.md).

This remains a source-bound extractive preview, not free-form generation, general QA, or language weaning. Checkpointed projection, posting shards, external merge, receipt-last publication, and the one-shot held-out evidence evaluation have run in reality. See the [20k development preview](docs/broad_qa_20k_preview_en.md), [external evaluation report](docs/broad_qa_external_evidence_eval.md), and [joint evaluation report](docs/broad_qa_joint_retrieval_eval.md) for contracts, reproduction paths, and limits.
