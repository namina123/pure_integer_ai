...
The joint evaluation then closed its source-version-alignment contract. A complete census found that 7,189 of 10,061 naturally title-anchored candidate questions retain a gold answer both on the current terminal page and within the actual indexed-passage budget, for `71.4541%` raw-population coverage; the uncovered population remains explicitly reported. After excluding every consumed title domain, the new frozen family ran its 300-question held-out split exactly once and reached `300/300` Recall@20, `300/300` top1 source hit, `296/296` valid ANSWER citations, and `253/300 = 84.3333%` evidence hit, passing the predeclared 60% gate. Aggregate SHA-256 is `84bfeb9023ffa31386fb4dcd159af9d82d797c92393d5e83322210a3cf4d30f3`; the compact public receipt is [`data/ph2/broad_qa_source_aligned_formal_receipt_v1.json`](data/ph2/broad_qa_source_aligned_formal_receipt_v1.json). See the [joint evaluation report](docs/broad_qa_joint_retrieval_eval.md).

After the formal vertical slice, the repository added a 100-question interactive development pack without consuming held-out data: five public question-surface buckets with 20 questions each. It reaches `99/100` Recall@20, `99/100` top1 source hit, `97/97` valid ANSWER citations, and `87/100 = 87%` evidence hit, with source-page gold coverage `100/100`; four authored UNKNOWN/CLARIFY production probes pass `4/4`. These buckets are development accounting, not proven semantic-understanding categories. The result is scoped as `DEVELOPMENT_NON_FORMAL`; its compact receipt is [`data/ph2/broad_qa_interactive_development_receipt_v1.json`](data/ph2/broad_qa_interactive_development_receipt_v1.json).
