...
V-03 增加 run.manifest.json、独立 seal、cursor.json 和同盘 staging。
segment/seal 先就位，manifest 作为唯一可见性指针最后发布；run 目录可先存在
metrics 等非恢复文件，但已存在 manifest 时禁止覆盖。

不属于单一 space 的全局身份索引写入 ``global_identity.dump``，禁止为了复用
per-space 过滤而把全局 registry 伪装成某个空间。
"""
...
RUN_MANIFEST_SEAL_NAME = "run.manifest.sha256"
CURSOR_STATE_NAME = "cursor.json"


def _run_component(run_id: str) -> str:
    """核验 run_id 是单一目录分量，防止恢复协议跨越指定根目录。"""
    if not isinstance(run_id, str) or not run_id:
        raise ValueError("run_id 必须是非空字符串")
    if run_id in {".", ".."} or os.path.isabs(run_id):
        raise ValueError("run_id 必须是相对单目录分量")
    if os.path.basename(run_id) != run_id or "/" in run_id or "\\" in run_id:
        raise ValueError("run_id 不得包含路径分隔符")
    return run_id
...
def space_dump_path(run_dir: str, run_id: str, space_id: int) -> str:
    ...
    return os.path.join(run_dir, _run_component(run_id), f"space_{space_id}.dump")


def run_dir_of(run_dir: str, run_id: str) -> str:
    """某 run 的目录（所有 space dump 文件在此·per-space 分文件）。"""
    return os.path.join(run_dir, _run_component(run_id))


def run_staging_dir(run_dir: str, run_id: str) -> str:
    """返回未发布恢复包的同盘 staging 目录。"""
    return os.path.join(run_dir, f".{_run_component(run_id)}.staging")


def global_identity_dump_path(run_dir: str, run_id: str) -> str:
    """全局身份索引 manifest 路径，不归属任何概念或记忆 space。"""
    return os.path.join(run_dir_of(run_dir, run_id), "global_identity.dump")


def run_manifest_path(run_dir: str, run_id: str) -> str:
    """返回已发布 run manifest 路径。"""
    return os.path.join(run_dir_of(run_dir, run_id), RUN_MANIFEST_NAME)


def run_manifest_seal_path(run_dir: str, run_id: str) -> str:
    """返回 manifest 独立封印路径。"""
    return os.path.join(run_dir_of(run_dir, run_id), RUN_MANIFEST_SEAL_NAME)


def cursor_state_path(run_dir: str, run_id: str) -> str:
    """返回与恢复包同次发布的训练游标路径。"""
    return os.path.join(run_dir_of(run_dir, run_id), CURSOR_STATE_NAME)
