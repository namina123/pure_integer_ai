"""normalization recovery family freeze 与唯一 formal runner 测试。"""
...
from pure_integer_ai.experiments import (
    ph2_broad_qa_normalization_recovery_evaluation_family as family_module,
)
from pure_integer_ai.experiments import (
    ph2_broad_qa_normalization_recovery_evaluation_runner as runner_module,
)
from pure_integer_ai.experiments.ph2_broad_qa_external_data import (
    BroadQaExternalDataError,
)
from pure_integer_ai.experiments.ph2_broad_qa_normalization_recovery_evaluation_family import (
...
from pure_integer_ai.experiments.ph2_broad_qa_normalization_recovery_evaluation_runner import (
    run_normalization_recovery_formal_evaluation,
)
...
def _install_family_readers(
    ...
    """安装 manifest-only evaluation 与 deterministic fake lineage。"""
    ...
    outputs = {"evidence.jsonl": ({"value": 1},)}

    def evaluation_reader(path, *, expected_manifest_sha256):
        """只返回冻结 manifest identity，不提供 payload。"""
    ...
    def pack_reader(path, **kwargs):
        ...
        return ({
            ...
            "manifest_sha256": current["pack_sha"],
            "mastery_claimed": 0,
            "production_enabled": 0,
            "protocol_manifest_sha256": training_sha,
            "runtime_state": "LEARNED_PACK_DISABLED",
    ...
    program = SimpleNamespace(
        ...
        transfer_profile=SimpleNamespace(sha256=lambda: _sha("transfer")),
    )
    monkeypatch.setattr(
        family_module,
    ...
    monkeypatch.setattr(
        family_module, "read_normalization_recovery_learner_input",
        training_reader)
    monkeypatch.setattr(
        family_module, "read_normalization_recovery_learner", learner_reader)
    monkeypatch.setattr(
        family_module, "read_normalization_recovery_rule_pack", pack_reader)
    monkeypatch.setattr(
        family_module, "compile_normalization_recovery_candidate",
        lambda **kwargs: program)
    return current


def test_family_freeze_binds_live_code_candidate_and_zero_payload_reads(
        tmp_path: Path,
        monkeypatch: pytest.MonkeyPatch,
        ) -> None:
    """family freeze 重算 lineage/code/candidate 且不打开 evaluation payload。"""
    arguments = _arguments(tmp_path)
    state = _install_family_readers(monkeypatch, arguments)
    monkeypatch.setattr(
        family_module, "require_normalization_recovery_k_run_root",
        lambda value: Path(value).resolve())
    freeze = build_normalization_recovery_evaluation_family_freeze(**arguments)
    assert freeze["evaluation_payload_read_count"] == 0
    assert freeze["reserve_payload_read_count"] == 0
    assert freeze["evaluation_run_count"] == 0
    assert freeze["production_enabled"] == 0
    assert freeze["candidate_freeze"]["candidate_program_sha256"] == (
        _sha("candidate"))
    assert freeze["candidate_freeze"]["transfer_profile_sha256"] == (
        _sha("transfer"))
    ...
    stored = read_normalization_recovery_evaluation_family_freeze(
        target, **arguments)
    assert stored["manifest_sha256"] == report["manifest_sha256"]
    with pytest.raises(BroadQaExternalDataError, match="target 已存在"):
        publish_normalization_recovery_evaluation_family_freeze(
            run_root=tmp_path, target_dir=target, **arguments)
    state["pack_sha"] = _sha("drifted-pack")
    with pytest.raises(BroadQaExternalDataError, match="external manifest"):
        read_normalization_recovery_evaluation_family_freeze(
            target, **arguments)
...
def _install_runner_readers(
    ...
    transfer_sha = _sha("runner-transfer")
    freeze = {
        "candidate_freeze": {
            "candidate_program_sha256": candidate_sha,
            "transfer_profile_sha256": transfer_sha,
        },
    ...
    monkeypatch.setattr(
        runner_module, "require_normalization_recovery_k_run_root",
        lambda value: Path(value).resolve())
    monkeypatch.setattr(
        runner_module,
        "read_normalization_recovery_evaluation_family_freeze",
    ...
    monkeypatch.setattr(
        runner_module, "read_normalization_recovery_rule_pack",
        lambda *args, **kwargs: (pack, {}))
    program = SimpleNamespace(
        sha256=lambda: candidate_sha,
        transfer_profile=SimpleNamespace(sha256=lambda: transfer_sha),
    )
    monkeypatch.setattr(
        runner_module, "compile_normalization_recovery_candidate",
        lambda **kwargs: program)
    monkeypatch.setattr(
        runner_module, "evaluate_normalization_recovery_candidate",
...
def test_formal_runner_is_unique_and_keeps_pack_disabled(
    ...
    report = run_normalization_recovery_formal_evaluation(
        run_root=tmp_path,
