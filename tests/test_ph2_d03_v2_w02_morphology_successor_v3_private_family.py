"""Public tests for the successor V3 private family freeze and guard."""
...
from pure_integer_ai.experiments.ph2_d03_v2_w02_morphology_successor_v3_private_publication import (
    W02_MORPH_V3_PRIVATE_ZERO_CALL_WINDOWS,
    W02_MORPH_V3_PRIVATE_ZERO_WRITE_KEYS,
    _validate_formal_pass_report,
)
from pure_integer_ai.experiments.run_ph2_d03_v2_w02_morphology_successor_v3_private_evaluation import (
    _failure_seal,
)
...
def _formal_pass_report(
    ...
    return {
        ...
        "zero_write_audit": {
            key: 0 for key in W02_MORPH_V3_PRIVATE_ZERO_WRITE_KEYS
        },
    }


def test_v3_private_family_freeze_is_metadata_only_and_binds_route_passes() -> None:
    value = build_w02_morphology_successor_v3_private_family_freeze(
        _repository())
    assert value["status"] == "W02_SUCCESSOR_V3_BLIND_PRIVATE_FAMILY_FROZEN"
    ...
    assert value["formal_successor_v3_route_dev_runs"] == 1
    assert value["formal_successor_v3_route_shadow_runs"] == 1
    ...
    assert tuple(row["repository_file"] for row in value["code_files"]) == (
        W02_MORPH_V3_PRIVATE_FAMILY_CODE_PATHS)
    assert w02_morphology_successor_v3_private_registration_from_freeze(
        value).to_dict() == value["registration"]


def test_v3_private_guard_commits_intent_and_is_single_use(tmp_path: Path) -> None:
    freeze = build_w02_morphology_successor_v3_private_family_freeze(
        _repository())
    root = tmp_path / W02_MORPH_V3_PRIVATE_FORMAL_FAMILY_NAME
    root.mkdir()
    ...
    write_immutable_json(
        guard, root / Path(*W02_MORPH_V3_PRIVATE_GUARD_AVAILABLE.split("/")))
    run_identity = "b" * 64
    consume_w02_morphology_successor_v3_private_guard(
        root,
...
def test_v3_private_pass_projection_rejects_ne_without_threshold_change() -> None:
    freeze = build_w02_morphology_successor_v3_private_family_freeze(
        _repository())
...
def test_v3_private_runner_failure_seal_is_append_only(tmp_path: Path) -> None:
    publication = tmp_path / "publication"
    publication.mkdir()
    error = RuntimeError("synthetic-v3-private-runner-failure")
    ...
    assert "synthetic-v3-private-runner-failure" not in target.read_text("utf-8")
