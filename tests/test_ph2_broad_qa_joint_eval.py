...
def _database(path: Path, pages: tuple[tuple[int, str, str], ...]) -> None:
    ...
    """)
    postings = {}
    for doc_id, (page_id, title, text) in enumerate(pages, start=1):
        passage_id = doc_id * 128 + 1
        ...
        connection.execute(
            ...
            (passage_id, doc_id, 1, 0, len(text), text_sha, text,
             text_sha, ""))
    ...
    metadata = {
        ...
        "passage_count": str(len(pages)), "selection_sha256": "a" * 64,
...
def test_joint_freeze_excludes_prior_titles_and_keeps_labels_separate(
        tmp_path: Path) -> None:
    """successor family 不复用旧标题域，questions 不泄漏标题或答案。"""
    ...
    report = freeze_joint_source_pack(
        items, prior_question_paths=(prior,), target_dir=tmp_path / "pack",
        source_report={"accepted_question_count": len(items)},
        dev_per_source=5, held_out_per_source=5)
    assert report["excluded_prior_title_count"] == 1
...
def test_joint_augmentation_prediction_and_scoring_close_end_to_end(
    ...
    _database(extra, ((2, "都江堰", "都江堰由李冰主持修建，用于防洪。"),))
