"""D-02D.4 AUTHORED_CC0_V1 typed EXISTS 资料包 T0。"""
...
SAMPLE_PATH = Path("data/ph2/authored_logic_exists_seed_v1.jsonl.sample")
SOURCE_PATHS = (
    ...
    Path("src/pure_integer_ai/experiments/ph2_authored_exists_course.py"),
)
...
def _exists(states: tuple[LogicEvidenceState, ...], closed: bool):
    ...
    if closed and all(item.refute for item in states):
        return LogicEvidenceState(False, True)
    return LogicEvidenceState(False, False)


def test_sample_covers_witness_closed_open_empty_type_and_quantifier_swap():
    """EXISTS 覆盖 witness、closed/open 差异、空域、类型失败和恢复链。"""
    seeds = read_authored_exists_seeds(SAMPLE_PATH)
    assert len(seeds) == 12
    assert LICENSE_ID == "CC0-1.0"
    assert {item.operator_family for item in seeds} == {"EXISTS"}
    ...
    states = set()
    for seed in seeds:
        assert seed.operator_kind == OPERATOR_EXISTS
        assert seed.structure_kind == STRUCTURE_EXISTS
        assert seed.instruction_kind == INSTRUCTION_EXISTS
        assert seed.body_role_kind == ROLE_EXISTS_BODY
        assert seed.value_role_kind == ROLE_EXISTS_VALUE
        ...
        if seed.perturbation_kind != "DOMAIN_TYPE_MISMATCH":
            ...
            result = _exists(value_states, bool(seed.domain.closed))
            states.add((result.support, result.refute))
    assert states == {
        (True, False), (False, True), (False, False), (True, True)}


def test_closed_and_open_exhaustion_diverge_without_closed_world_guess():
    """同为全 refute，closed EXISTS 才为 false，open 必须保持 unknown。"""
    seeds = read_authored_exists_seeds(SAMPLE_PATH)
    closed = next(item for item in seeds
                  if item.seed_id == "teacher-exists-closed-all-refute-v1")
    opened = next(item for item in seeds
                  if item.seed_id == "teacher-exists-open-all-refute-v1")
    ...
    assert _exists(closed_states, True) == LogicEvidenceState(False, True)
    assert _exists(open_states, False) == LogicEvidenceState(False, False)


def test_compiler_is_bit_identical_and_owner_separated(tmp_path):
    """两目录 bit-identical，12/12/9/3 分账并保持 expected 私有。"""
    first = compile_authored_exists_course(SAMPLE_PATH, tmp_path / "first")
    second = compile_authored_exists_course(SAMPLE_PATH, tmp_path / "second")
...
def test_payload_roundtrips_binder_variable_domain_candidate_and_budget(tmp_path):
    """恢复 Binder/Variable/SetExpr/domain、operator candidate、bound root 和预算。"""
    build = compile_authored_exists_course(SAMPLE_PATH, tmp_path)
    for observation in _records(build, RECORD_OBSERVATION):
        ...
        assert definition.slots == (OperatorSlot(
            authored_logic_role_identity(ROLE_EXISTS_BODY), 0),)
        ...
        assert payload["open_domain_exhaustion_is_false"] == 0
        assert payload["current_absence_is_counterexample"] == 0
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda rows: rows[0].__setitem__("operator_family", "FORALL"),
         "profile"),
        (lambda rows: rows[0].__setitem__("operator_kind", 6), "profile"),
        (lambda rows: rows[0].__setitem__("structure_kind", 6), "profile"),
        (lambda rows: rows[0].__setitem__("instruction_kind", 6), "profile"),
        (lambda rows: rows[0].__setitem__("body_role_kind", 8), "Role profile"),
        (lambda rows: rows[0]["domain"].__setitem__(
            "closure_evidence_local_ids", []), "closure evidence"),
        (lambda rows: (
            rows[2]["domain"].__setitem__("closed", 0),
            rows[2]["domain"].__setitem__(
                "closure_evidence_local_ids", [1]),
        ), "open quantifier"),
...
def test_float_noncanonical_existing_pack_and_legacy_are_fail_closed(tmp_path):
    ...
    with pytest.raises(AuthoredQuantifierCourseError, match="规范 JSON"):
        read_authored_exists_seeds(bad_float)
    bad_json = tmp_path / "noncanonical.sample"
    bad_json.write_bytes(b'{"surface": "x"}\n')
    with pytest.raises(AuthoredQuantifierCourseError, match="规范 JSON"):
        read_authored_exists_seeds(bad_json)
    build = compile_authored_exists_course(
