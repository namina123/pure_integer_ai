...
def test_published_v5_receipt_is_historical_and_strictly_invalidated() -> None:
    target = ROOT / V5_RECEIPT_PATH
    assert hashlib.sha256(target.read_bytes()).hexdigest() == PUBLISHED_V5_SHA256
    value = read_v5_performance_successor_receipt(ROOT, verify_current=False)
    assert value["status"] == "PERFORMANCE_SUCCESSOR_EVIDENCED"
    assert value["prior_successor_receipt"]["sha256"] == PUBLISHED_V4_SHA256
    assert value["readiness_transition"] == {
        "LANGUAGE_READINESS_REPUBLISHED": 0,
        "PW00A_STARTED": 0,
    }
...
def test_published_v7_receipt_is_canonical_and_current() -> None:
    target = ROOT / V7_RECEIPT_PATH
    assert hashlib.sha256(target.read_bytes()).hexdigest() == PUBLISHED_V7_SHA256
    value = read_v7_performance_successor_receipt(ROOT)
    assert value["status"] == "PERFORMANCE_SUCCESSOR_EVIDENCED"
    assert value["prior_successor_receipt"]["sha256"] == PUBLISHED_V6_SHA256
    assert value["readiness_transition"] == {
        "LANGUAGE_READINESS_REPUBLISHED": 0,
        "PW00A_STARTED": 0,
    }
...
def test_v5_performance_receipt_build_is_invalidated_by_v6_source() -> None:
    with pytest.raises(ValueError, match="源码 identity 漂移"):
        build_v5_performance_successor_receipt(ROOT)


def test_v5_performance_receipt_publish_is_append_only() -> None:
    with pytest.raises(ValueError, match="禁止覆盖"):
        publish_v5_performance_successor_receipt(ROOT)


def test_v5_performance_receipt_rejects_source_and_readiness_drift(
        tmp_path: Path,
        ) -> None:
    value = read_v5_performance_successor_receipt(
        ROOT, verify_current=False)
