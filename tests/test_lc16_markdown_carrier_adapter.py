"""LC-16 MARKDOWN tree/render-order adapter and catalog contract tests."""
...
from pure_integer_ai.experiments import ph2_markdown_carrier_catalog as catalog
...
from pure_integer_ai.experiments.ph2_markdown_carrier_catalog import (
...
_SAMPLE = _ROOT / Path(*MARKDOWN_CARRIER_SAMPLE_PATH.split("/"))


@pytest.fixture(scope="module")
def records():
    return read_markdown_carrier_records(_SAMPLE)


@pytest.fixture(scope="module")
def manifest():
    return build_markdown_carrier_manifest(_ROOT)


def test_sample_freezes_seven_cc0_markdown_payloads(records):
    ...
    assert records[1].raw_text.endswith("下一行")
    assert records[3].raw_text.startswith(":::future")
    assert records[5].raw_text.endswith("\n")


def test_adapter_preserves_raw_tree_paths_and_render_ordinals(records):
    for record in records:
        materialization = adapt_markdown_carrier_record(record)
        ...
        payload = serialize_markdown_carrier_materialization(materialization)
        assert deserialize_markdown_carrier_materialization(payload, record) == materialization


def test_unknown_extension_and_markdown_attributes_remain_in_token_receipts(records):
    unknown = next(item for item in records if item.sample_kind == "UNKNOWN")
    materialization = adapt_markdown_carrier_record(unknown)
    receipts = [bytes(node.qualifiers) for node in materialization.structure_nodes]
    assert any(b":::future" in payload for payload in receipts)
    ambiguous = next(item for item in records if item.sample_kind == "AMBIGUOUS")
    ambiguous_materialization = adapt_markdown_carrier_record(ambiguous)
    assert any(b"title" in bytes(node.qualifiers)
               for node in ambiguous_materialization.structure_nodes)
...
def test_serializer_rejects_record_drift_and_noncanonical_bytes(records):
    ...
    payload = serialize_markdown_carrier_materialization(adapt_markdown_carrier_record(first))
    with pytest.raises(MarkdownCarrierAdapterError, match="record 身份"):
        deserialize_markdown_carrier_materialization(payload, second)
    value = parse_canonical_json_bytes(payload[:-1], require_object=True)
    value["unexpected"] = 1
    with pytest.raises(MarkdownCarrierAdapterError, match="字段不精确"):
        deserialize_markdown_carrier_materialization(canonical_json_bytes(value) + b"\n", first)
    with pytest.raises(MarkdownCarrierAdapterError, match="newline"):
        deserialize_markdown_carrier_materialization(payload + b"\n", first)
...
def test_payload_and_manifest_writers_are_idempotent_never_overwrite(records, manifest, tmp_path):
    ...
    assert write_markdown_carrier_records(records, sample_target) == sample_target
    assert write_markdown_carrier_records(records, sample_target) == sample_target
    sample_target.write_bytes(b"{}\n")
    with pytest.raises(MarkdownCarrierContractError, match="内容不同"):
        write_markdown_carrier_records(records, sample_target)
    manifest_target = tmp_path / "manifest.json"
    assert write_markdown_carrier_manifest(manifest, manifest_target) == manifest_target
    assert read_markdown_carrier_manifest(manifest_target) == manifest
    manifest_target.write_bytes(b"{}\n")
    with pytest.raises(MarkdownCarrierContractError, match="内容不同"):
        write_markdown_carrier_manifest(manifest, manifest_target)
...
def test_manifest_binds_parent_parser_budget_and_zero_execution(manifest):
    ...
    assert manifest.parser_package == "markdown-it-py"
    assert manifest.parser_version == "4.2.0"
    ...
    verify_markdown_carrier_files(manifest, repository_root=_ROOT)


def test_stored_manifest_is_current_and_rebuilds_identically(manifest):
    stored = read_markdown_carrier_manifest(
        _ROOT / Path(*MARKDOWN_CARRIER_MANIFEST_PATH.split("/")))
    rebuilt = build_markdown_carrier_manifest(_ROOT)
    assert stored == manifest == rebuilt
    assert stored.canonical_bytes() == rebuilt.canonical_bytes()
    verify_markdown_carrier_files(stored, repository_root=_ROOT)
...
def test_catalog_rejects_parser_drift_and_parent_sha(manifest, monkeypatch):
    monkeypatch.setattr(catalog, "_parser_version", lambda: (_ for _ in ()).throw(
        MarkdownCarrierCatalogError("markdown parser version 漂移")))
    with pytest.raises(MarkdownCarrierCatalogError, match="parser version"):
        build_markdown_carrier_manifest(_ROOT)
...
def test_manifest_rejects_execution_claim_and_extra_fields(manifest):
    ...
    with pytest.raises(MarkdownCarrierContractError, match="全零"):
        replace(manifest, execution_state=CanonicalJsonObject.from_value(state))
    value = manifest.to_dict()
    value["unexpected"] = 1
    with pytest.raises(MarkdownCarrierContractError, match="字段不精确"):
        MarkdownCarrierPayloadManifest.from_dict(value)
