"""LC-16 MATH_NOTATION token/tree adapter and catalog contract tests."""
...
from pure_integer_ai.experiments import ph2_math_notation_carrier_catalog as catalog
...
from pure_integer_ai.experiments.ph2_math_notation_carrier_catalog import (
    MATH_NOTATION_CARRIER_MANIFEST_PATH,
    MATH_NOTATION_CARRIER_SAMPLE_PATH,
    PARENT_PACK_SHA256,
    MathNotationCarrierCatalogError,
    build_math_notation_carrier_manifest,
)
...
_SAMPLE = _ROOT / Path(*MATH_NOTATION_CARRIER_SAMPLE_PATH.split("/"))


@pytest.fixture(scope="module")
def records():
    return read_math_notation_carrier_records(_SAMPLE)


@pytest.fixture(scope="module")
def manifest():
    return build_math_notation_carrier_manifest(_ROOT)


def test_sample_freezes_seven_cc0_math_notation_payloads(records):
    ...
    assert records[1].raw_text == r"\frac{a}{b"
    assert records[3].raw_text.startswith(r"\futureop")
    assert r"\left" in records[5].raw_text


def test_adapter_preserves_raw_tree_paths_and_ordinals(records):
    for record in records:
        materialization = adapt_math_notation_carrier_record(record)
        ...
        payload = serialize_math_notation_carrier_materialization(materialization)
        assert deserialize_math_notation_carrier_materialization(payload, record) == materialization


def test_operator_operand_binder_scope_script_and_parser_states_are_separate(records):
    materializations = tuple(adapt_math_notation_carrier_record(item)
                             for item in records)
    receipts = [
    ...
    assert {item["family"] for item in receipts} == {
        "NOTATION", "PARSER_STATE", "TOKEN"}
    notation_types = {item["type"] for item in receipts
                      if item["family"] == "NOTATION"}
    parser_states = {item["type"] for item in receipts
                     if item["family"] == "PARSER_STATE"}
    assert {"BINDER", "OPERAND", "OPERATOR", "SCOPE_DELIMITER",
            "SCRIPT_MARKER"} <= notation_types
    assert {"COMMAND_SET_KNOWN", "DELIMITER_OK", "SCAN_OK",
            "UNBALANCED_DELIMITER", "UNKNOWN_COMMANDS_PRESENT"} <= parser_states
...
def test_unknown_names_and_invalid_syntax_remain_structural_observations(records):
    ...
    materialization = adapt_math_notation_carrier_record(unknown)
    receipts = [bytes(node.qualifiers) for node in materialization.structure_nodes]
    assert any(b"futureop" in payload for payload in receipts)
    negative = next(item for item in records if item.sample_kind == "NEGATIVE")
    negative_receipts = [bytes(node.qualifiers) for node in
                         adapt_math_notation_carrier_record(negative).structure_nodes]
    assert any(b"UNBALANCED_DELIMITER" in payload for payload in negative_receipts)
    assert not any(b'\"type\":\"DELIMITER_OK\"' in payload
                   for payload in negative_receipts)


def test_revision_binds_text_tree_and_structure_mappings(records):
...
def test_serializer_rejects_record_drift_and_noncanonical_bytes(records):
    ...
    payload = serialize_math_notation_carrier_materialization(adapt_math_notation_carrier_record(first))
    with pytest.raises(MathNotationCarrierAdapterError, match="record 身份"):
        deserialize_math_notation_carrier_materialization(payload, second)
    value = parse_canonical_json_bytes(payload[:-1], require_object=True)
    value["unexpected"] = 1
    with pytest.raises(MathNotationCarrierAdapterError, match="字段不精确"):
        deserialize_math_notation_carrier_materialization(canonical_json_bytes(value) + b"\n", first)
    with pytest.raises(MathNotationCarrierAdapterError, match="newline"):
        deserialize_math_notation_carrier_materialization(payload + b"\n", first)
...
def test_payload_and_manifest_writers_are_idempotent_never_overwrite(records, manifest, tmp_path):
    ...
    assert write_math_notation_carrier_records(records, sample_target) == sample_target
    assert write_math_notation_carrier_records(records, sample_target) == sample_target
    sample_target.write_bytes(b"{}\n")
    with pytest.raises(MathNotationCarrierContractError, match="内容不同"):
        write_math_notation_carrier_records(records, sample_target)
    manifest_target = tmp_path / "manifest.json"
    assert write_math_notation_carrier_manifest(manifest, manifest_target) == manifest_target
    assert read_math_notation_carrier_manifest(manifest_target) == manifest
    manifest_target.write_bytes(b"{}\n")
    with pytest.raises(MathNotationCarrierContractError, match="内容不同"):
        write_math_notation_carrier_manifest(manifest, manifest_target)
...
def test_manifest_binds_parent_parser_budget_and_zero_execution(manifest):
    ...
    assert manifest.parser_version == "latex-notation-v1"
    ...
    assert tuple(item.envelope_count for item in manifest.materializations) == (
        1, 1, 1, 1, 2, 1, 1)
    assert all(item.anchor_count > item.envelope_count
    ...
    verify_math_notation_carrier_files(manifest, repository_root=_ROOT)


def test_stored_manifest_is_current_and_rebuilds_identically(manifest):
    stored = read_math_notation_carrier_manifest(
        _ROOT / Path(*MATH_NOTATION_CARRIER_MANIFEST_PATH.split("/")))
    rebuilt = build_math_notation_carrier_manifest(_ROOT)
    assert stored == manifest == rebuilt
    assert stored.canonical_bytes() == rebuilt.canonical_bytes()
    verify_math_notation_carrier_files(stored, repository_root=_ROOT)
...
def test_catalog_rejects_parser_drift_and_parent_sha(manifest, monkeypatch):
    monkeypatch.setattr(catalog, "_parser_version", lambda: (_ for _ in ()).throw(
        MathNotationCarrierCatalogError("math_notation parser version 漂移")))
    with pytest.raises(MathNotationCarrierCatalogError, match="parser version"):
        build_math_notation_carrier_manifest(_ROOT)
...
def test_manifest_rejects_execution_claim_and_extra_fields(manifest):
    ...
    with pytest.raises(MathNotationCarrierContractError, match="全零"):
        replace(manifest, execution_state=CanonicalJsonObject.from_value(state))
    value = manifest.to_dict()
    value["unexpected"] = 1
    with pytest.raises(MathNotationCarrierContractError, match="字段不精确"):
        MathNotationCarrierPayloadManifest.from_dict(value)
