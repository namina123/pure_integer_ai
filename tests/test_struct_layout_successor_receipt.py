...
from scripts.struct_layout_successor_receipt import (
    ARTIFACT_KIND,
    BATCH2_CHANGE_COMMIT,
    BATCH2_PARENT_COMMIT,
    BATCH2_RECEIPT_PATH,
    CHANGE_COMMIT,
    PARENT_COMMIT,
    RECEIPT_PATH,
...
def test_batch2_receipt_builds_with_rejected_candidate_and_no_readiness(
        tmp_path: Path,
        ) -> None:
    value = build_batch2_struct_layout_successor_receipt(ROOT)
    assert value["change_commit"] == BATCH2_CHANGE_COMMIT
    assert value["parent_commit"] == BATCH2_PARENT_COMMIT
    assert value["receipt_relative_path"] == BATCH2_RECEIPT_PATH
    assert value["readiness_transition"] == {
        "LANGUAGE_READINESS_REPUBLISHED": 0,
        "PW00A_STARTED": 0,
    }
    assert value["benchmark_evidence"]["rejected_candidates"] == [{
    ...
    target = tmp_path / "batch2.json"
    target.write_bytes(canonical_json_bytes(value) + b"\n")
    assert read_batch2_struct_layout_successor_receipt(ROOT, target) == value


def test_batch2_receipt_publish_is_append_only(tmp_path: Path) -> None:
    target = tmp_path / "batch2.json"
    published = publish_batch2_struct_layout_successor_receipt(
        ROOT, target=target)
    assert read_batch2_struct_layout_successor_receipt(
        ROOT, target) == published
    with pytest.raises(ValueError, match="禁止覆盖"):
        publish_batch2_struct_layout_successor_receipt(ROOT, target=target)


def test_batch2_receipt_rejects_source_and_readiness_drift(
        tmp_path: Path,
        ) -> None:
    value = build_batch2_struct_layout_successor_receipt(ROOT)
    target = tmp_path / "batch2.json"
    ...
    target.write_bytes(canonical_json_bytes(altered) + b"\n")
    with pytest.raises(ValueError, match="source binding"):
        read_batch2_struct_layout_successor_receipt(ROOT, target)
